...
/// ⚙ **出厂默认 = true** —— `UTAI_REST_BUCKET_TARGET=0` 关回「桶按被借音符的长度选」。
///
/// # 为什么开（用户 2026-08-28 拍板）
/// > 「那响度问题不是这刀直接造成的 那这刀还是值得翻，它现在确实是在我们现在这个问题上
/// >  **最干净的一次**」「我们本身也判断不了这 2db 到底是"对还是不对"，但是显然
/// >  **之前给辅音套那么长是被症状暗示了更像不太对的**」「如果这一刀只是让它"掉了 2db"
/// >  而其他负面作用完全没有的话 那显然**利大于弊**」
///
/// # 依据
/// `onset_target_frames(p, fr)` 的桶原本按**被借音符**的长度选：下一个音 420 ms(21 帧)
/// ⇒ long bucket ⇒ `w` 4 帧(80 ms)、`k` 6 帧(**120 ms**)。**可它借的是那 140 ms 的休止。**
/// 短桶(≤7 帧)才是训练集（`score2cv_dur_priors.rs`，cvfinal **44947 clips**）里歌手在
/// **短情境**下唱出来的值：`w` 3 帧(60 ms)、`k` 4 帧(80 ms)。
///
/// 实测（`diag_sp_phone_dur_after_preroll`，休止 200 ms）：
/// `うぉ` SP 6→**7** 帧 / w 4→**3**；`か` SP 4→**5** / k 6→**5**；`た` SP 5→**6** / t 5→**4**；
/// 纯元音不变。**总帧数不变 ⇒ 辅音位置一个字节没动**，只是起点后移。
/// ⭐ 用户划的红线是「渲出来的辅音**位置**」，不是分配长度：
/// 「那他妈要明知道 120ms 是错的还他妈非得坚持那 120ms 那我是傻逼吗？」
///
/// # 副作用（等响之后筛的）
/// 削波 0/0；全频带最大 +1.35 dB(16-24k，听不见)；
/// ⭐ **咬字代理（音头前 60 ms 相对稳态，n=1214）Δ 仅 −0.09 dB** ⇒ 辅音变短没削弱咬字；
/// 落点 **124/124 完全不变**。
///
/// ⚠ 唯一的连带：整条响度低 2 dB —— 但那是 **`peak_normalize` 的机制问题**，不是这一刀的错：
/// 归一化前的最高峰**换了位置**（0.9598 @103.137 s → 1.1183 @214.485 s，都在深救援区），
/// 而整条响度由那个**单点**决定。见 TODO【14】：那个脆弱点本来就在，换 praat 时还要重新讨论。
/// ⛔ 波及**所有语言**；英语侧只有仪器背书（`medial_syllable_onset_gets_its_measured_target`）。
