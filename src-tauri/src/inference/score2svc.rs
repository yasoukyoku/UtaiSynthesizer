...
/// S85 dead-only 拼接编排(score 路径;sovits/rvc 两臂共用,donor 渲染由闭包提供)。
/// base = range_shift=0 的完整渲染;每个 distinct shift 渲一个完整 donor(闭包内部走
/// render_score_* 的 range_shift=s → 逆变换回写谱位 + peak-norm,与 base 同构=S85 三轮
/// 耳判验证过的拼接口径);jobs=(shift, 起帧, 止帧)@50fps(vocal_range::dead_group_windows),
/// 10ms 余弦交叉淡化(窗短于双淡化时收缩淡化宽度,绝不静默丢弃——审查 S85)。
///
/// 电平匹配(审查 S85 major):base 与 donor 各自按自身全曲峰值 peak_normalize(0.92),
/// 两个归一因子互相独立——死音短语恰是峰值来源(base 炸响压低 base 增益)或 donor 里无关
/// 段落在 shift 下渲炸(压低 donor 增益)时,拼接窗与邻域出现短语级响度台阶(病理可达
/// 6-12dB)。修 = donor 先按全曲 active-RMS 比例对齐 base(峰值病理对 RMS 不敏感;整曲
/// 95%+ 素材同谱面,比值≈两次归一因子之差)。任一侧静音(无 active 窗)⇒ 不缩放。
///
/// 帧→样本映射(审查 S85):SoVITS 逐 chunk round_half_even(hop 网格)使真实音频时轴相对
/// 名义 sr/50 有累积漂移(长曲可 >40ms)——用 base.len()/total_frames 的实测每帧样本数
/// 线性映射,端点精确、局部残差 ≤ 半 hop(RVC 网格下两者相等)。
pub fn apply_dead_only_windows(
    base: &mut [f32],
    sample_rate: u32,
    total_frames: i64,
    jobs: &[(i64, i64, i64)],
    mut donor_render: impl FnMut(i64) -> Result<Vec<f32>>,
) -> Result<()> {
    ...
    for s in shifts {
        ...
        if donor.len().abs_diff(base.len()) > spf.ceil() as usize {
            tracing::warn!(
                "range-extend(score/dead-only): donor {} samples vs base {} — windows clamped",
                donor.len(),
                base.len()
            );
        }
        ...
        for &(_, fa, fb) in jobs.iter().filter(|j| j.0 == s) {
            ...
            // 窗短于双淡化 → 收缩淡化宽度(1 帧死音也要拿到 donor 内容);完全空窗才放弃,响亮。
            let xfw = xf.min((b.saturating_sub(a)) / 2);
            if b <= a || xfw == 0 {
                tracing::warn!(
                    "range-extend(score/dead-only): window frames {fa}..{fb} degenerate after clamp ({a}..{b} samples) — NOT rescued"
                );
                continue;
            }
            for k in a..b {
...
mod tests {
    ...
    fn dead_only_splice_blends_only_the_windows() {
        // S85: 窗外逐位不动、窗心=donor、缘上 10ms 余弦 ramp 的半程 =0.5。
        ...
        let a = (10.0 / 50.0 * 48000.0) as usize; // 帧 10 → 9600
    ...
    fn dead_only_splice_matches_donor_level_to_base() {
        // S85 审查 major:base/donor 各自独立 peak-norm → 拼接窗响度台阶。修=donor 按全曲
        // active-RMS 比例对齐 base。base 恒 0.5、donor 恒 0.25 ⇒ g=2 ⇒ 窗心 ≈0.5。
    ...
    fn dead_only_splice_rescues_a_single_frame_window() {
        // S85 审查:1 帧窗曾被静默 continue(donor 白渲、审计谎报已救)——现收缩淡化宽度。
        let mut base = vec![0.0f32; 48000];
        let donor = vec![1.0f32; 48000];
