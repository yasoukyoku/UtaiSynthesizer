...
/// ⚙ 出厂默认 = Some((TRIM_HEAD_MS, TRIM_TAIL_MS)) —— 卸乘客 = **只裁尾**(S158d;`=0` 渲旧臂)
...
mod tests {
    ...
    /// 旋钮本身:**默认开 = 只裁尾**(S158d;当中翻过、撤过、又翻回来,全过程在
    /// [`TRIM_DEFAULT`] 的 doc 里),而且解析是纯函数(测试里读进程环境既会被并行污染,
    /// 又会在别人 export 了变量时**静默通过** —— S150 在 `parse_phase_lock` 上付过学费)。
    #[test]
