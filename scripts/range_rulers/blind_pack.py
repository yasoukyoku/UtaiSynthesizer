...
⇒ 这一份**只按文件内容定序**,并且把四条容易退化的规矩做成 assert。
...
## 四条硬 assert(违反就退 3 = 跑不起来,不是「读数不符」)

1. **空白对照必须逐位相同** —— 发出去之前 `max|Δ| == 0`。
2. **文件名只由内容决定** —— 落盘后回读,断言字母序 == 内容 sha1 序。
3. **KEY 不与素材同目录**,而且**不打到 stdout**(打包者已经知道答案,但别让它进转录/终端历史)。
4. **每一段都必须真的有人在唱**(峰值 + 有声帧比例)—— S148 的空白对照第一版切在**数字静音**上,
   两个文件当然「一样」,而那一组什么都没验到。
   ⚠ **这条闸只查【整段】比例 ≥50%,拦不住尾部静音**:同一场 V3 段尾带着 3.5 秒静音照样过闸,
   是我重看波形才发现的。**要更严就按首尾分段查,这一版没做。**

## ⛔ `level_match` 不是默认开着就对

**凡是被测效应【本身表现为电平】的轮次,必须写 `"none"`。** S148 一场踩了两次:
r3 的 Z2 对齐增益 −1.194 dB、r4 的 V3 −1.964 dB —— **那两个数就是效应本身**
(乘客被放掉 / 陷波被填平),对齐等于把被测的东西减掉再问用户听不听得出。
⇒ 本脚本会打印每段的实际增益;**超过 1 dB 就停下来查它是不是效应本身**。
"""
import hashlib
import io
import json
...
def main(spec_path):
    ...
    for p in spec["pairs"]:
        ...
        if not blank and spec.get("level_match", "rms") == "rms":
            ...
            if gr > 0 and gc > 0:
                ...
                # ⛔ S148 一场踩了两次:对齐增益本身**就是被测效应**(乘客被放掉 / 陷波被填平)
                #    ⇒ 对齐等于把要问的东西减掉再问用户听不听得出。
                #    脚本判不了「效应是不是电平」,所以它逼签字:>1 dB 必须在 spec 里写明理由,
                #    否则退 3。⚠ 「打印出来提醒我一下」不是判据 —— 我两次都看见了那个数还是发了出去。
                if abs(gain_db) > 1.0 and not spec.get("level_match_ack"):
                    die(f"{label}: 对齐增益 {gain_db:+.3f} dB > 1 dB —— "
                        f"⛔ 先确认它不是**被测效应本身**(S148 r3 −1.194 / r4 −1.964 都是)。"
                        f"确认无误就在 spec 里加 \"level_match_ack\": \"<为什么这个台阶不是效应>\";"
                        f"若它就是效应,改 \"level_match\": \"none\"。")
    ...
    # ⛔⛔ S150 —— 听者那份表**不许点名空白对照,也不许带逐组增益**。
    # 之前这里写 `**(空白对照)**` 并列出候选臂增益,而空白对照的增益恒为 `+0.000 dB`
    # ⇒ 两处各自单独就把它指出来了。一个知道「这一组是同一段的副本」的听者,
    # 在那一组上必然自信地答「没区别」—— 于是这条对照**验的是他会不会读表,不是他能不能分辨**,
    # 而它存在的全部意义正是后者(S148 u1 的 U3 之所以值钱,就因为用户不知道它是对照)。
    # ⚠ 同族但**更隐蔽**:阴性对照(不是空白,是仪器预言「听不出」的那一组)的增益通常明显小于
    # 承重组(本轮 −0.137 dB 对 −1.5~−2.3),照样能被反推出来。⇒ 增益整列移到 KEY 里。
    md += ["", "## 各组", "", "| 组 | 秒 | 区间(秒) |", "|---|---|---|"]
    for r in report:
        md.append(f"| {r['label']} | {r['seconds']} | {r['span_s'][0]}–{r['span_s'][1]} |")
    if spec.get("note"):
        md += ["", "## 备注", "", spec["note"]]
