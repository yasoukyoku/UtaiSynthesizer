...
# ⛔⛔ **参照臂自己有量程**,超出就不再是参照:本仓素材被救段 f0 中位 610 Hz,+6 后 862 Hz
#   还在量程内;**+12 就撞穿 1400 Hz 的上限** —— S146 实测 praat 臂在 +12 上包络位移读 +2.70、
#   峰值相关塌到 0.661、浊帧只剩 41.1%,而我们自己的实现同一格是 +2.50 / 0.815 / 57.1%。
#   ⇒ **|shift| > 6 时不许拿 praat 当参照臂**(那里它比被测者还差),要么抬 ceiling 重新标定,
#     要么把那一档明写成【无参照】。
PRAAT_USABLE_SHIFT_ST = 6.0
