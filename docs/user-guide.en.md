...
**NVIDIA users should install the CUDA runtime** — when the button is shown; it appears only on cards our CUDA package supports, and other NVIDIA cards run on DirectML with the reason stated in Settings: in "Settings" → "CUDA Runtime", click "Download CUDA Runtime" (about 1.6 GB). It is fully self-contained — you do not need to install the CUDA Toolkit yourself; the app downloads every runtime library automatically. **Restart the app** after the download to activate it.

**Multi-GPU machines can choose which card to use**: a "Preferred GPU" dropdown appears below "Inference Device", listing this machine's GPUs (greyed entries cannot be selected, and each row says why: a virtual system adapter, a duplicate entry for one card the system enumerated twice, a card our CUDA package cannot run, or a card whose compute capability could not be read). Under "Auto" it defaults to "Automatic (high-performance first)" but can be set to a specific card — "Auto" still decides between CUDA and DirectML; "Preferred GPU" only decides which card.
...
- **"Phonemes"**: every phone this track will actually sing, drawn as a coloured block whose width is the duration it really got. Read it against the piano roll right above: **a consonant block reaching left of its note's start was moved ahead of the beat** (see "Automatic phoneme timing" in 5.10). It runs the very allocator the render uses, so what you see here is what will be sung.
...
- **Drag the boundary between two phones of the same note** to change their **duration** split — hovering near a boundary switches straight to the left-right resize cursor, no modifier key needed (away from a boundary you are back on the vertical strength drag). The note's total length never moves (one side grows at the other's expense), so an edit can never push other notes around. Draggable boundaries carry a bright vertical handle — vertical lines without a handle are just block outlines and are not meant to move. What you see while dragging is exactly the split that will stick: the preview runs the same integer allocation math as the renderer, so it stops at the floor instead of snapping back on release.
...
⚠ **v0.12 correction**: this used to read "turn it off when importing a UTAU CVVC / VCCV phoneme score". **That was wrong** and has been corrected. UTAU keeps a note's pre-utterance in the note's own `PreUtterance` field, and the engine moves the SAMPLE, never the note's position; Two measurements on the four reference USTs make that concrete: (1) in the three parallel scores 486-489 notes each carry a numeric `PreUtterance` (median 82-120 ms), while the fourth leaves it blank and inherits it from the voicebank's `oto.ini` — either way it is a property of the SAMPLE; (2) against the 1/8-beat (60-tick) unit these files are actually written on, the median start offset is **0 for every onset class**, and longer consonants do not start earlier (nasals are in fact the latest) — a hand-made pre-shift would leave exactly that fingerprint, and it is absent. The head start is therefore applied once (by us) — **leave this ON for alias scores too**.

The cost, stated plainly: the consonant now has to come out of *this note's* own duration, so it can never take more than half of it, and on a note too short to spare even 2 frames (40 ms) that consonant is dropped. To see exactly how a note was divided, open the phoneme lane (see 5.9) — it redraws the moment you flip the switch, and you can see at a glance how far ahead of its note's start the consonant block reaches.
...
**Export a model / Import a package** (moving a model to another machine): every voice-model row has an **Export** button — it bundles the model into a single `.zip` package holding its whole file set (ONNX weights + config + retrieval/cluster + diffusion attachment + avatar — the same set that 7.4's "Delete model" removes, minus this machine's audition cache). Copy that `.zip` to another computer, pick it with the **Import Package** button at the top of the "Voice Models" tab, and the model is **restored exactly** — **including the SoVITS retrieval/cluster bank and the avatar** (which a plain "+ Import Model" of a bare `.onnx` does NOT carry over, so use the package for moving or sharing a model). After importing you jump to the matching sub-tab. Since v0.12, pressing **Export** first asks which format you want: the **UTAI package (.zip)** (the lossless round-trip bundle above) or the **community format** (`.pth` and friends written **straight into a folder you pick**, no zip — for other tools). The community files come from one of two places: locally trained models resolve through the training-side export ledger, and **models imported on v0.12 or later keep their source `.pth` (plus the SoVITS config) inside the model family** — it travels with the .zip package too, so they can re-export any time. Models imported earlier have no retained source; re-import once to unlock. When no source is found the option is greyed out with the reason. Vocoders have no community-standard format and always export as .zip.
...
| "Logs" | "Clean" | |
| "Training projects" | One row per **project**; "Details" expands to the shared dataset and each architecture slot | Three actions with three different blast radii — do not confuse them:<br>· "Clean snapshots" (slot row) — removes only that architecture's periodic snapshots that nothing needs any more. Imported ones, best, the final export, resumable checkpoints, the pretrained base **and everything predating the migration** are kept; the result reads "Freed X · kept N"<br>· "Delete archives" (slot row) — removes that architecture's archives and preprocessing caches. The shared dataset and the other architectures are untouched. **On the sovits row this also removes shallow-diffusion progress** — the confirmation says how many steps<br>· "Delete project" (project row) — removes everything including the shared dataset; the confirmation itemises each architecture's size. **Irreversible**; models already imported into the resource manager are independent copies and are unaffected |
