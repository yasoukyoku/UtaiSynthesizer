# Vendored from so-vits-svc 4.0-v2 (models.py @ cf5a8fb) — VISinger2
# architecture, TRAINING classes (the inference-side port lives in
# converter/architectures/sovits_v4v2.py and is gate-verified bit-exact against
# this exact file). Training math is UNCHANGED, including the F0Decoder
# `x = torch.detach(x); x += ...` alias write-through (S68 deviation⑦): detach
# shares storage with the caller's decoder_input, so the in-place += leaks the
# f0_prenet term into every later decoder_input use — that side effect IS part
# of the v2 training distribution and MUST NOT be "fixed" here (the inference
# export accounts for it with the explicit f0d_cond input).
# Changes vs upstream (deliberate, zero-math):
#   - package-relative imports; sys.path hack dropped
#   - torch>=2 API: Generator_Noise's torch.istft real-[...,2] input was removed
#     in torch 2.x -> torch.view_as_complex on the same buffer (bit-identical
#     view, the same shim gate1_sovits_v2.py/e2e_sovits_v2_ref.py use to run the
#     upstream reference on torch 2.5)
#   - dead code pruned (never constructed/called in v2 train or infer):
#     PostF0Decoder, pad_v2, LengthRegulator (+ the parameterless `self.LR`
#     instance — no params/buffers, state_dict unaffected),
#     MultiFrequencyDiscriminator, BaseFrequenceDiscriminator, the
#     modules.stft/TorchSTFT import (only the dead discriminator used it) and
#     the unused torch.distributions import
import copy
import math
import torch
...
class Generator_Noise(torch.nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # torch>=2: real [..., 2] istft input was removed — view_as_complex is a
        # bit-identical view of the same buffer (same shim as the S68 reference
        # harnesses; upstream passed `spec` directly with return_complex=False)
        istft_x = torch.istft(torch.view_as_complex(spec.contiguous()), self.fft_size, self.hop_size, self.win_size,
                              self.window.to(amp), True, length=x.shape[2] * self.hop_size, return_complex=False)
