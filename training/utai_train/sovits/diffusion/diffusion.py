# Vendored VERBATIM from so-vits-svc 4.1-Stable diffusion/diffusion.py (@ 730930d).
# Zero changes: GaussianDiffusion training loss (q_sample + p_losses on eps) and
# the inference samplers (used by solver test()) are byte-identical to upstream.
# tqdm goes to stderr (stdout = JSONL protocol) so it is allowed to stay.
