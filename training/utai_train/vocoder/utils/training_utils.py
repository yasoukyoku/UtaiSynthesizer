# vendored: openvpi/SingingVocoders @ 4d0889c (2026-03-08) -- utils/training_utils.py
# 逐字拷贝;仅改动 = 本头注 + import 行重写到 utai_train.vocoder 命名空间(见 vendor_vocoder.py)
# + 一处登记偏离(设计红队 A4):DsModelCheckpoint 的两处 relative_to(cwd) 抽为
#   _display_path(try/except 回退绝对路径)——上游从 repo 根启动故 cwd 恒为工作区祖先,
#   我们的 sidecar CWD 与训练工作区分离(可跨盘),原式在首次 verbose 存档/清扫时
#   ValueError 崩训练。纯日志展示面,存档/清扫/步号解析行为不变。
# 其余数学/RNG/控制流与上游逐字一致。
import math
import re
...
class DsModelCheckpoint(ModelCheckpoint):
    ...
    def _save_checkpoint(self, trainer: "pl.Trainer", filepath: str) -> None:
        ...
        if self._verbose:
            relative_path = _display_path(filepath)
...
def _display_path(filepath: Path) -> Path:
    """登记偏离(设计红队 A4,见头注):上游 relative_to(cwd) 要求 CWD 是工作区
    祖先——我们的 sidecar CWD(<app>/training)与训练工作区(数据目录,可能跨盘)
    不满足,首次 verbose 存档/清扫即 ValueError 崩训练。仅用于日志展示,回退绝对
    路径;.stem 语义不变(_remove_checkpoint 的步号解析照常)。"""
    try:
        return filepath.relative_to(Path('.').resolve())
    except ValueError:
        return filepath
