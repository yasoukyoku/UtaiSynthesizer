# vendored: openvpi/SingingVocoders @ 4d0889c (2026-03-08) -- training/nsf_HiFigan_task.py
# 逐字拷贝;仅改动 = 本头注 + import 行重写到 utai_train.vocoder 命名空间(见 vendor_vocoder.py)
# + 一处登记偏离(纯显示面,零数学零RNG):spec_to_figure 的 plt.pcolor → plt.pcolormesh
#   ——上游在 1025×一万多列的全长频谱网格上逐格画 quad,每次 val 10 张图 = 用户实测
#   ~2 分钟边界停顿的大头;openvpi 自家 DiffSinger 的 HEAD commit(#302 perf:
#   plt.pcolormesh)就是同一函数族的同款修复,SingingVocoders 尚未跟进。
#   TB 标量/loss/权重/RNG 流不受任何影响。
# 其余数学/RNG/控制流与上游逐字一致;新行为一律在 harness.py/pipeline.py 实现。
import pathlib
import random
...
from utai_train.vocoder.models.nsf_HiFigan.models import Generator, AttrDict, MultiScaleDiscriminator, MultiPeriodDiscriminator
from utai_train.vocoder.modules.loss.HiFiloss import HiFiloss
from utai_train.vocoder.training.base_task_gan import GanBaseTask
from utai_train.vocoder.utils.wav2F0 import PITCH_EXTRACTORS_ID_TO_NAME, get_pitch
from utai_train.vocoder.utils.wav2mel import PitchAdjustableMelSpectrogram
...
def spec_to_figure(spec, vmin=None, vmax=None):
    ...
    # 登记偏离(见头注): pcolor→pcolormesh — DiffSinger #302 同款显示面提速
