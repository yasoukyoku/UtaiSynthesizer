...
/** MIDI-SAG 五种和弦风格（原生确定性语义，见头注释）。 */
export type ChordMidiStyle = "POP_STANDARD" | "POP_COMPLEX" | "DARK" | "RANDB" | "NOCONSTRAINT";

export const CHORD_MIDI_STYLES: readonly ChordMidiStyle[] = [
...
export interface ChordMidiOptions {
  ...
  /** 每小节和弦数（1 = 整小节，2 = 半小节）。 */
  chordsPerBar: 1 | 2;
  /** "auto"（跟随检测）或 "C" / "Cm" / "F#"…（升号记法）。无效输入回落 auto。 */
  key: string;
}

export interface ChordMidiResult {
  key: KeyEstimate;
  /** 和声化后的和弦段（网格对齐、相邻同和弦已合并）。 */
  segments: ChordSegment[];
  /** 块状 voicing（一段一块，根位形态，音区 [50,69] = C3..C#5）。 */
  notes: Note[];
  startTick: number;
  endTick: number;
  bars: number;
...
function nearestDegree(root: number, degrees: number[]): number {
  let best = 0;
...
function parseKeyOption(key: string): { tonic: number; major: boolean } | null {
...
function hashNotes(notes: ChordAnalysisNote[]): number {
  let h = 2166136261;
...
export function generateChordMidi(
  input: { notes: ChordAnalysisNote[]; timeSignature: [number, number] } & ChordMidiOptions,
): ChordMidiResult | null {
  ...
  const chordAt = (tick: number): ChordSegment | null => {
  ...
  const cellPcs = (start: number, end: number): Set<number> => {
    const s = new Set<number>();
    for (const n of notes) {
      if (n.tick >= end || n.tick + Math.max(0, n.duration) <= start) continue;
      s.add(mod12(n.pitch));
    }
    return s;
  };

  const pcsOfChord = (root: number, quality: ChordQuality): number[] =>
    (VOICING[quality] ?? VOICING.maj).map((iv) => mod12(root + iv));
   ...
   */
  const extendTriad = (root: number, quality: ChordQuality, deg: number, pcs: Set<number>, extendMaj: boolean): ChordQuality => {
    let next: ChordQuality | null = null;
    if (major) {
      if (extendMaj && (deg === 0 || deg === 3) && quality === "maj") next = "maj7";
      else if (extendMaj && deg === 4 && quality === "maj") next = "7";
      else if (deg === 1 || deg === 2 || deg === 5) { if (quality === "min") next = "min7"; }
      else if (deg === 6) { if (quality === "dim") next = "m7b5"; }
    } else {
      if (deg === 0 || deg === 3 || deg === 4) { if (quality === "min") next = "min7"; }
      else if (extendMaj && (deg === 2 || deg === 5 || deg === 6)) { if (quality === "maj") next = "maj7"; }
      else if (deg === 1) { if (quality === "dim") next = "m7b5"; }
    }
    if (next === null) return quality;
    const base = VOICING[quality] ?? VOICING.maj;
    const ext = VOICING[next] ?? VOICING.maj;
    const extPcs = new Set(ext.map((iv) => mod12(root + iv)));
    for (const iv of ext) {
      if (base.includes(iv)) continue;
      const a = mod12(root + iv);
      for (const m of pcs) {
        const d = Math.abs(a - m);
        if ((d === 1 || d === 11) && !extPcs.has(m)) return quality; // 非和弦音的小九度冲突 → 保三和弦
      }
    }
    return next;
  };
  ...
  // 逐格定和弦（网格对齐，相邻同和弦合并）。
  const cellsOut: Array<{ start: number; root: number; quality: ChordQuality }> = [];
  for (let bar = 0; bar < bars; bar++) {
    for (let c = 0; c < cells; c++) {
      ...
      let quality: ChordQuality = an ? an.quality : major ? "maj" : "min";
  ...
  const segments: ChordSegment[] = cellsOut.map((cell, i) => ({
