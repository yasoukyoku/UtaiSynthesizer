...
import { arrange, type ArrangeInput } from "./arranger";
...
function chordMelody(pitches: number[], beats: number, startTick = 0): ChordAnalysisNote[] {
  const notes: ChordAnalysisNote[] = [];
  for (let b = 0; b < beats; b++) {
    for (const p of pitches) notes.push({ tick: startTick + b * BEAT, duration: BEAT, pitch: p });
  }
  return notes;
}

/** 某小节内某轨音符的 (tick 相对小节, pitch) 对，按序。 */
function pairsInBar(notes: { tick: number; pitch: number }[], bar: number, barTicks = BAR) {
  return notes
    .filter((n) => n.tick >= bar * barTicks && n.tick < (bar + 1) * barTicks)
    .map((n) => [n.tick - bar * barTicks, n.pitch] as [number, number])
    .sort((a, b) => a[0] - b[0] || a[1] - b[1]);
}

const base = (notes: ChordAnalysisNote[], over: Partial<ArrangeInput> = {}): ArrangeInput =>
  ({ notes, tempo: 100, timeSignature: [4, 4], style: "pop", mood: "neutral", ...over });
...
describe("arrange 引擎", () => {
  ...
  it("3/4 拍号安全降级：越出小节的步被丢弃，永不越界", () => {
    ...
    for (const bar of [0, 1]) {
      ...
      // 全部鼓点都在小节内。
      for (const [t] of pairsInBar(res.drums, bar, barTicks)) expect(t).toBeLessThan(barTicks);
    }
  });
  ...
  it("跨度对齐：首音符所在小节起、末音符结束小节止", () => {
    ...
    const notes: ChordAnalysisNote[] = [];
    for (let b = 0; b < 3; b++) for (const p of [60, 64, 67]) notes.push({ tick: 100 + b * BEAT, duration: 400, pitch: p });
  ...
  it("扩充乐器：6 类新件全部非空、有序、力度合法（C 大调 2 小节）", () => {
    const res = arrange(base(chordMelody([60, 64, 67], 8)))!;
    const byTickPitch = (a: { tick: number; pitch: number }[], b: { tick: number; pitch: number }[]) => {
  ...
  ] as const)("新风格 %s：鼓/贝斯/新乐器均能产出", (style) => {
  ...
  it("外部骨架优先：传入 A 小调和弦时,输出和弦/贝斯根音跟随外部而非自估", () => {
    ...
    const extChords = [
      { startTick: 0, endTick: BAR, root: 9, quality: "min" as const, bass: null, label: "Am" },
      { startTick: BAR, endTick: 2 * BAR, root: 9, quality: "min" as const, bass: null, label: "Am" },
    ];
    const res = arrange(base(chordMelody([60, 64, 67], 8), {
  ...
  it("AI 旋律:小三和弦上不弹大三度,A 自然小调内无离调音", () => {
    ...
    for (const n of res.melody) expect(aMinorPcs.has(n.pitch % 12)).toBe(true);
  });
  ...
  it("AI 旋律:经过音按调性移调,D 大调歌曲不出现 C/F 等 C 调残留离调音", () => {
    ...
    expect(res.melody.length).toBeGreaterThan(0);
    for (const n of res.melody) expect(dMajorPcs.has(n.pitch % 12)).toBe(true);
  });
});
