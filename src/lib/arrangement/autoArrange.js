...
import { useSoundfontStore, defaultSoundfontFor, firstPresetOf, pickInstrumentSoundfont, GM_PROGRAM, } from "../../store/soundfont";
...
function collectTrackNotes(trackId) {
...
function isAutoArrangeTrackName(name) {
...
function collectStaleArrangeIds(trackId) {
...
export function clearAutoArrange(trackId) {
...
export function analyzeSourceProfile(trackId) {
    const proj = useProjectStore.getState();
 ...
 */
export async function runAutoArrange(trackId, style, mood, opts = {}) {
    const proj = useProjectStore.getState();
    ...
    const pick = (sfKind) => pickPartFont(sfKind, fonts);
    const selectedInstruments = opts.instruments ?? ["drums", "bass", "piano", "chords"];
    const partsIndex = {
    ...
    const parts = selectedInstruments
    ...
    useAppStore.getState().showToast(i18n.t("arrange.done", { count: created, bars: res.bars, style: STYLE_ZH[style], key: res.key.label }), "success");
    if (missingSf > 0) {
...
export function resolveMelodyTrackId() {
    const proj = useProjectStore.getState();
    ...
    if (app.activeTrackId && hasNotes(app.activeTrackId))
        return app.activeTrackId;
    const selTrack = app.selectedSegment?.trackId;
    if (selTrack && hasNotes(selTrack))
        return selTrack;
 ...
 */
export async function runChordMidi(trackId, opts) {
    const proj = useProjectStore.getState();
    const track = proj.tracks.find((t) => t.id === trackId);
    if (!track)
        return false;
    ...
    if (font && preset)
        t.soundfont = { fontId: font.id, presetId: preset.id, presetName: preset.name };
    const history = useHistoryStore.getState();
    history.beginTransaction();
    try {
    ...
    useAppStore.getState().showToast(i18n.t("chordMidi.done", { count: res.segments.length, bars: res.bars, key: res.key.label, track: t.name }), "success");
    if (!font || !preset) {
...
let previewSources = [];
let previewEndedCb = null;
/** 停止正在播放的编曲预览(幂等;不触发 onEnded —— 调用方自行更新 UI)。 */
export function stopAutoArrangePreview() {
    previewEndedCb = null;
...
export async function previewAutoArrange(trackId, style, mood, opts) {
    ...
    const wavs = await Promise.all(parts.map(async (p) => {
        ...
        try {
            if (picked) {
                return await invoke("render_soundfont_notes", {
            ...
            return await invoke("preview_notes_render", {
                bpm: proj.tempo,
