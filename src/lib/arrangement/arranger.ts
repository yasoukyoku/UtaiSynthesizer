...
import {
  analyzeChords,
  estimateKey,
  type ChordAnalysisNote,
  type ChordQuality,
  type ChordSegment,
  type KeyEstimate,
} from "../analysis/chordAnalysis";
import type { Note } from "../../types/project";
import { MOODS, STEP_TICKS, STYLE_TEMPLATES, type ArrangeMood, type ArrangeStyle } from "./styles";
...
export interface ArrangeInput {
  ...
  style: ArrangeStyle;
  mood: ArrangeMood;
  /** 外部扒带调性(一键扒带传入):优先于从 notes 自估,保证与干音同调。 */
  externalKey?: KeyEstimate;
  /** 外部扒带和弦段:优先于内部 analyzeChords,保证和声严格贴合干音。 */
...
export const VOICING: Record<ChordQuality, readonly number[]> = {
...
export function pcInRange(pc: number, lo: number, hi: number): number {
...
function voice(rootPc: number, quality: ChordQuality, lo: number): number[] {
...
function chordAt(chords: ChordSegment[], tick: number): ChordSegment | null {
  if (chords.length === 0) return null;
  let hit: ChordSegment | null = null;
  for (const c of chords) {
    if (c.startTick <= tick) hit = c;
    else break;
  }
  return hit ?? chords[0]!;
}
...
export function arrange(input: ArrangeInput): ArrangeOutput | null {
  ...
  const chords =
    input.externalChords && input.externalChords.length > 0
  ...
  for (const n of notes) {
    if (n.tick < firstTick) firstTick = n.tick;
  ...
  const pad: Note[] = [];
  // 扩充乐器输出缓冲。
  const guitarArp: Note[] = [];
  const guitarStrum: Note[] = [];
  const strings: Note[] = [];
  const epiano: Note[] = [];
  const synthPad: Note[] = [];
  const pluck: Note[] = [];
  ...
  for (let bar = 0; bar < bars; bar++) {
    ...
    const quality: ChordQuality = cs ? cs.quality : "maj";
    ...
    const pushDrum = (pitch: number, step: number, vel: number) => {
      if (step >= maxStep) return; // 非 4/4:越出小节的步直接丢弃
    ...
    for (const s of d.kick) pushDrum(KICK, s, 100);
    for (const s of d.snare) pushDrum(SNARE, s, 108);
    if (mood.density !== 0) for (const s of d.ghost ?? []) pushDrum(SNARE, s, 36);
    let hatSteps = d.hihat;
    if (mood.density === 0) hatSteps = hatSteps.filter((s) => s % 4 === 0);
    else if (mood.density === 2 && hatSteps.length > 0) hatSteps = Array.from({ length: maxStep }, (_, i) => i);
    for (const s of hatSteps) pushDrum(HIHAT, s, s % 4 === 0 ? 74 : 62);
    for (const s of d.openHat ?? []) pushDrum(OPEN_HAT, s, 78);
    for (const s of d.rim ?? []) pushDrum(RIM, s, 72);
    if (d.crashOnFirstBar && bar === 0) pushDrum(CRASH, 0, 112);
    if (fill) {
      // 句尾 fill:末拍军鼓 16 分爬升(整段最后一小节的最后一步换吊镲收束)。
      for (let s = Math.max(0, maxStep - 4); s < maxStep; s++) {
        if (s === maxStep - 1 && bar === bars - 1) pushDrum(CRASH, s, 112);
        else pushDrum(SNARE, s, 90 + (s - (maxStep - 4)) * 10);
      }
    }
    ...
    if (styleId === "jazz") {
      ...
      const near = (interval: number) => bassRootPitch + (interval > 6 ? interval - 12 : interval);
      ...
      for (const bn of style.bass.notes) {
        if (bn.offset >= maxStep) continue;
        const off = bn.offset % 4 === 2 ? swingDelay : 0;
        let pitch = bassRootPitch;
        if (bn.useFifth) pitch = bassRootPitch + 7 <= 45 ? bassRootPitch + 7 : bassRootPitch - 5;
        else if (bn.octaveUp) pitch += 12;
        bass.push({
          id: `b${++bId}`,
    ...
    const pushPianoChord = (tick: number, dur: number, vel: number) => {
    ...
    //    力度从起始音向末音线性衰减 (-9dB), 每个音有 ±4 velocity + ±2 tick 微抖。
    {
      ...
      for (let si = 0; si < strumPos.length; si++) {
        ...
        if (startStep + 1 >= maxStep) continue;
    ...
    // 6) Pluck 十六分琶音:voicing 上行循环,短促颗粒感(稀疏情绪只留正拍)。
    {
      ...
      for (let s = 0; s < maxStep; s++) {
        if (mood.density === 0 && s % 4 !== 0) continue;
  ...
  for (let i = 0; i < seedStr.length; i++) _seed = (_seed * 31 + seedStr.charCodeAt(i)) | 0;
  ...
  for (let bar = 0; bar < bars; bar++) {
    ...
    const chordQuality: ChordQuality = cs ? cs.quality : (key.major ? "maj" : "min");
    const chordRootPc = cs ? cs.root : key.tonic;
    const chordPcs: number[] = (VOICING[chordQuality] ?? VOICING.maj)
      .slice(0, 4)
    ...
    for (let n = 0; n < notesPerBar; n++) {
      ...
      if (nt >= barTick + barTicks) break;
      const isChordTone = melRand() < 0.65;
      const pc: number = (isChordTone
  ...
  const activeCount = Object.values(allParts).filter((a) => (a as Note[]).length > 0).length;
  if (activeCount >= 5) {
    const BALANCE_BIG: Record<keyof typeof allParts, number> = {
      drums: 95, bass: 82, piano: 68, pad: 55, melody: 88,
      guitarArp: 65, guitarStrum: 68, epiano: 66, strings: 58, synthPad: 52, pluck: 62,
    };
    const BALANCE_SMALL: Record<keyof typeof allParts, number> = {
    ...
    (Object.keys(allParts) as Array<keyof typeof allParts>).forEach((k) => {
      if (k === "drums") return; // drums 保留原始 velocity (MIDI punchy), 永远不做归一化
      const part = allParts[k] as Note[];
    ...
    if (activeCount >= 9) {
      for (const n of bass) {
        ...
        if (h === 0 && n.pitch < 40) n.pitch = n.pitch + 12;
      }
    }
  }

  // 稳定排序:同轨按 tick 再按 pitch,保证任何环境下列表序一致(同输入同输出)。
  const byTickPitch = (a: Note, b: Note) => a.tick - b.tick || a.pitch - b.pitch;
