...
import type { Note, Segment, Track } from "../../types/project";

const BEAT = 480;

function notesTrack(id: string, pitches: number[], beats: number): Track {
  const notes: Note[] = [];
...
function instTrack(id: string, name: string): Track {
  return {
...
function seed(tracks: Track[], tempo = 120) {
