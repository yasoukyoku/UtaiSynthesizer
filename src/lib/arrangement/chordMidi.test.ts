...
import { generateChordMidi, type ChordMidiStyle } from "./chordMidi";
import { VOICING } from "./arranger";
import type { ChordAnalysisNote, ChordQuality } from "../analysis/chordAnalysis";
...
const run = (notes: ChordAnalysisNote[], style: ChordMidiStyle, over: { chordsPerBar?: 1 | 2; key?: string } = {}) =>
  generateChordMidi({ notes, timeSignature: [4, 4], style, chordsPerBar: over.chordsPerBar ?? 1, key: over.key ?? "auto" })!;

const MAJOR_PCS = [0, 2, 4, 5, 7, 9, 11];
const MINOR_PCS = [0, 2, 3, 5, 7, 8, 10];

/** 某调性（tonic + 大/小）的顺阶绝对音级。 */
const scaleOf = (tonic: number, major: boolean): number[] =>
  (major ? MAJOR_PCS : MINOR_PCS).map((pc) => (pc + tonic) % 12);
...
describe("generateChordMidi 引擎", () => {
  ...
  it("POP_COMPLEX：三和弦按级位扩七和弦（C 大调 I→maj7、V→7、ii/vi→min7）", () => {
    ...
    const notes: ChordAnalysisNote[] = [];
    const prog: Array<[number[], ChordQuality]> = [
    ...
    prog.forEach(([pcs], bar) => {
      for (let b = 0; b < 4; b++) for (const p of pcs) notes.push({ tick: bar * BAR + b * BEAT, duration: BEAT, pitch: p });
    });
  ...
  it("POP_COMPLEX：扩音与旋律构成小九度冲突时放弃扩展", () => {
    ...
    const notes: ChordAnalysisNote[] = [];
    for (let b = 0; b < 4; b++) {
      for (const p of [67, 71, 74]) notes.push({ tick: b * BEAT, duration: BEAT * 0.9, pitch: p });
  ...
  it("chordsPerBar=2：网格对齐半小节，相邻同和弦合并", () => {
    ...
    for (let b = 0; b < 8; b++) {
      ...
      for (const p of pcs) notes.push({ tick: b * BEAT, duration: BEAT, pitch: p });
    }
