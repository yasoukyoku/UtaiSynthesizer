...
let classFor = () => ({ kind: "ok" });
const invokeMock = vi.fn((_cmd, args) => invokeFails
    ? Promise.reject(new Error("validate_lyrics exploded"))
    : Promise.resolve((args?.notes ?? []).map((n) => classFor(n?.lyric ?? ""))));
vi.mock("@tauri-apps/api/core", () => ({ invoke: (c, a) => invokeMock(c, a) }));
...
const mk = (id, tick, duration, pitch) => ({ id, tick, duration, pitch, lyric: "あ", velocity: 100 });
...
function seed() {
    const seg = { id: SEG, startTick: 0, durationTicks: 2000, content: { type: "notes", notes: NOTES() } };
    const track = {
...
describe("oovWatch — frame warnings survive a split and a backend failure", () => {
    ...
    it("★ splits the phoneme-typo verdict out of the lyric verdict, as a SUBSET", async () => {
        ...
        classFor = (l) => l === "[a zzz]" ? { kind: "unknown", phone: "zzz" } : l === "qqq" ? { kind: "unknown" } : { kind: "ok" };
        const notes = [
        ...
        for (const id of app.vocalUnknownPhone[SEG])
            expect(app.vocalOov[SEG]).toContain(id);
    ...
    it("★ an ordinary OOV publishes NOTHING to the phoneme channel (the control)", async () => {
        ...
        const notes = [{ ...mk("w", 0, 480, 60), lyric: "qqq" }];
        const seg = { id: SEG, startTick: 0, durationTicks: 960, content: { type: "notes", notes } };
        useProjectStore.setState({
            name: "P",
            tracks: [{ id: "T", name: "V", trackType: "vocal", segments: [seg], voiceModel: "V",
                    volumeDb: 0, pan: 0, muted: false, solo: false, expanded: true, laneControls: {} }],
    ...
    it("★ publishes the alias hint WITHOUT touching the blocking channels", async () => {
        classFor = (l) => l === "love" ? { kind: "phones", hint: "multiple_nuclei" } : l === "qqq" ? { kind: "unknown" } : { kind: "phones" };
        const notes = [
    ...
    it("★ re-pointing the REST token re-publishes the verdicts", async () => {
        ...
        useProjectStore.setState({
            tracks: [{ ...tracks[0], segments: [{ id: SEG, startTick: 0, durationTicks: 2000, content: { type: "notes", notes } }] }],
        });
        uninstall = installOovWatch();
        await settle();
    ...
    it("★ switching the phoneme convention re-validates, and the payload carries it", async () => {
        ...
        expect(invokeMock.mock.calls[before - 1][1].phonemeSet).toBe(null);
        ...
        const last = invokeMock.mock.calls[invokeMock.mock.calls.length - 1][1];
