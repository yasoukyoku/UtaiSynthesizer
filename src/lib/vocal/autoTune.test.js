...
vi.mock("@tauri-apps/api/core", () => ({
    invoke: (cmd, args) => {
        ...
        return Promise.resolve(notes.map(() => ({
...
describe("autoTuneScalesOf — 旋钮单一读取点", () => {
    ...
    it("显式值透传", () => {
        expect(autoTuneScalesOf({ autoTuneExpr: 0.5, autoTuneVib: 1.5, autoTuneTake: 7 })).toEqual({ expr: 0.5, vib: 1.5, take: 7 });
    });
});
...
describe("applyAutoTune — silent notes are not part of the tuned line", () => {
    const mkNote = (id, tick, pitch, lyric) => ({ id, tick, duration: 480, pitch, lyric, velocity: 100 });
    function seed(notes, restToken) {
        const seg = { id: "S", startTick: 0, durationTicks: 2400, content: { type: "notes", notes } };
        const track = {
    ...
    const notesNow = () => useProjectStore.getState().tracks[0].segments[0].content.notes;
