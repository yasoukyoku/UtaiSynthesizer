...
import { quantizeSpans, offGridCount, QUANTIZE_IMPORT_KEY } from "./quantize";
 ...
 */
export async function importScoreFromDrop(path) {
    if (busy)
        return;
    busy = true;
    try {
...
async function importScoreFromPath(sel, opts) {
    try {
        const score = await invoke("import_score_file", { path: sel });
        ...
        const absSpans = (tk) => tk.notes.map((n) => ({ start: tk.start_tick + n.tick, end: tk.start_tick + n.tick + n.duration }));
        ...
        try {
            ...
            if (score.bpm != null && Number.isFinite(score.bpm) && score.bpm > 0)
                store.setTempo(score.bpm);
            if (score.time_sig)
                store.setTimeSignature(score.time_sig[0], score.time_sig[1]);
            ...
            for (const it of score.tracks) {
                if (!it.notes.length)
                    continue; // defensive: Rust already skips empty tracks
        ...
        const report = [];
        if (movedTotal > 0)
            report.push(t("import.quantMoved", { count: movedTotal }));
        if (widenedTotal > 0)
            report.push(t("import.quantWidened", { count: widenedTotal }));
        if (skippedBaked > 0)
            report.push(t("import.quantSkippedBaked", { count: skippedBaked }));
        if (report.length)
            useAppStore.getState().showToast(report.join(" · "), "info");
