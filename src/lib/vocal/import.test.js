...
function score(notes, startTick = 0, pitchDev = null) {
    return {
        tracks: [
            {
                name: "V",
...
function answerDialog(quantize) {
...
describe("importScoreFile — S87 grid-rounding option", () => {
    ...
    it("rounding ON snaps boundaries in ABSOLUTE space and keeps legato", async () => {
        ...
        for (const n of r.notes)
            expect((r.start + n.tick) % Q).toBe(0);
    ...
    it("a BAKED pitch curve exempts its track from rounding (curve stays keyed to its notes)", async () => {
        invokeMock.mockResolvedValue(score([{ tick: 0, duration: 137, pitch: 60, lyric: "a" }, { tick: 137, duration: 223, pitch: 62, lyric: "i" }], 1000, { xs: [0, 100, 300], ys: [0, 50, -20] }));
        answerDialog(true);
        await importScoreFile();
        const r = imported();
    ...
    it("Cancel touches NOTHING (no track, no undo step)", async () => {
        ...
        useAppStore.setState({ showConfirm: async () => "" });
