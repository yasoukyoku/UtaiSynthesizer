...
import { isBreathLyric, isRestLyric, isSilentLyric, vocalTokens, ZERO_TRANSITION, DEFAULT_CONSONANT_EMPHASIS_DB, DEFAULT_CONSONANT_VALLEY } from "../vocalNotes";
...
export async function preflightVocalModels() {
...
export async function preflightAuxPack(packId) {
    try {
...
export function vocalRenderErrorMessage(e) {
    const msg = String(e);
    ...
    if (dict)
        return i18n.t("vocalEditor.render.dictMissing", { file: dict[1] });
    ...
    if (unk)
        return i18n.t("vocalEditor.render.unknownPhone", { phone: unk[1] });
    const oov = msg.match(/VOCAL_OOV:\s*(.*)$/);
    if (oov)
        return i18n.t("vocalEditor.render.oov", { lyric: oov[1] });
    const ph = msg.match(/VOCAL_PHONE_MISSING:\s*(.*)$/);
    if (ph)
        return i18n.t("vocalEditor.render.phoneMissing", { phone: ph[1] });
    if (msg.includes(VOCAL_NO_VOICE))
        return i18n.t("vocalEditor.render.noVoice");
    if (msg.includes(VOCAL_EMPTY))
        return i18n.t("vocalEditor.render.empty");
    if (msg.includes(VOCAL_RENDER_BUSY))
        return i18n.t("vocalEditor.render.busy");
    if (msg.includes(VOCAL_SPK_MIX_DIFFUSION))
        return i18n.t("vocalEditor.render.spkMixDiffusion");
...
export function isVocalCancelError(e) {
    if (/VOCAL_(OOV|DICT_MISSING|PHONE_MISSING|UNKNOWN_PHONE|ALIAS):/.test(String(e)))
        return false;
...
export const renderFrameTicks = (tempo) => Math.max(1, Math.ceil(msToTicks(1000 / RENDER_FPS, tempo)));
 ...
 */
export function buildVocalScore(notes, pitchDev, tempo, defaultTransition, tokens, paramCurves, formantScalar = 0, defaultLangId = DEFAULT_LANG_ID) {
    ...
    const { cents, voiced } = evalF0CentsFrames(pitchNotes, pitchDev, { frameStartTick: 0, ticksPerFrame, frameCount }, { tempo, defaultTransition });
    ...
    const formantEnv = formantScalar !== 0 || formantCurve
...
export function buildScoreTriples(notes, tempo, tokens, defaultLangId) {
    const ticksPerFrame = msToTicks(1000 / RENDER_FPS, tempo); // 20 ms per 50fps frame
    const frameOf = (relTick) => Math.round(relTick / ticksPerFrame);
    ...
    const mapLyric = (l) => (isRestLyric(l, tokens.rest) ? "R" : isBreathLyric(l, tokens.breath) ? "AP" : l);
    const items = [];
    const shortNoteIds = [];
    ...
    for (const n of sorted) {
        ...
        if (end <= cursor)
            continue; // fully swallowed by a previous note (defensive — notes don't overlap)
        ...
        if (!isRest && n.duration < ticksPerFrame)
            shortNoteIds.push(n.id);
        ...
        if (startF > cursorF)
            items.push({ note: null, startF: cursorF, endF: startF, lang: defaultLangId, rest: true });
    ...
    const canLend = (it) => it.endF - it.startF >= (it.rest ? MIN_KEEP_REST : MIN_KEEP_NOTE) + 1;
    ...
    const rescueIsSafe = (next) => !next || (next.rest && next.endF > next.startF);
    const borrowedNoteIds = [];
    const droppedNoteIds = [];
    ...
    const frameTick = (f) => f * ticksPerFrame;
    const spanOf = (n) => retimed.get(n.id) ?? { start: n.tick, end: n.tick + n.duration, grace: false };
    const setSpan = (n, start, end, grace) => retimed.set(n.id, { start, end: Math.max(start + ticksPerFrame * 1e-3, end), grace });
    for (let i = 0; i < items.length; i++) {
        const it = items[i];
        // Nothing to rescue for a REST that rounded away (silence is what it asked for) — only sung notes.
        if (!it.note || it.rest || it.endF > it.startF)
            continue; // not a sung note, or it already sounds
        ...
        if (prev?.note && !prev.rest)
            setSpan(prev.note, spanOf(prev.note).start, bs, spanOf(prev.note).grace);
        if (next?.note && !next.rest)
            setSpan(next.note, be, spanOf(next.note).end, spanOf(next.note).grace);
    }
    ...
    for (const it of items) {
        ...
        if (frames <= 0)
            continue; // an empty gap rest, or a note that could not borrow (already reported)
        if (!it.note) {
        ...
        const t = { lyric: mapLyric(it.note.lyric), note_num: it.rest ? 0 : it.note.pitch, frames, lang: it.lang };
        if (it.note.phonemeInput)
            t.phoneme_input = it.note.phonemeInput;
        ...
        if (pt)
            t.phone_edit = { phones: pt.phones, scale: pt.scale, gain_db: pt.gainDb ?? null };
        triples.push(t);
...
function sampleParamFrames(curve, ticksPerFrame, frameCount, transform) {
    const out = new Array(frameCount);
    for (let f = 0; f < frameCount; f++)
        out[f] = transform(evalCurveAt(curve, f * ticksPerFrame));
    return out;
}
let vocalRunSeq = 0;
/**
...
export async function renderVocalSegment(req) {
    ...
    if (useAppStore.getState().vocalRenderActive)
        throw new Error(VOCAL_RENDER_BUSY);
    ...
    const deposit = (outs) => useProjectStore.getState().replaceProcessedOutputs(trackId, segmentId, outs ?? []);
    ...
    try {
        ...
        await invoke("render_vocal_segment", {
        ...
        const info = await invoke("load_audio_file", { path: outputPath });
        if (seg()) {
        ...
        if (seg())
            deposit(prevOutputs); // restore the prior bake (re-render) or clear the first-render spinner
        throw e;
    }
    finally {
...
export function vocalRenderSig(track, seg, tempo) {
...
export async function ensureDictionarySig() {
    if (dictionarySig === null) {
        dictionarySig = await invoke("dictionary_fingerprint").catch(() => UNKNOWN_DICTIONARY_SIG);
    }
    return dictionarySig;
}
export function vocalTrackSig(track, tempo) {
...
export function resolveTrackVoice(track) {
...
export const RANGE_ALGO_VERSION = "s166e";
...
function rangeRecordSig(track) {
    ...
    const body = Object.entries(rec.speakers)
        .map(([id, sp]) => `${id}=${JSON.stringify(sp?.usable)}~${JSON.stringify(sp?.comfort)}~${hash32(JSON.stringify(sp?.semitones ?? null))}~${hash32(JSON.stringify(sp?.semitones_onset ?? null))}`)
        .sort()
        .join(",");
...
export function splitSegmentVocalAware(trackId, segId, tick, tempo) {
    ...
    if (newId)
        stampSplitWindowSigs(trackId, [segId, newId], tempo, parentWasDirty);
    return newId;
}
...
export function stampSplitWindowSigs(trackId, segIds, tempo, parentWasDirty) {
    for (const segId of segIds) {
        const track = useProjectStore.getState().tracks.find((t) => t.id === trackId);
        const seg = track?.segments.find((s) => s.id === segId);
        if (!track || !seg || seg.content.type !== "notes" || !seg.processedOutputs?.length)
            continue;
        if (!seg.processedOutputs.some((o) => o.laneId === VOCAL_LANE_ID && !o.loading))
            continue;
...
export async function renderVocalPart(track, seg, tempo, laneLabel) {
    if (seg.content.type !== "notes")
        return;
    const vp = track.vocalParams ?? DEFAULT_VOCAL_PARAMS;
    const entry = useVoiceModelStore.getState().models[vp.backend]?.find((m) => m.name === track.voiceModel);
    if (!entry)
        throw new Error(VOCAL_NO_VOICE);
    ...
    const { triples, f0Cents, f0Voiced, loudnessEnv, formantEnv } = buildVocalScore(seg.content.notes, seg.content.pitchDev, tempo, vp.transition, vocalTokens(vp), seg.content.paramCurves, vp.formant ?? 0, vp.langId);
    ...
    if (!triples.some((t) => t.lyric !== "R"))
        throw new Error(VOCAL_EMPTY);
...
export function vocalRenderOptions(vp) {
    return {
        backend: vp.backend,
...
export function isVocalDirty(track, seg, tempo) {
    if (seg.content.type !== "notes")
        return false;
    ...
    if (seg.content.notes.length === 0)
        return !!bake;
    if (!track.vocalParams)
        return false;
    if (!resolveTrackVoice(track))
        return false;
...
async function waitVocalIdle(timeoutMs = 120000) {
...
export async function renderDirtyVocals(list, tempo, laneLabel, opts) {
    await waitVocalIdle();
    let rendered = 0;
    let failed = 0;
    ...
    for (const { trackId, segmentId } of list) {
        ...
        if (!tr || !sg || !isVocalDirty(tr, sg, tempo))
            continue;
        ...
        try {
            ...
            if (String(e).includes(VOCAL_EMPTY))
                continue;
            failed++;
