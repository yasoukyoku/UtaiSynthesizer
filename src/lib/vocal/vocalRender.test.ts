...
describe("vocalTrackSig — the version terms are present and literal", () => {
  ...
  it("carries the g2p + timing tokens", () => {
    expect(vocalTrackSig(track, 120)).toContain("|g2p:s90|st:s92e");
    expect(G2P_ALGO_VERSION).toBe("s90"); // untouched — S92 changes frame allocation, never lyric→phone
    expect(SCORE_TIMING_VERSION).toBe("s92e"); // S92: the coda-cluster split in allocate_in_note
  });
  ...
  it("has exactly this shape — a change here invalidates every stored bake", () => {
    expect(vocalTrackSig(track, 120)).toBe(
      "vp:sovits,49,2,0,0,0,100,70,15,15,200|sv:|rv:|re:0|vm:V|bpm:120|rr:|g2p:s90|st:s92e",
    );
  });
});
