    ...
    it("dumps knob-model Option-A f0 for each E1 segment", async () => {
        ...
        for (const name of SEGMENTS) {
            ...
            if (fs.existsSync(dPath)) {
                ...
                expect(JSON.stringify(triples), `${name}: triples 漂移(θ 不该动谱面)`).toBe(JSON.stringify(d.triples));
            }
            fs.writeFileSync(join(WORK_DIR, `${name}_score_${KTAG}.json`), JSON.stringify({ name, tempo, ckpt: theta.ckpt, triples, f0Cents, f0Voiced }));
