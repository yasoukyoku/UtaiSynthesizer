...
export function scoreExportableTracks(tracks) {
    const out = [];
    for (const t of tracks) {
        if (t.trackType !== "vocal")
            continue;
        let n = 0;
        for (const s of t.segments)
            if (s.content.type === "notes")
                n += s.content.notes.length;
        if (n > 0)
            out.push({ trackId: t.id, name: t.name, noteCount: n });
    }
    return out;
}
...
function flattenPitchDev(track) {
    const xs = [];
    const ys = [];
    ...
    for (const seg of segs) {
        ...
        for (let i = 0; i < c.xs.length; i++) {
            ...
            if (xs.length > 0 && x <= (xs[xs.length - 1] ?? -Infinity))
                continue;
            xs.push(x);
            ys.push(c.ys[i] ?? 0);
        }
...
export async function runScoreExport(format, outPath, trackIds) {
