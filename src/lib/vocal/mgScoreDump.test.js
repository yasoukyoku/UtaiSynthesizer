...
describe.skipIf(!process.env.UTAI_MG_DUMP)("MG score dump (diagnostic, not a gate)", () => {
    it("dumps production triples + default-tuning f0 for the Mother Goose UST", async () => {
        ...
        const { triples, f0Cents, f0Voiced, loudnessEnv, formantEnv } = buildVocalScore(notes, undefined, tempo, DEFAULT_TRANSITION, { breath: "AP", rest: "R" }, // 生产默认的两个标记
        ...
        console.log(`[mgdump] ${meta.n_notes} notes -> ${triples.length} triples, ${sum} frames ` +
