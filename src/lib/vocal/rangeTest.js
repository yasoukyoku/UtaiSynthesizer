...
import { useVoiceModelStore, voiceSpeakerOptions, } from "../../store/voice-models";
import { SOVITS_DEFAULTS, RVC_DEFAULTS } from "../workflow/voiceDefaults";
import { VOCAL_RENDER_BUSY } from "./vocalRender";
import { boundsPayload } from "./rangeBounds";
const t = (k) => i18n.t(k);
...
export function buildScaleScore(lyric = "あ", frames = NOTE_FRAMES) {
    const triples = [{ lyric: "R", note_num: 0, frames: REST_FRAMES, lang: 2 }];
    const spans = [];
...
export function classifySemitones(f0, spans, 
...
quality) {
    return spans.map(({ midi, start100, end100 }, i) => {
        ...
        if (!voiced.length)
            return { midi, errCents: Infinity, voicedRatio, ...extra };
...
const isUsable = (s) => s.errCents < 100 && s.voicedRatio > 0.5;
...
const isComfort = (s) => s.errCents < 50 &&
    s.voicedRatio > 0.8 &&
...
function bridgedFlags(stats, flag) {
    ...
    while (i < f.length) {
        ...
        while (j < f.length && !f[j])
            j++;
        if (i > 0 && j < f.length && j - i <= BRIDGE_MAX_GAP)
            f.fill(true, i, j);
        i = j;
    }
    return f;
...
function longestRun(stats, flag) {
    ...
    for (let i = 0; i <= flags.length; i++) {
        ...
        if (ok && start < 0)
            start = i;
        if (!ok && start >= 0) {
            if (!best || i - 1 - start > best[1] - best[0])
                best = [start, i - 1];
            start = -1;
        }
    }
...
export function deriveRanges(stats) {
...
export function buildSpeakerRecord(stats) {
...
export function deriveCautionZones(semitones, usable, comfort) {
    const stats = Object.entries(semitones)
        .map(([k, v]) => ({
        midi: Number(k),
        errCents: v[0],
        voicedRatio: v[1],
    ...
    const singsWrong = (s) => s.voicedRatio > 0.5 &&
        s.errCents >= 200 &&
        s.errCents < 9999 &&
...
export async function runRangeTest(name, backend, modelPath, speakerId = 0) {
    const store = useVoiceModelStore.getState();
    if (store.rangeTesting[name] !== undefined)
        return;
    ...
    try {
        ...
        const dir = (await invoke("ensure_cache_dir", { segmentId: "range_test" })).replace(/\\/g, "/");
        ...
        const pass = async (lyric, frames, lo, hi, withQuality) => {
            ...
            const f0 = await invoke("detect_f0", { audioPath: wavPath });
            ...
            const quality = withQuality
                ? await invoke("analyze_scale_quality", {
                    audioPath: wavPath,
        ...
        try {
            ...
            void logToBackend("warn", `range test: onset probe pass failed, keeping the 「あ」-only landing verdict: ${String(e)}`);
        }
        ...
        const existing = entry?.config?.vocal_range;
        ...
        useAppStore.getState().showToast(`${t("rangeTest.done")} ${midiName(record.comfort[0])}–${midiName(record.comfort[1])}`, "success");
    }
    catch (e) {
        ...
        if (!isCancelError(e))
            logToBackend(isBusyError(e) ? "warn" : "error", `Range test failed (${name}): ${msg}`);
        if (isCancelError(e)) { /* user cancelled — not an error, no toast */ }
        else if (msg.includes(VOCAL_RENDER_BUSY))
            useAppStore.getState().showToast(t("rangeTest.busy"), "info");
        // S67c: fatal modal-class errors (INFERENCE_LOW_MEMORY) open the alert dialog instead of a toast.
            ...
            useAppStore.getState().showToast(shared, isBusyError(e) ? "info" : "error");
        else
            useAppStore.getState().showToast(`${t("rangeTest.failed")}: ${msg}`, "error");
    }
    finally {
        unlisten();
...
export function collectRangeTestTargets(models) {
    const out = [];
    for (const backend of ["sovits", "rvc"]) {
        for (const m of models[backend] ?? []) {
            ...
            for (const id of ids) {
                ...
                if (!sp)
                    out.push({ name: m.name, backend, path: m.path, speakerId: id, reason: "missing" });
...
export async function runRangeTestBatch(targets) {
    const store = useVoiceModelStore.getState();
    if (store.rangeBatch && !store.rangeBatch.finished)
        return; // single-flight (a finished-with-failures summary may be replaced)
    useVoiceModelStore
        .getState()
        .setRangeBatch({ done: 0, total: targets.length, failed: [], cancel: false, finished: false });
    for (const tgt of targets) {
        if (useVoiceModelStore.getState().rangeBatch?.cancel)
            break;
...
function readRangeRecord(m, speakerId) {
    return m?.config
...
export async function runCandidateRangeTest(workspace, backend, ckptPath, candidateId) {
    const { triples, spans } = buildScaleScore();
    // S66/O5: the render writes the probe wav Rust-side — compute the path up front.
    const dir = (await invoke("ensure_cache_dir", { segmentId: "range_test" })).replace(/\\/g, "/");
    const wavPath = `${dir}/cand_${Date.now().toString(36)}.wav`;
    await invoke("render_candidate_scale", {
        backend,
        ckptPath,
        workspace,
        candidateId,
    ...
    if (!record)
        return null; // nothing usable — an undertrained checkpoint; audition stays unshifted
...
export async function setVocalRangeBounds(name, backend, speakerId, usable, comfort) {
    const entry = useVoiceModelStore.getState().models[backend]?.find((m) => m.name === name);
