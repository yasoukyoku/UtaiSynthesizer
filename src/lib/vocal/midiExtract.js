...
import { useHistoryStore, registerUndoInterceptor, timelineUndoDepth, inGestureTransaction, } from "../../store/history";
...
import { quantizeSpans, QUANTIZE_IMPORT_KEY } from "./quantize";
...
export function midiExtractProgress(key) {
...
function ensureProgressListener() {
    ...
    void listen("midi-extract-progress", (e) => {
        ...
        if (meta)
            progressByKey.set(meta.key, (meta.index + e.payload.progress) / meta.total);
    });
}
// ── undo-cancels-extraction (one registration while ANY job is active) ──
let unregisterInterceptor = null;
function interceptorWouldConsume() {
    ...
    if (app.workflowSegmentId != null && app.activePane === "workflow")
        return false;
...
function cancelJobs(keys) {
    const jobs = useAppStore.getState().midiExtracting;
    for (const k of keys) {
        cancelledKeys.add(k);
        for (const jobId of jobs[k]?.jobIds ?? [])
            void invoke("cancel_midi_extract", { jobId });
    }
}
function interceptorConsume() {
...
export function cancelExtractionsForTeardown() {
    const app = useAppStore.getState();
...
export function midiExtractErrorMessage(e) {
    const msg = e instanceof Error ? e.message : String(e);
    if (msg.includes("MIDI_EXTRACT_NOT_INSTALLED"))
        return t("midiExtract.errNotInstalled");
    if (msg.includes("MIDI_EXTRACT_TOO_SHORT"))
        return t("midiExtract.errTooShort");
    if (msg.includes("MIDI_EXTRACT_LOAD_FAILED"))
        return t("midiExtract.errLoad");
    if (isCancelError(msg))
        return t("midiExtract.cancelled"); // e.g. GAME_DL_FAILED: DOWNLOAD_CANCELLED
    ...
    if (shared)
        return `${t("midiExtract.errFailed")}: ${shared}`;
...
function absNotes(raw, seg, out, tempo) {
    ...
    const audible = (srcMs) => pieces.some((p) => srcMs >= p.startMs && srcMs <= p.endMs);
    const items = [];
    for (const n of raw) {
        if (!audible((n.onset_ms + n.offset_ms) / 2))
            continue;
...
function buildNotes(spans, items, partStart, lyric) {
...
async function whenNoGestureTransaction() {
 ...
 */
export async function extractMidiForLaneGroup(trackId, segId, group) {
    const key = extractKey(segId, group);
    const app = useAppStore.getState();
    if (app.midiExtracting[key])
        return;
    const p = useProjectStore.getState();
    const track = p.tracks.find((tk) => tk.id === trackId);
    const seg = track?.segments.find((s) => s.id === segId);
    if (!track || !seg || seg.content.type !== "audioClip" || seg.loading)
        return;
    ...
    if (useAppStore.getState().midiExtracting[key])
        return; // double-trigger across the await
    ...
    if (useAppStore.getState().midiExtracting[key])
        return; // double-trigger across the dialog await
    const tempo = p.tempo;
    const c = seg.content;
    const r = segStretch(seg);
    ...
    const segStillValid = () => {
        ...
        if (!s2 || s2.content.type !== "audioClip")
            return false;
        const c2 = s2.content;
        return (`${getLoadEpoch()}:${c2.sourcePath}:${c2.offsetMs}:${s2.startTick}:${s2.durationTicks}:${segStretch(s2)}:${p2.tempo}` ===
            winSig);
    };
    ensureProgressListener();
    const jobs = outs.map((o, i) => ({ o, jobId: crypto.randomUUID(), index: i }));
    for (const j of jobs)
        jobMetaByJobId.set(j.jobId, { key, index: j.index, total: jobs.length });
    ...
    try {
        ...
        for (const { o, jobId, index } of jobs) {
            ...
            if (cancelledKeys.has(key) || !segStillValid())
                break;
            const notes = await invoke("extract_midi_from_audio", {
                path: o.audioPath,
        ...
        if (cancelledKeys.has(key))
            return; // user cancelled — even if the last stem raced to Ok
        ...
        try {
            ...
            for (const { o, notes: raw } of results) {
                ...
                if (!seg2 || seg2.content.type !== "audioClip")
                    break;
        ...
        if (droppedTotal > 0)
            useAppStore.getState().showToast(t("midiExtract.quantDropped", { count: droppedTotal }), "info");
        if (made > 0) {
            flushAutosaveNow();
