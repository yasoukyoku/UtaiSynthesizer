...
describe("后端那道闸(comfort ⊆ 扫描量出来的可用域)永远满足", () => {
    ...
    it("⭐ 收窄可用上界【不再】把目标范围一起拖下去", () => {
        const sp = rec({ usable_auto: [36, 80] });
...
describe("可用域夹在扫描量出来的范围之内", () => {
    ...
    it("已经被压低过的记录,还原回的是扫描值而不是当前值", () => {
        const sp = rec({ usable: [36, 60], comfort: [36, 58], usable_auto: [36, 80] });
...
describe("落盘载荷", () => {
    it("两个边界在同一份载荷里,而且顺带补上 usable_auto", () => {
        const p = boundsPayload(rec(), [36, 60], [36, 79]);
    ...
    it("二次编辑不会把 usable_auto 覆盖成被压低后的值", () => {
        ...
        const twice = boundsPayload(once, [36, 50], [36, 48]);
...
describe("targetRange —— 读侧愈合(镜像 Rust 的 speaker_range)", () => {
    ...
    it("站不住的存值退回 comfort_auto,再退回整条扫描带", () => {
        const mk = (comfort, auto) => rec({ usable: [42, 70], comfort, comfort_auto: auto });
    ...
    it("参照系是扫描带,不是 usable —— 换个说法钉同一条", () => {
        const legacy = rec({ usable: [42, 70], comfort: [45, 60], comfort_auto: [42, 70] });
...
describe("最小跨度", () => {
    it("退化的区间被撑开到最小跨度,且不越出参照系", () => {
        ...
        const sp = rec({ usable: [42, 70], usable_auto: [42, 70] });
    ...
    it("参照系本身就比最小跨度窄时照抄它,不造假", () => {
        const sp = rec({ usable: [60, 63], comfort: [61, 61], usable_auto: [60, 63] });
