 ...
 */
export function quantizeSpans(spans, policy = "widen", unit = GRID_QUANT_TICKS) {
    const q = (t) => Math.round(t / unit) * unit;
    const order = spans
        .map((_, i) => i)
...
export function offGridCount(spans, unit = GRID_QUANT_TICKS) {
    let n = 0;
    for (const s of spans)
        if (s.start % unit !== 0 || s.end % unit !== 0)
            n++;
    return n;
}
