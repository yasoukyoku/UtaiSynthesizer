...
function clampInto(band, within) {
    ...
    if (hi - lo < MIN_COMFORT_SPAN) {
        if (within[1] - within[0] < MIN_COMFORT_SPAN)
            return [within[0], within[1]]; // 参照系本身就窄:照抄,别造假
 ...
 *  旋钮比没有更糟。 */
export function clampBounds(sp, usable, comfort) {
...
export function targetRange(sp) {
    const band = scanBand(sp);
    if (fitsIn(sp.comfort, band))
        return sp.comfort;
    if (fitsIn(sp.comfort_auto, band))
        return sp.comfort_auto;
    return band;
}
/** 「还原」的目标 = 扫描量出来的那一对。 */
export function autoBounds(sp) {
    const band = scanBand(sp);
    return clampBounds(sp, band, sp.comfort_auto ?? band);
}
/** 这份记录的两个边界有没有被人动过(决定「还原」按钮亮不亮)。 */
export function boundsAreEdited(sp) {
    const a = autoBounds(sp);
    return ["usable", "comfort"].some((k) => sp[k][0] !== a[k][0] || sp[k][1] !== a[k][1]);
}
/** 构造一次 `set_model_vocal_range` 的完整 speaker 记录。
 *  ⚠ 顺带补写 `usable_auto`,否则用户压低可用上限之后**再也回不去**(只能重测)。 */
export function boundsPayload(sp, usable, comfort) {
...
export function speakerRecordOf(config, speakerId) {
    const rec = config?.vocal_range;
