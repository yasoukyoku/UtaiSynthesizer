...
describe.skipIf(!process.env.UTAI_E1_DUMP)("E1 param-f0 dump (diagnostic, not a gate)", () => {
    it("dumps production-default Option-A f0 + triples for each E1 segment", async () => {
        ...
        const tempo = manifest.tempo; // 98 — the svp's (and thus the wav slice's) tempo
        for (const name of SEGMENTS) {
            ...
            const notes = meta.notes.map((n, i) => ({
            ...
            const { triples, f0Cents, f0Voiced, loudnessEnv, formantEnv } = buildVocalScore(notes, undefined, tempo, DEFAULT_TRANSITION, { breath: "AP", rest: "R" });
            ...
            console.log(`[e1dump] ${name}: ${meta.n_notes} notes -> ${triples.length} triples, ${sum} frames ` +
                `(${(sum / 50).toFixed(2)}s @50fps), voiced ${voicedN}/${sum}`);
            fs.writeFileSync(join(WORK_DIR, `${name}_score.json`), JSON.stringify({ name, tempo, triples, f0Cents, f0Voiced }));
        }
    });
});
