...
function oovSig(track, seg, tempo) {
...
const validated = new Map(); // segmentId → last VALIDATED inputs
let timer = null;
let running = false;
let lastTracks = null;
let lastTempo = null;
async function validatePass() {
    ...
    if (st.tracks === lastTracks && st.tempo === lastTempo)
        return; // playhead-only updates
    running = true;
    try {
        ...
        for (const tr of st.tracks) {
            for (const seg of tr.segments) {
                ...
                if (prev && prev.notes === seg.content.notes && prev.vp === tr.vocalParams && prev.tempo === st.tempo)
                    continue;
                const sig = oovSig(tr, seg, st.tempo);
                const stamp = { notes: seg.content.notes, vp: tr.vocalParams, tempo: st.tempo, sig };
                if (prev?.sig === sig) {
                ...
                try {
                    const classes = await invoke("validate_lyrics", {
                    ...
                    if (!trNow || !segNow || oovSig(trNow, segNow, now.tempo) !== sig)
                        continue;
        ...
        if (useProjectStore.getState().tracks !== st.tracks)
            schedule();
    }
    finally {
        running = false;
...
export function installOovWatch() {
