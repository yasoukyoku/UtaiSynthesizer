...
export function autoTuneScalesOf(p) {
...
export function phaseForTake(take, noteId) {
    if (take === 0)
        return 0;
    const s = `${take}:${noteId}`;
    let h = 0x811c9dc5;
...
function thetaToFields(th, scales, phase) {
 ...
 */
export async function applyAutoTune(trackId, segmentId, scales, opts = {}) {
    ...
    for (const n of notes) {
        if (isSilentLyric(n.lyric, tokens))
            skipped++;
        else
            sung.push(n);
    }
    ...
    const targetIdx = []; // 下标是 `sung` 内的位置,θ 与 `sung` 同序同长
    sung.forEach((n, i) => {
        if (!isUserTuned(n))
            targetIdx.push(i);
        else
    ...
    if (theta.length !== sung.length)
        throw new Error("AUTOTUNE_SHAPE: theta/notes length");
    ...
    if (opts.silent)
        useHistoryStore.getState().runSilent(write);
    else
        write();
