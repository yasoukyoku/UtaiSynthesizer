...
import { evalF0CentsFrames, effTransition } from "../f0eval";
...
const ri = (rng, lo, hi) => Math.floor(rf(rng, lo, hi + 1));
let nextId = 0;
const mkNote = (tick, duration, pitch, extra) => ({
...
const lastEnd = (notes) => notes.reduce((m, n) => Math.max(m, n.tick + n.duration), 0);
const prodFrame = (notes, tempo) => {
...
function directedCases() {
    ...
    const vib = (d, f, p, s, ei, eo) => ({ depthCents: d, freqHz: f, phase: p, startMs: s, easeInMs: ei, easeOutMs: eo });
    cases.push({
...
function randomCases(count) {
    ...
    for (let c = 0; c < count; c++) {
        ...
        const defaultTransition = rng() < 0.7 ? { ...DEFAULT_TRANSITION } : {
        ...
        for (let i = 0; i < n; i++) {
            ...
            const extra = {};
            if (rng() < 0.3)
                extra.detune = rf(rng, -100, 100);
            if (rng() < 0.5) {
                const t = {};
                if (rng() < 0.4)
                    t.offsetMs = rf(rng, -2000, 2000);
                if (rng() < 0.4)
                    t.durLeftMs = rf(rng, 0, 2000);
                if (rng() < 0.4)
                    t.durRightMs = rf(rng, 0, 2000);
                if (rng() < 0.4)
                    t.depthLeftCents = rf(rng, -1200, 1200);
                if (rng() < 0.4)
                    t.depthRightCents = rf(rng, -1200, 1200);
                if (rng() < 0.4)
                    t.openEdgeCents = rf(rng, 0, 1200);
...
describe.skipIf(!process.env.UTAI_G0_DUMP)("G0 twin-parity dump (diagnostic, not a gate)", () => {
    it("dumps directed + seeded-random f0eval fixtures for the SVC2SVS renderer twin", async () => {
        ...
        for (const c of cases) {
            const opts = { tempo: c.tempo, defaultTransition: c.defaultTransition };
            ...
            const voicedN = voiced.reduce((s, v) => s + v, 0);
            if (c.notes.length > 0)
                expect(voicedN, `${c.name}: no voiced frames`).toBeGreaterThan(0);
            else
                expect(voicedN, `${c.name}: empty score must be all-rest`).toBe(0);
            expect(voicedN, `${c.name}: no rest frames`).toBeLessThan(frame.frameCount);
            for (let f = 0; f < frame.frameCount; f++) {
                if (!voiced[f])
                    expect(cents[f], `${c.name}: rest frame ${f} cents != 0`).toBe(0);
            }
            dumped.push({
