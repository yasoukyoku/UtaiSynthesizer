...
const mkNote = (id, tick, duration, pitch, lyric = "あ") => ({
...
describe("isVocalDirty — dual-sig split-window acceptance", () => {
    const vp = { backend: "sovits", speakerId: 49, langId: 2, transpose: 0, formant: 0, transition: DEFAULT_TRANSITION, breathToken: "AP" };
    const mkSeg = (out) => ({
    ...
    const mkTrack = (seg) => ({ id: "t", name: "V", trackType: "vocal", segments: [seg], volumeDb: 0, pan: 0, muted: false, solo: false, expanded: true, laneControls: {}, voiceModel: "V", vocalParams: vp });
    ...
    it("split of a DIRTY bake does NOT launder it clean — both halves stay dirty (→ re-render, not stale audio)", () => {
        ...
        useProjectStore.setState({ tracks: [track], tempo: 120 });
...
describe("buildVocalScore", () => {
    ...
    it("breath note → AP phone + UNVOICED f0 (breaks the pitch chain, §M3)", () => {
        ...
        for (let f = ap.s; f < ap.e; f++)
            expect(f0Voiced[f]).toBe(0); // breath frames unvoiced
...
describe("buildScoreTriples — zero-frame note drop is loud", () => {
    it("a note that CANNOT borrow is still dropped loudly, and the frame timeline stays conserved", () => {
        ...
        for (const id of droppedNoteIds)
            expect(tripleNoteIds).not.toContain(id);
...
describe("buildScoreTriples — S87 frame borrowing", () => {
    const sumFrames = (t) => t.reduce((s, x) => s + x.frames, 0);
    ...
    describe("★ a rescued note actually OWNS its borrowed frame in the f0 feed (swept over tempo)", () => {
        ...
        const frameIndexOf = (triples, upTo) => triples.slice(0, upTo).reduce((s, t) => s + t.frames, 0);
        ...
        const tpf = (tempo) => (1000 / 50 / 60000) * tempo * 480;
        const build = (kind, tempo) => {
    ...
    it("★ SWEEP: over 600 pseudo-random scores, EVERY borrowed frame is voiced and at its own pitch", () => {
        ...
        for (let i = 0; i < 600; i++) {
            ...
            if (!r.borrowedNoteIds.includes("b"))
                continue;
            borrows++;
            const idx = r.tripleNoteIds.indexOf("b");
            ...
            if (sv.f0Voiced[f] !== 1)
                unvoiced++;
            else if (Math.abs(sv.f0Cents[f] / 100 - 72) > 0.01)
                wrongPitch++;
        }
    ...
    describe("the rest token — a written rest IS a gap", () => {
        ...
        it("a CUSTOM trigger is that same silence; pointing it elsewhere makes the word sing again", () => {
            ...
            for (let f = s.s; f < s.e; f++)
                expect(asRest.f0Voiced[f]).toBe(0);
...
describe("vocalTrackSig — the version terms are present and literal", () => {
    const vp = { backend: "sovits", speakerId: 49, langId: 2, transpose: 0, formant: 0, transition: DEFAULT_TRANSITION, breathToken: "AP" };
    const track = { id: "t", name: "V", trackType: "vocal", segments: [], volumeDb: 0, pan: 0, muted: false, solo: false, expanded: true, laneControls: {}, voiceModel: "V", vocalParams: vp };
    ...
    it("carries the g2p + timing tokens", () => {
        ...
        // back to the cut it had BEFORE s110 — so the cumulative count FELL, 611 → 567.
    ...
    it("has exactly this shape — a change here invalidates every stored bake", () => {
        expect(vocalTrackSig(track, 120)).toBe("vp:sovits,49,2,0,0,0,100,70,15,15,200|sv:|rv:|re:1|vm:V|bpm:120|rr:|g2p:s113b|st:s97c|dict:d0");
    });
...
describe("isVocalDirty — the dictionary-fingerprint shield (§E2E-D4)", () => {
    ...
    const track = { id: "t", name: "V", trackType: "vocal", segments: [seg], volumeDb: 0, pan: 0, muted: false, solo: false, expanded: true, laneControls: {}, voiceModel: "V", vocalParams: vp };
    it("★ 指纹为 null 时短路成【不脏】;拿到指纹之后才照常判", async () => {
        ...
        expect(fresh.isVocalDirty(track, seg, 120), "指纹还没到就把存量 bake 判脏 = 每次开机整个工程重渲染;S101 定的安全方向是先答「不脏」").toBe(false);
        fresh.setDictionarySig("d0");
        expect(fresh.isVocalDirty(track, seg, 120), "指纹到了之后这一段仍然判「不脏」—— 那面盾没有解除,存量 bake 从此永远不会因为换词典而重渲染").toBe(true);
    });
});
...
describe("vocalRenderOptions — every per-track knob must actually reach the wire", () => {
    const vp = { backend: "sovits", speakerId: 49, langId: 2, transpose: 0, formant: 0, transition: DEFAULT_TRANSITION };
    const base = () => ({ ...vp });
    ...
    it("★ S159 音域扩展的默认是 ON,而且三态各自到位", () => {
        ...
        expect(vocalRenderOptions({ ...base(), rangeExtend: false }).range_extend).not.toBe(vocalRenderOptions(base()).range_extend);
    });
});
