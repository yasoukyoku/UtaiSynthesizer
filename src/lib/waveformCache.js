...
export function peaksSignature(peaks) {
    const n = peaks.length;
    if (n === 0)
        return "0";
    const q = n >> 2;
    const f = (i) => (peaks[i] ?? 0).toFixed(3);
    return `${n}.${f(0)}.${f(q)}.${f(q * 2)}.${f(q * 3)}.${f(n - 1)}`;
}
function build(peaks, color) {
 ...
 */
export function getWaveformCache(id, peaks, color) {
    if (peaks.length === 0)
        return null;
 ...
 */
export function blitWaveform(ctx, wave, x, y, w, h, startRatio, endRatio, clipWidth) {
    if (w <= 0 || h <= 0 || endRatio <= startRatio)
        return;
 ...
 */
export function drawWaveform(ctx, id, peaks, color, x, y, w, h, startRatio, endRatio, clipWidth) {
    if (w <= 0 || h <= 0 || endRatio <= startRatio || peaks.length === 0)
        return;
    const destL = Math.max(x, 0);
    const destR = Math.min(x + w, clipWidth);
    if (destR <= destL)
    ...
    if (deviceW > colsInWindow) {
        ...
        if (wave)
            blitWaveform(ctx, wave, x, y, w, h, startRatio, endRatio, clipWidth);
    }
}
...
function drawWaveformDirect(ctx, peaks, color, x, y, w, h, startRatio, endRatio, destL, destR) {
    const midY = y + h / 2;
    const amp = h / 2 - 1;
    const span = endRatio - startRatio;
    const n = peaks.length;
