...
import { resolveArchiveIndex, indexPathArg, indexWarningCode, } from "./indexPath";
...
describe("存档导入的索引解析", () => {
    ...
    it("sovits 家族按 kmeans 优先、再退到检索向量", async () => {
        ...
        expect(await resolveArchiveIndex({ backend: "sovits", workspace: S, exists: both.exists })).toEqual({ kind: "probed", path: `${S}\\cluster\\kmeans_10000.pt` });
        const onlyVectors = probe([`${S}\\cluster\\0.index_vectors.npy`]);
        expect(await resolveArchiveIndex({ backend: "sovits", workspace: S, exists: onlyVectors.exists })).toEqual({ kind: "probed", path: `${S}\\cluster\\0.index_vectors.npy` });
    });
    ...
    it("给 Rust 的实参只在真的知道路径时才有值", async () => {
        ...
        for (const [r, want] of cases)
            expect(indexPathArg(r)).toBe(want);
    });
});
