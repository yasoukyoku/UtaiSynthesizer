 ...
 */
import { resumeLockedFields, scopeInvalidatesPool } from "../resumeLock";
/**
 ...
 */
export function poolCostIdsIn(rows) {
    return new Set(rows.filter((f) => f.tier === "costly" && scopeInvalidatesPool(f.scope)).map((f) => f.id));
}
/**
 ...
 */
export const EDITABLE_POOL_FIELDS = {
    rvc: ["sampleRate"],
    sovits: [],
    sovits_v2: [],
    sovits_diff: [],
 ...
 */
export function poolCostFieldIds(backend, poolAtStake) {
    if (!poolAtStake)
        return new Set();
    const out = poolCostIdsIn(resumeLockedFields(backend));
    for (const id of EDITABLE_POOL_FIELDS[backend])
        out.add(id);
    return out;
}
/**
 ...
 */
export function poolAtStake(info) {
