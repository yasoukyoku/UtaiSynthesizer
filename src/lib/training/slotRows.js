 ...
 */
export function visibleRuns(runs, liveRunId = null) {
 ...
 */
export function slotStarted(runs, liveRunId = null) {
    return visibleRuns(runs, liveRunId).length > 0;
}
export function pickDiffHost(slot) {
 ...
 * 「没池却画出这一行」会各自成为可能。
 */
export function prepPoolLine(slot) {
 ...
 */
export function sortRunRows(rows, liveRunId) {
    const named = (r) => (r.modelName ?? "").trim();
 ...
 */
export function foldRunRows(rows, expanded, limit = RUN_ROWS_BEFORE_FOLD) {
 ...
 */
export function newRunNameProblem(raw, runs, 
