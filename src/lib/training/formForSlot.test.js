...
describe("formForSlot —— locked 档(不还原就会被 Rust 拒的值)", () => {
    ...
    it("sovits_diff:有扩散进度才还原 kStepMax(与后端 guard 同一个信号)", () => {
        ...
        expect(formForSlot("sovits_diff", ws({ diff_steps: 0, diff_k_step_max: 200 }))).not.toHaveProperty("diffKStepMax");
    });
...
describe("★棘轮:锁表 ↔ 表单字段", () => {
    it("每个 backend 的每一条 costly 规则,要么有表单字段,要么明写它为什么没有", () => {
        for (const backend of ALL_BACKENDS) {
            for (const id of lockedFieldIds(backend, "costly")) {
                ...
                expect(declared, `${backend}.${id} 没有落点:给它一个表单字段,或写进 NO_FORM_CONTROL`).toBe(true);
            }
        }
    });
    it("反向:声明了表单字段的每一条,必须真的出现在 formForSlot 的返回值里", () => {
        ...
        for (const backend of ALL_BACKENDS) {
            for (const [id, field] of Object.entries(POOL_FORM_FIELDS[backend])) {
                ...
                const got = formForSlot(backend, ws(probe.info));
                expect(got[field], `${backend}.${id} 声明了 ${field} 却没还原它`).toBe(probe.expect);
            }
        }
    });
    it("NO_FORM_CONTROL 里的每一条都必须真的在某个 backend 的锁表里(别攒死条目)", () => {
        ...
        for (const backend of ALL_BACKENDS)
            for (const id of lockedFieldIds(backend, "costly"))
                all.add(id);
        for (const id of NO_FORM_CONTROL)
            expect(all.has(id), `${id} 已经不在锁表里了`).toBe(true);
    });
});
