...
import { foldRunRows, newRunNameProblem, pickDiffHost, prepPoolLine, slotStarted, sortRunRows, startedRun, visibleRuns, } from "./slotRows";
/**
...
const legacyStartedRun = (r) => r.hasResumePoint || r.info.has_main_progress;
const legacyVisibleRuns = (runs) => runs.filter((r) => r.id !== "" || legacyStartedRun(r));
const legacyStarted = (runs) => legacyVisibleRuns(runs).length > 0;
const legacyDiffHost = (sovitsSlot) => sovitsSlot?.runs.find((r) => r.info.has_main_progress) ?? sovitsSlot?.runs[0];
const legacyPinned = (diffHost) => 
...
diffHost?.info.version === "4.1" || diffHost?.info.version === "4.0"
    ...
    : undefined;
const legacyDiffSteps = (diffHost) => diffHost?.info.diff_steps ?? 0;
const legacyPrepShow = (slot) => (slot?.prepPoolCount ?? 0) > 0;
const legacyPrepCount = (slot) => slot?.prepPoolCount ?? 0;
const legacyPrepBytes = (slot) => slot?.prepPoolBytes ?? 0;
...
describe("foldRunRows(S141:run 多了才收)", () => {
    const runs = (n) => Array.from({ length: n }, (_, i) => mkRun(`r${i}`));
...
describe("sortRunRows(§E2E-M25 ⑷:正在跑的置顶,其余按名字)", () => {
    ...
    it("★★★ 把排过序的行喂给 `pickDiffHost` **会**换掉浅扩散的宿主 —— 这条钉的是那个危险是真的", () => {
        ...
        expect(sorted.host?.id, "排序后喂进去竟然给同一个宿主 —— 那条源码闸就成了装饰件,这份夹具失去了分辨力").not.toBe(raw.host?.id);
...
describe("newRunNameProblem(§E2E-M24)", () => {
    const named = (id, modelName) => mkRun(id, { modelName });
...
describe("★ 搬家前后逐个输入对拍(纯提取的证据形状)", () => {
    it("2000 个随机槽上,新模块与搬家前的表达式给出同一个答案", () => {
        ...
        const IDS = ["", "raaa", "rbbb", "rccc"];
        const VERSIONS = ["", "4.0", "4.1", "v1", "v2", "40k"];
        ...
        for (let i = 0; i < 2000; i++) {
            ...
            if (mains.length > 0 && runs[0] !== mains[0])
                sawFindDiffersFromFirst++;
            if (n === 0)
                sawEmpty++;
            if (runs.some((r) => r.id === ""))
                sawFakeRow++;
            ...
            for (const r of runs)
                expect(startedRun(r)).toBe(legacyStartedRun(r));
