 ...
 */
export const NO_FORM_CONTROL = new Set(["dataset"]);
/**
 ...
 */
export function formForSlot(backend, info, pin) {
    ...
    if (known) {
        ...
        if (fields.loudnorm && known.loudnorm !== null) {
            pool[fields.loudnorm] = known.loudnorm;
        }
        ...
        if (fields.augCopies && augIsThisRunsChoice) {
            pool[fields.augCopies] = known.aug_copies;
        }
    }
    // ---- locked:不还原就会被拒的那些值 -----------------------------------------------------
    if (backend === "rvc") {
        const v = known?.version === "v1" || known?.version === "v2" ? known.version : undefined;
        const sr = known?.sample_rate === "32k" || known?.sample_rate === "40k" || known?.sample_rate === "48k"
            ? known.sample_rate
            : undefined;
