...
import { lockedFieldIds, poolInvalidatingIds, resumeLockedFields, resumeWouldBeGuarded, scopeInvalidatesPool } from "../resumeLock";
...
const ALL_BACKENDS = ["rvc", "sovits", "sovits_v2", "sovits_diff", "vocoder"];
const ids = (s) => [...s].sort();
// ─── ⑴ 搬家之前的原文(`TrainingPage.tsx` 的 :876-884,S142 笔 1 之前)──────────────────
//     ⛔ 原样抄下来,一个字都不许「顺手改好」—— 它是对照臂,不是产品代码。
function legacyPoolAtStake(info, retrainIntent) {
    return !retrainIntent && !!info?.exists && (info.version !== "" || info.sample_rate !== "");
}
function legacyCostly(backend, info, retrainIntent) {
...
function corpus(n, seed = 20260813) {
    const r = rng(seed);
 ...
 */
function assertCorpusHasPower(samples) {
    const has = (f) => samples.some(f);
    const diverges = (s) => legacyPoolAtStake(s.info, s.retrainIntent) !== poolAtStake(s.info);
    const backends = new Set(samples.map((s) => s.backend));
    if (backends.size !== ALL_BACKENDS.length)
        throw new Error(`语料只覆盖了 ${backends.size} 个 backend`);
    if (new Set(samples.map((s) => s.retrainIntent)).size !== 2)
        throw new Error("retrainIntent 只有一种取值");
    if (!has((s) => s.info === null))
        throw new Error("语料里没有探针失败(info=null)那一格");
    // ★ 新旧两个谓词的**输入轴**都必须活着
    if (!has((s) => s.info?.has_preprocessing === true))
        throw new Error("语料里没有「有池」那一格");
    if (!has((s) => s.info?.has_preprocessing === false))
        throw new Error("语料里没有「没池」那一格");
    ...
    if (!has((s) => s.info?.exists === true &&
        s.info.version === "" &&
        s.info.sample_rate === "" &&
        s.info.diff_steps > 0)) {
        throw new Error("语料里没有 diff-first 那一格 —— 合并守卫是空的");
    }
    const sigs = new Set(samples.map((s) => `${s.backend}:${ids(poolCostFieldIds(s.backend, poolAtStake(s.info))).join(",")}`));
    if (sigs.size < 6)
        throw new Error(`语料只产出 ${sigs.size} 种结果签名`);
}
// ─── ⑶ 判据 ──────────────────────────────────────────────────────────────────────────
...
describe("poolCostFieldIds —— 槽里没有池就一个都不挂", () => {
    ...
    it("★ 没池 ⇒ 每个 backend 都是空集", () => {
        for (const b of ALL_BACKENDS)
            expect(poolCostFieldIds(b, false).size, b).toBe(0);
    });
    // ── ★★S144 §E2E-M10-⒜′ ────────────────────────────────────────────────────────────
    ...
    it("★ EDITABLE_POOL_FIELDS 的内容是**手写**的,改它必须是一次有意的改动", () => {
        ...
        for (const [backend, fields] of Object.entries(EDITABLE_POOL_FIELDS)) {
            const rows = resumeLockedFields(backend);
...
describe("★ 笔 3 到底改了什么:新旧逐个输入对拍", () => {
    ...
    it("★ 语料塌成一格时,自检必须抛(否则它什么也没检)", () => {
        const flat = Array.from({ length: 2000 }, () => ({
            backend: "rvc",
            info: ws(),
            retrainIntent: false,
        }));
    ...
    it("★ 两个谓词一致时集合逐个相同;不一致时【只】由 poolAtStake 说了算", () => {
        ...
        for (const s of samples) {
            ...
            const editable = (b) => [...EDITABLE_POOL_FIELDS[b]];
            const withEditable = (base) => [...new Set([...base, ...editable(s.backend)])].sort();
            if (legacyPoolAtStake(s.info, s.retrainIntent) === poolAtStake(s.info)) {
                ...
                expect(ids(now), why).toEqual(poolAtStake(s.info) ? withEditable(ids(poolCostIdsIn(resumeLockedFields(s.backend)))) : []);
