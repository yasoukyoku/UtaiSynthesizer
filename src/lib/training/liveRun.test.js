...
import { diffusionIsLive, isRunningState, liveRunIdFor, runRowActions, trainingIsLive, } from "./liveRun";
...
describe("runRowActions(四颗按钮跟的不是同一个谓词)", () => {
    ...
    it("★★ 试听/渲染/分离在飞而训练没在跑 —— 删除与改名仍然要禁", () => {
        ...
        expect(g.retrain.disabled, "试听在飞时再训一个被挡住了 —— 这一档禁得比后端还严").toBe(false);
    });
    ...
    it("★★ 语料自检:上面这些用例真的走到过四种不同的 reason", () => {
        ...
        for (const over of [
            ...
            for (const k of [g.cont, g.retrain, g.del, g.rename])
                seen.add(k.reason);
        }
        expect(seen, "四种 reason 没有全部被走到 —— 上面的用例覆盖不到某一档").toEqual(new Set(["", "flagged", "training", "tasks", "inflight"]));
    });
});
