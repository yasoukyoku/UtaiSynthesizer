...
function rustFields(src) {
    return [...declBody(src, "pub struct TrainingSnapshot {").matchAll(/^\s*pub ([a-z0-9_]+):/gm)].map((m) => m[1]);
}
...
describe("TrainingSnapshot Rust ↔ TS 对拍(M25 载体的上游闸)", () => {
    it("★存活证明:两侧解析器都真的读到了东西(否则这道闸是两个空集相等)", async () => {
        ...
        for (const [side, got] of [
            ...
            expect(got.length, `${side} 只解析出 ${got.length} 个字段,解析器多半读空了`).toBeGreaterThan(15);
            expect(new Set(got).size, `${side} 有重复字段名`).toBe(got.length);
        }
    });
    it("字段名集合完全一致 —— 少抄一个,四个消费者会一起静默答「谁都没在跑」", async () => {
        const fs = await importFs();
        expect(new Set(tsFields(fs.readFileSync(TS, "utf8")))).toEqual(new Set(rustFields(fs.readFileSync(RS, "utf8"))));
    });
    ...
    it("★`run_id` 必须是**必填**字段 —— 声明成可选就把 tsc 那道免费的闸关掉了", async () => {
        ...
        expect(new Set(keys), "IDLE_SNAPSHOT 与接口的必填字段对不上 —— 少一个键,「什么都没在跑」那一档读到的是 undefined").toEqual(new Set(required));
    });
});
