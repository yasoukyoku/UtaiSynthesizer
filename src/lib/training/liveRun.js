 ...
 * 是**开着**的,而后端已经在拒了。
 */
export function trainingIsLive(snap, pendingStart) {
 ...
 */
export function liveRunIdFor(snap, projectId, family) {
    if (!isRunningState(snap.state))
        return null;
    if (snap.project_id !== projectId)
        return null;
    if (backendFamily(snap.backend) !== family)
        return null;
    return snap.run_id;
}
/**
 ...
 */
export function diffusionIsLive(snap, projectId) {
    return snap.backend === "sovits_diff" && liveRunIdFor(snap, projectId, "sovits") !== null;
}
const gate = (reason) => ({ disabled: reason !== "", reason });
/**
 ...
 * 「三语齐全」的姿态活下去。
 */
export function gateReasonKey(reason) {
    switch (reason) {
        case "":
            return null;
        case "flagged":
...
export const GATE_REASON_KEYS = ["flagged", "training", "tasks", "inflight"].map((r) => gateReasonKey(r));
/**
 ...
 * 等训练结束的假话。
 */
export function runRowActions(input) {
    const { blocked, inFlight, trainingLive, anyTaskLive, hasRunId } = input;
    const start = blocked ? "flagged" : trainingLive ? "training" : "";
    const mutate = blocked
        ? "flagged"
        : anyTaskLive
            ? "tasks"
            : inFlight
