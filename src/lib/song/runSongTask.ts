...
import { ACE_TASK_OF, SONG_TASKS, type SongTaskId } from "../models/song-tasks";
import { getSongInstallStatus, songModelById } from "../models/song-catalog";

// ── 入参 / 出参 ────────────────────────────────────────────────────────────

export interface SongTaskPayload {
  /** 歌名（结果标签与落盘文件名）；缺省按任务+时间戳生成 */
  songName?: string;
  lyrics?: string;
  prompt?: string;
  /** 源音频绝对路径（cover/repaint/complete/extract/lego/stems 必填） */
  srcAudioPath?: string;
  /** 源音频时长（秒），用于结果展示与重绘区间校验 */
  srcDurationSec?: number;
  /** lego / extract（单选）轨种 */
  trackName?: string;
  /** complete / extract（批量）轨种列表 */
  trackClasses?: string[];
  /** repaint 起止秒 */
  repaintStart?: number;
  repaintEnd?: number;
  ...
  wantMidi?: boolean;
  wantLrc?: boolean;
  /** 目标时长（秒）；0/缺省 = 自动（YuE2 由 max_tokens 决定，ACE direct 任务取源时长） */
  durationSec?: number;
  seed?: number;
  /** 覆盖任务默认模型（如 generate 可在 yue2/acestep 间切换） */
  model?: string;
  /** sheet 续写：把已有 ABC 作为条件传给 YuE2 */
  abc?: string;
  /** 模型专属高级参数直通，最后合并到对应模型请求 */
  extra?: Partial<SongGenRequest>;
}

export interface SongTaskProgress {
  stage: string;
  message: string;
  /** 0~100 */
  percent: number;
...
export interface SongTaskResult {
  task: SongTaskId;
  modelId: string;
  /** 规整后的产物；extract 多轨合并为单条 stems 记录 */
  outputs: SongOutput[];
  /** 主音频时长（秒）；源任务取源时长，其余为 0（由 UI 按需探测） */
  durationSec: number;
  label: string;
}

export interface SongTaskRunOptions {
  onProgress?: (p: SongTaskProgress) => void;
}
...
function defaultLabel(task: SongTaskId): string {
  return `${task}_${Date.now().toString(36)}`;
}

async function ensureModelReady(modelId: string): Promise<void> {
  const catalog = songModelById(modelId);
  if (!catalog) return; // 未知模型（如未来扩展）交给后端校验
...
async function bindProgress(
  cb: (p: SongTaskProgress) => void,
  /** extract 批量时的整体包装：innerPct(0~100) → overallPct */
  wrap?: (innerPct: number) => number,
): Promise<() => void> {
  const unlisten = await listen<{
    ...
    total: number;
    stem_label?: string;
  }>("song-generation-progress", (e) => {
    const { stage, current, total, stem_label } = e.payload;
    let pct = STAGE_WEIGHTS[stage] ?? 0;
    if (stage === "generate" && total > 0 && current <= total) {
      pct = Math.round((current / total) * 100);
    }
    if (wrap) pct = wrap(pct);
    cb({ stage, message: stem_label || "", percent: Math.max(0, Math.min(100, pct)) });
  });
  return unlisten;
}

function reqBase(p: SongTaskPayload, modelId: string, songName: string): SongGenRequest {
  const req: SongGenRequest = {
    model: modelId,
    song_name: songName,
    lyrics: p.lyrics ?? "",
    prompt: p.prompt ?? "",
    audio_duration: p.durationSec ?? p.srcDurationSec ?? 0,
    format: "wav",
  };
  if (typeof p.seed === "number" && p.seed >= 0) req.seed = p.seed;
  if (p.wantStems) req.want_stems = true;
  if (p.wantMidi) req.want_midi = true;
  if (p.wantLrc) req.want_lrc = true;
  return req;
}

function firstAudio(outputs: SongOutput[]): string | undefined {
  return outputs.find((o) => o.audio_path)?.audio_path;
}

// ── 主入口 ─────────────────────────────────────────────────────────────────

export async function runSongTask(
  task: SongTaskId,
  payload: SongTaskPayload,
  opts: SongTaskRunOptions = {},
  ...
  if (task !== "stems") await ensureModelReady(modelId);

  const report = opts.onProgress ?? (() => {});

  // ── 快速四分轨（demucs，stems_only 轻量任务）──
  if (task === "stems") {
    if (!payload.srcAudioPath) throw new Error("请先选择源音频");
    ...
    try {
      ...
      if (!stems) throw new Error("分轨完成但没有返回任何分轨文件");
      return {
  ...
  if (task === "extract") {
    ...
    if (classes.length === 0) throw new Error("请至少选择一个要分离的轨种");
    let done = 0;
    const unlisten = await bindProgress(report, (inner) =>
      Math.round(((done + inner / 100) / classes.length) * 100),
    );
    ...
    try {
      for (const cls of classes) {
        ...
        if (!audio) throw new Error(`分离 ${cls} 失败：未返回音频文件`);
        stems[cls] = audio;
        done += 1;
      }
      return {
  ...
  const req: SongGenRequest = reqBase(payload, modelId, label);
  ...
  if (isAceTask) {
    ...
    if (task === "lego" && payload.trackName) req.track_name = payload.trackName;
  ...
  if (payload.extra) Object.assign(req, payload.extra);

  const unlisten = await bindProgress(report);
  try {
    const outputs = await songGenerate(req);
    if (!firstAudio(outputs)) throw new Error("生成完成但没有返回音频文件");
    return {
