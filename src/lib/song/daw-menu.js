...
if (typeof window !== "undefined") {
    ...
    window.addEventListener("mousedown", (e) => { if (e.button === 2)
        ctrlAtMenuOpen = e.ctrlKey; }, true);
    window.addEventListener("contextmenu", (e) => { ctrlAtMenuOpen = e.ctrlKey; }, true);
}
function openSongStudioWith(p) {
    useAppStore.getState().setPendingSongTask(p);
    const app = useAppStore.getState();
    if (!app.songStudioOpen)
        app.toggleSongStudio();
}
/** 构造"歌曲模型 ▸"二级子菜单项集合（与歌曲弹窗 8 模式对齐，源音频任务仅在有音频时可用） */
export function songTaskSubmenuItems(opts) {
    const t = (k) => i18n.t(k);
    const hasAudio = !!opts.audioPath;
    const base = {
        source: opts.source,
        sourceLabel: opts.sourceLabel,
        returnTarget: opts.trackId
            ? { kind: "track", trackId: opts.trackId, segmentId: opts.segmentId, align: true }
            : undefined,
    };
    const item = (task, key, icon, needAudio = true, extra) => ({
        label: t(key),
        icon,
                ...
                if (ctrlAtMenuOpen && hasAudio && opts.audioPath) {
                    ...
                    enqueueSongTask({
                        ...
                        if (added === 0)
                            ...
                            app.showToast(i18n.t("songMenu.quickStemsDone", { count: added }), "success");
                    }).catch((err) => {
                        // 失败必须让用户看见：模型未下载、格式不支持、被取消都会走到这里。
                        // 只 console.error 的话用户只会觉得「点了没反应」。
                        app.showToast(i18n.t("songMenu.quickStemsFailed", { error: backendErrorMessage(err) ?? String(err) }), "error");
                    });
                }
                else {
                    // 普通路径：打开歌曲弹窗让用户选参数
