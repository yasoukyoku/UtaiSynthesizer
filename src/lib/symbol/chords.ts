...
export interface ChordBlockChord {
  label: string;
  root: number;
  quality?: string;
}

/** chordBlockIn / reharmonize 输出的 JSON 形状（engine 里 autoArrange/melodyGen 消费）。 */
export interface ChordBlock {
  type: "chordBlock";
  chords: ChordBlockChord[];
  bpm?: number;
  ppq?: number;
}
...
const QUALITY_SUFFIXES: ReadonlyArray<{ suffix: string; quality: ChordQuality }> = [
...
export function parseChordLabel(label: string): ParsedChord {
  const fail: ParsedChord = { root: 0, quality: "maj", bass: null };
  ...
  if (m[2] === "#") root = mod12p(root + 1);
  if (m[2] === "b") root = mod12p(root - 1);
  ...
  if (bm) {
    ...
    if (bm[2] === "#") bp = mod12p(bp + 1);
    if (bm[2] === "b") bp = mod12p(bp - 1);
    bass = bp;
  }
...
export function chordLabel(root: number, quality: ChordQuality, bass: number | null = null): string {
  const name = PITCH_NAMES[mod12p(root)] ?? "C";
  let label = name + (QUALITY_TO_SUFFIX[quality] ?? "");
  if (bass != null && mod12p(bass) !== mod12p(root)) label += "/" + (PITCH_NAMES[mod12p(bass)] ?? "C");
  return label;
}

export function isMinorishQuality(q: ChordQuality): boolean {
  return q === "min" || q === "min7" || q === "m7b5" || q === "dim" || q === "dim7";
}

/** chordBlock JSON → 等宽小节和弦段（每个和弦占一小节）。 */
export function chordBlockToSegments(block: ChordBlock, beatsPerBar = 4): ChordSegment[] {
  ...
  return block.chords.map((c, i) => {
    ...
    const quality = (c.quality as ChordQuality) ?? parsed.quality;
    return {
...
export function segmentsToChordBlock(segments: ChordSegment[], bpm?: number, ppq = TICKS_PER_BEAT): ChordBlock {
  return {
    type: "chordBlock",
