...
function notesOf(pitches: number[], step = 240, dur = 220, vel = 100): ChordAnalysisNote[] {
...
describe("swingQuantizeNotes", () => {
  ...
  it("quantize=100 吸附到网格，swing=0 时为纯量化", () => {
    const out = swingQuantizeNotes(
      [{ tick: 130, duration: 100, pitch: 60, velocity: 100 }],
      { grid: 8, swing: 0, quantize: 100 },
    );
...
describe("varyRhythm", () => {
  ...
  it("syncopate 同种子可复现，正拍有概率提前", () => {
    const o = { mode: "syncopate" as const, amount: 100, seed: 9 };
