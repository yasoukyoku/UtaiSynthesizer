...
function applyStrategy(s, strat, rng) {
    switch (strat) {
        ...
        case "diatonic": {
            ...
            if (s.quality === "maj")
                return seg(s, s.root + 9, "min");
            if (s.quality === "min")
                return seg(s, s.root + 3, "maj");
            return null;
        }
        case "borrowed": {
            // 同主音借用：平行大小互换（色彩突变）。
            if (s.quality === "maj")
                return seg(s, s.root, "min");
            if (s.quality === "min")
                return seg(s, s.root, "maj");
            return null;
        }
    }
}
export function reharmonizeSegments(segments, o) {
    if (segments.length === 0)
        return [];
    const rng = makeRng(o.seed);
    const density = clamp01(o.density / 100);
    const strategies = o.strategies.length > 0 ? o.strategies : ["extension"];
    return segments.map((s) => {
        if (!rngChance(rng, density))
            return { ...s };
