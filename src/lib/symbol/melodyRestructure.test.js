...
import { melodySimilarity, degreeSwap, rhythmRestructure, contourMorph, motifDevelop, } from "./melodyRestructure";
...
describe("contourMorph（法三，验收核心）", () => {
    ...
    it("轮廓走向保持：非零音程符号不变", () => {
        ...
        for (let i = 0; i + 1 < m.length; i++) {
            ...
            if (a === 0)
                continue; // 同度允许被钳制边界打破
            if (b === 0)
                continue; // 音域贴边的同度塌缩是合法输出
            expect(b).toBe(a);
        }
    });
...
describe("motifDevelop（法四）", () => {
    ...
    it("各技巧都有输出且音域合法", () => {
        const m = testMelody();
        for (const technique of ["sequence", "invert", "retrograde", "augment", "diminish", "mixed"]) {
    ...
    it("同种子可复现", () => {
        const m = testMelody();
        const o = { technique: "mixed", motifBars: 2, seed: 37 };
