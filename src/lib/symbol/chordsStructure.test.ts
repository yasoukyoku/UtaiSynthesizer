...
import type { ChordAnalysisNote, ChordSegment } from "../analysis/chordAnalysis";
import {
  parseChordLabel,
  chordLabel,
  isMinorishQuality,
  chordBlockToSegments,
  segmentsToChordBlock,
  type ChordBlock,
} from "./chords";
...
function seg(i: number, root: number, quality: ChordSegment["quality"]): ChordSegment {
  return {
    startTick: i * 1920,
    endTick: (i + 1) * 1920,
    root,
...
function fourBars(): ChordSegment[] {
  return [seg(0, 0, "maj"), seg(1, 7, "maj"), seg(2, 9, "min"), seg(3, 5, "maj")];
}

function note(tick: number, duration: number, pitch = 72): ChordAnalysisNote {
...
describe("chords：和弦标签解析", () => {
  ...
  it("isMinorishQuality", () => {
    for (const q of ["min", "min7", "m7b5", "dim", "dim7"] as const) {
      expect(isMinorishQuality(q)).toBe(true);
    }
    for (const q of ["maj", "7", "maj7", "sus4", "6"] as const) {
...
describe("structure：段落结构编辑", () => {
  ...
  it("空输入不崩溃", () => {
    for (const op of ["repeatTail", "dropTail", "transposeTail", "lengthenTail"] as const) {
