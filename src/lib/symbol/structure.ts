/**
 * 段落结构编辑（规划 2-7，对应 4.5 结构层面原创化）。
 * 在和弦段网格上做段落级操作：尾段重复 / 尾段删除 / 尾段移调 / 尾段延长。
 * 纯函数，无随机成分（结构操作是确定性的）。
 */
import type { ChordSegment } from "../analysis/chordAnalysis";
import { TICKS_PER_BEAT } from "../constants";
import { mod12p } from "./common";
import { chordLabel } from "./chords";

export type StructureOp = "repeatTail" | "dropTail" | "transposeTail" | "lengthenTail";
...
export interface StructureEditOptions {
  ...
  sectionBars: number;
  beatsPerBar?: number;
  /** transposeTail 的移调半音数。 */
  semitones?: number;
  ppq?: number;
}

function transposeSegment(s: ChordSegment, semis: number): ChordSegment {
...
export function editStructure(segments: ChordSegment[], o: StructureEditOptions): ChordSegment[] {
