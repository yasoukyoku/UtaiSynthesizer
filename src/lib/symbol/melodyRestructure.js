...
function intervalsOf(notes) {
    ...
    for (let i = 1; i < notes.length; i++)
        out.push(notes[i].pitch - notes[i - 1].pitch);
    return out;
}
function ngramSet(seq, n) {
    const s = new Set();
    for (let i = 0; i + n <= seq.length; i++)
        s.add(seq.slice(i, i + n).join(","));
    return s;
}
...
function signsOf(notes) {
    const out = [];
    for (let i = 1; i < notes.length; i++)
        out.push(Math.sign(notes[i].pitch - notes[i - 1].pitch));
    return out;
}
/** 旋律相似度 [0,1]：1 = 完全一致，0 = 完全无关。音高项用相对首音的音高（转置不变）。 */
export function melodySimilarity(a, b) {
...
export function degreeSwap(notes, o) {
    if (notes.length === 0)
        return [];
...
export function rhythmRestructure(notes, o) {
    if (notes.length === 0)
        return [];
    ...
    while (i < sorted.length) {
        ...
        let j = i;
        while (j < sorted.length && Math.floor(sorted[j].tick / barTicks) === barIdx)
            j++;
...
export function contourMorph(notes, o) {
    if (notes.length < 2)
    ...
    for (let i = 0; i + 1 < orig.length; i++) {
        ...
        if (o.keepClimax && i + 1 === climaxIdx) {
            ...
            if (sign > 0)
                pitches[i] = Math.min(pitches[i], orig[i + 1] - 1);
            else
                pitches[i] = Math.max(pitches[i], orig[i + 1] + 1);
            continue;
        }
...
export function motifDevelop(notes, o) {
    if (notes.length === 0)
        return [];
    const barTicks = Math.max(1, o.beatsPerBar ?? 4) * TICKS_PER_BEAT;
    ...
    const TECHS = ["sequence", "invert", "retrograde", "augment", "diminish"];
    const out = [];
    for (let g = 0; g < totalGroups; g++) {
        if (g === 0) {
            for (const n of motifRel)
                out.push({ ...n, tick: n.tick + startTime });
            continue;
        }
        const tech = o.technique === "mixed" ? TECHS[rngInt(rng, 0, TECHS.length - 1)] : o.technique;
...
/** 统一入口（供引擎/测试按法分发）。 */
export function restructureMelody(notes, method, opts) {
    switch (method) {
        case "degreeSwap":
