...
import { resumeLockedFields, resumeWouldBeGuarded, } from "./resumeLock";
const importFs = () => {
    const spec = "node:fs";
 ...
 */
function rustTable(src, backend) {
    const body = src.slice(src.indexOf("pub fn resume_locked_fields"), src.indexOf("/// Everything the guard needs to know"));
    ...
    const evalCond = (cond) => {
        ...
        if (m)
            return [...m[1].matchAll(/"(\w+)"/g)].some((x) => x[1] === backend);
    ...
    for (const raw of body.split("\n")) {
        ...
        for (const m of t.matchAll(/\b(locked|costly)\(\s*"(\w+)"([^)]*)\)/g)) {
            ...
            if (!stack.every((f) => f.applies === true))
                continue;
...
describe("续训锁 Rust ↔ TS 对拍", () => {
    it("每个后端的锁定字段、档位与 scope 完全一致", async () => {
        ...
        for (const b of BACKENDS) {
            ...
            const key = (f) => `${f.id}:${f.tier}:${f.scope}`;
    ...
    it("自检:解析真的读到了东西(否则上面的相等是两个空集)", async () => {
        ...
        const scopeOf = (rows, id) => rows.find((f) => f.id === id)?.scope;
    ...
    it("★条件化一行、或在顶层新增一行,解析器都必须看得见(上一版这两条是瞎的)", async () => {
        ...
        const added = src.replace('v.push(costly("dataset", LockScope::Pool));', 'v.push(costly("dataset", LockScope::Pool));\n    v.push(costly("zzzProbe", LockScope::Run));');
        ...
        const gated = src.replace('v.push(costly("augCopies", LockScope::Pool));', 'if backend == "rvc" {\n        v.push(costly("augCopies", LockScope::Pool));\n    }');
        ...
        const decoy = src.replace('v.push(costly("dataset", LockScope::Pool));', '// v.push(costly("zzzDecoy", LockScope::Run));\n    v.push(costly("dataset", LockScope::Pool));');
...
describe("resumeWouldBeGuarded —— 镜像后端 check_resume_locks(审查 S78)", () => {
    ...
    it("空槽 / 不存在的槽不锁", () => {
        ...
        expect(resumeWouldBeGuarded("rvc", slot({ exists: false, version: "v2", sample_rate: "40k" }))).toBe(false);
    });
    it("★预处理阶段停训(manifest 有版本、无 checkpoint)仍锁 —— 否则 UI 让改、后端拒", () => {
        ...
        expect(resumeWouldBeGuarded("rvc", slot({ has_main_progress: false, version: "v2", sample_rate: "40k" }))).toBe(true);
        expect(resumeWouldBeGuarded("sovits", slot({ has_main_progress: false, version: "4.1", sample_rate: "44k" }))).toBe(true);
    });
    it("已有 checkpoint 的槽当然锁", () => {
        expect(resumeWouldBeGuarded("rvc", slot({ has_main_progress: true, version: "v2", sample_rate: "40k" }))).toBe(true);
    });
    ...
    it("sovits_diff 用 diff_steps 而非 manifest 版本(版本由主模型钉)", () => {
        expect(resumeWouldBeGuarded("sovits_diff", slot({ diff_steps: 0, version: "4.1", sample_rate: "44k" }))).toBe(false);
        expect(resumeWouldBeGuarded("sovits_diff", slot({ diff_steps: 12, version: "4.1", sample_rate: "44k" }))).toBe(true);
    });
});
