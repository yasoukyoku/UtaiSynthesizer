...
export function collectGridTicks(from: number, to: number, gridStep: number): number[] {
  if (gridStep <= 0) return [];
  const first = Math.floor(from / gridStep) * gridStep;
  const out: number[] = [];
  for (let t = first; t <= to + gridStep; t += gridStep) out.push(t);
  return out;
}
...
export function snapTick(tick: number, targets: number[], tol: number, gridStep = 0): number {
...
export function snapMovedStart(startTick: number, durationTicks: number, targets: number[], tol: number, gridStep = 0): number {
  let best = startTick;
  let bestDist = tol;
