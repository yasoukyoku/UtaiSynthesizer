...
export const DEFAULT_TRANSITION = { offsetMs: 0, durLeftMs: 100, durRightMs: 70, depthLeftCents: 15, depthRightCents: 15, openEdgeCents: 200 };
...
export const ZERO_TRANSITION = { offsetMs: 0, durLeftMs: 0, durRightMs: 0, depthLeftCents: 0, depthRightCents: 0, openEdgeCents: 0 };
const TRANSITION_KEYS = ["offsetMs", "durLeftMs", "durRightMs", "depthLeftCents", "depthRightCents", "openEdgeCents"];
const TRANSITION_BOUNDS = {
...
function normalizeTransition(t) {
    ...
    for (const k of TRANSITION_KEYS) {
        ...
        if (typeof v === "number" && Number.isFinite(v))
            out[k] = clampNum(v, TRANSITION_BOUNDS[k][0], TRANSITION_BOUNDS[k][1], 0);
    }
    return out;
}
 ...
 */
export function sanitizeText(s, maxLen = MAX_LYRIC_LEN) {
...
export function normalizeNote(n) {
    ...
    if (n.phoneme)
        out.phoneme = sanitizeText(n.phoneme);
    if (n.detune && Number.isFinite(n.detune))
        out.detune = clampNum(n.detune, -DETUNE_CAP, DETUNE_CAP, 0);
    ...
    if (n.pitchAuto === false)
        out.pitchAuto = false; // absent/true (default) → absent
    if (n.tie)
        out.tie = true; // false → absent (Phase-6 mirror enforces tie ≡ sustain-lyric)
    if (n.autoTuned === true)
        out.autoTuned = true; // S73 机器调教所有权;false → absent
    ...
    if (n.lang && isVocalLangCode(n.lang))
        out.lang = n.lang;
    if (n.phonemeInput)
        out.phonemeInput = sanitizeText(n.phonemeInput);
    ...
    if (pt &&
        ...
        const gain = (pt.gainDb ?? pt.phones.map(() => 0)).map((g) => Number.isFinite(g) ? Math.round(clampNum(g, -12, 12, 0) * 10) / 10 : 0);
 ...
 */
export function normalizeNotesArray(notes, cap = MAX_NOTES_PER_SEGMENT) {
 ...
 */
export function normalizeCurve(curve, kind) {
    if (!curve || !Array.isArray(curve.xs) || !Array.isArray(curve.ys))
        return undefined;
    const roundY = kind === "cents" ? (y) => Math.round(y) : (y) => Math.round(y * 1000) / 1000;
    const pts = [];
 ...
 */
export function retimeNote(note, dTickStart, dDuration) {
 ...
 */
export function resolveOverlaps(notes, activeIds, minTicks) {
    const active = notes
    ...
    for (const n of notes) {
        ...
        for (const a of active) {
            if (a.end <= start || a.start >= end)
                continue; // disjoint / abutting (legato-legal)
            if (a.start <= start)
                start = Math.max(start, a.end); // active covers/precedes head → push head to its end
            else
                end = Math.min(end, a.start); // active starts inside → truncate tail to its start (keep head)
        }
        const dur = end - start;
        if (dur < minTicks)
            continue; // swallowed / clipped below the audible floor → drop
 ...
 */
export function isBreathLyric(lyric, breathToken) {
    const l = lyric.trim();
 ...
 */
export function restTokenKey(restToken) {
...
export function breathTokenKey(breathToken) {
 ...
 */
export function isRestLyric(lyric, restToken) {
    const l = lyric.trim();
...
export function isSilentLyric(lyric, tokens) {
...
export function splitLyricTokens(s, emptyFallback) {
    if (!s)
    ...
    if (/\s/.test(s))
        return s.match(/[^\s[［]*[[［][^\]］]*[\]］](?!\S)|\S+/g) ?? [emptyFallback];
    ...
    const out = [];
    for (const ch of s) {
        if (out.length > 0 && SMALL_KANA.has(ch))
            out[out.length - 1] += ch;
        else
            out.push(ch);
    }
 ...
 */
export function sanitizeVocalParams(p) {
...
function sanitizeOpts(raw, defaults) {
