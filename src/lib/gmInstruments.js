...
const normalizeKey = (s) => s.trim().toLowerCase().replace(/\s+/g, " ");
const BY_EN_KEY = new Map(GM_INSTRUMENTS.map((g) => [normalizeKey(g.en), g]));
/**
 * 由轨道名（GM 全名或 YourMT3 口语名）解析显示名与 GM program。
 * 匹配顺序：别名精确 → GM 全名精确 → 原样返回（display=原名, program=undefined 语义用 matched 表达）。
 */
export function matchGmName(name, lang) {
...
export function translateGmName(name, lang) {
    return matchGmName(name, lang).display;
}
/** 名称是否更像鼓轨（用于试听通道选择）。 */
export function isDrumLikeName(name) {
    const key = normalizeKey(name);
 ...
 */
const VOCAL_NAME_RE = /vocal|voice|singer|singing|choir|aahs|oohs|人声|干声|干音|主唱|演唱|唱歌/i;
export function isVocalTrackName(name) {
    const key = normalizeKey(name || "");
    if (!key)
        return false;
    if (isDrumLikeName(key))
        return false; // "Drum Voice" 之类的打击乐名不算干音
...
export function gmDrumName(pitch, lang) {
...
export function gmDrumShort(pitch, lang) {
    const d = GM_DRUMS[pitch];
    if (!d)
        return "";
...
const DRUM_GROUP_ORDER = {
    kick: 0, snare: 1, tom: 2, hihat: 3, cymbal: 4, perc: 5,
};
export function gmDrumOrder(pitch) {
    const d = GM_DRUMS[pitch];
