...
export const ACCENT_RGB = [57, 197, 187];
...
export function hexToRgb(hex, fallback = ACCENT_RGB) {
...
export function pickUniqueTrackColor(existing) {
    const used = new Map();
    for (const t of existing) {
        if (t.color && t.color.startsWith("#"))
            used.set(t.color, (used.get(t.color) ?? 0) + 1);
    }
    let min = Infinity;
    for (const c of RANDOM_TRACK_COLORS)
        min = Math.min(min, used.get(c) ?? 0);
