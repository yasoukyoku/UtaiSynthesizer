...
import { cleanChangedTotal, detectKeyPitchClasses, smartCleanStems, } from "./midiCleanup";
const OPTS = { bpm: 120, ppq: 480, durationTicks: 1920 * 10 };
const note = (tick, duration, pitch, velocity = 100, id) => ({ tick, duration, pitch, velocity, id });
describe("smartCleanStems v3 — 新增 Pass", () => {
    ...
    it("拖尾截断：鼓轨超过半小节封顶", () => {
        const stems = [{ channel: 9, notes: [note(0, 3000, 38)] }];
        const stats = smartCleanStems(stems, OPTS);
    ...
    it("节拍量化：远离网格的切分音不受影响", () => {
        const stems = [{ channel: 0, notes: [note(505, 240, 60)] }];
    ...
    it("周期性补漏（v3 规则二）：非紧邻重复的空小节按周期回填", () => {
        ...
        layout.forEach((p, bar) => {
            if (p != null)
                notes.push(note(bar * 1920, 480, p));
        });
        const stems = [{ channel: 0, notes }];
        const stats = smartCleanStems(stems, OPTS);
...
describe("v2 行为回归", () => {
    ...
    it("音域外音高删除", () => {
        const stems = [{ channel: 0, notes: [note(0, 480, 120)] }];
        const stats = smartCleanStems(stems, OPTS);
    ...
    it("detectKeyPitchClasses 音符太少返回 null", () => {
        const stems = [{ channel: 0, notes: [note(0, 480, 60)] }];
