...
function readStore() {
    const raw = loadSetting(STORAGE_KEY, {});
...
export function listNodeParamPresets(nodeType) {
...
export function saveNodeParamPreset(nodeType, name, params) {
    ...
    if (at < 0 && list.length >= MAX_PRESETS_PER_TYPE)
        return false;
    const entry = { name: trimmed, params: clean, updatedAt: Date.now() };
...
export function deleteNodeParamPreset(nodeType, name) {
    const store = readStore();
    const list = Array.isArray(store[nodeType]) ? store[nodeType] : [];
    ...
    if (next.length === 0)
        delete nextStore[nodeType];
    else
        nextStore[nodeType] = next;
