...
export type PortKind = "audio" | "midi" | "chords" | "lyrics" | "report" | "any";
...
const MIDI = "midi" as const;
const CHORDS = "chords" as const;
const LYRICS = "lyrics" as const;
const REPORT = "report" as const;
const ANY = "any" as const;

const audio = (n: number): PortKind[] => Array.from({ length: n }, () => AUDIO);
const midi = (n: number): PortKind[] => Array.from({ length: n }, () => MIDI);

export const NODE_PORTS: Record<WorkflowNodeType, NodePorts> = {
  // I/O 与母带链
...
export function inputPortKind(nodeType: WorkflowNodeType, port: number): PortKind {
  return NODE_PORTS[nodeType]?.inputs[port] ?? ANY;
}

/** 取某出端口类型；越界（可变分轨数）回退 any。 */
export function outputPortKind(nodeType: WorkflowNodeType, port: number): PortKind {
...
export function canConnect(
  ...
  return portsCompatible(
    outputPortKind(fromType, fromPort),
    inputPortKind(toType, toPort),
  );
}
