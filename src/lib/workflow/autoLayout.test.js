...
function n(id, x = 0, y = 0, measured) {
...
describe("computeAutoLayout · 分层", () => {
    ...
    it("同层多个节点按实测高度往下堆,不重叠", () => {
        const r = computeAutoLayout([n("src"), n("t1", 0, 0, { width: W, height: 300 }), n("t2", 0, 999, { width: W, height: 80 })], [e("src", "t1"), e("src", "t2")]);
        const t1 = at(r, "t1");
        const t2 = at(r, "t2");
    ...
    it("列宽取该列最宽的节点,宽卡片不会压到下一列", () => {
        const r = computeAutoLayout([n("a", 0, 0, { width: 280, height: H }), n("b")], [e("a", "b")]);
        expect(at(r, "b").x).toBe(280 + COLUMN_GAP);
    });
    it("同列顺序按上游 y 的重心排,保留用户原有的上下关系", () => {
        // t2 的上游在上面、t1 的上游在下面 → 整理后 t2 应该排在 t1 上面。
        const r = computeAutoLayout([n("hi", 0, 0), n("lo", 0, 500), n("t1"), n("t2")], [e("lo", "t1"), e("hi", "t2")]);
...
describe("computeAutoLayout · 连通分量", () => {
    it("互不相连的子图上下分开,不在同一列里交错", () => {
        const r = computeAutoLayout([n("a1"), n("a2"), n("b1"), n("b2")], [e("a1", "a2"), e("b1", "b2")]);
...
describe("computeAutoLayout · 纯函数 / 起点", () => {
    ...
    it("measured 缺失或非法(0/NaN)时退回默认尺寸", () => {
        const r = computeAutoLayout([n("a", 0, 0, { width: 0, height: Number.NaN }), n("b")], [e("a", "b")]);
        expect(at(r, "b").x).toBe(PITCH);
    });
});
