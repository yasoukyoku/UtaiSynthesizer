...
import { MSST_CATALOG, MSST_DEFAULT_PRECISION } from "../models/msst-catalog";
...
import { runSongTask } from "../song/runSongTask";
import { ACE_TRACK_CLASSES, DEMUCS_STEM_KINDS } from "../models/song-tasks";
import { abcToMidi } from "../backendSong";
// P2 符号域原创化节点族（规划 7.1）：确定性 MIDI 变换 / 旋律重构 / 和声重配 / 曲式编辑 / 换气规划。
import { humanizeNotes, applyVelocityCurve, swingQuantizeNotes, varyRhythm, } from "../symbol/midiTransforms";
import { degreeSwap, rhythmRestructure, contourMorph, motifDevelop, melodySimilarity, } from "../symbol/melodyRestructure";
import { reharmonizeSegments } from "../symbol/reharmonize";
import { editStructure } from "../symbol/structure";
...
export async function audioBufferToWav(buf) {
    ...
    off += 2; // PCM
    view.setUint16(off, numCh, true);
    off += 2;
    view.setUint32(off, sr, true);
    off += 4;
...
const voiceInvokesInFlight = new Map();
const voiceDrainWaiters = new Set();
...
async function waitVoiceDrain(segmentId) {
    if ((voiceInvokesInFlight.get(segmentId) ?? 0) === 0)
        return true;
 ...
 *  in executeNode). */
async function rejectIfSeparationBusy(segmentId, workflow, targetNodeId) {
    ...
    if (!needsSeparation)
        return false;
    const status = await invoke("get_separation_status")
        .catch(() => null);
    const busy = status !== null && typeof status.state === "string"
        && (status.state === "Separating" || status.state === "LoadingModel");
    if (busy)
        useAppStore.getState().showToast(i18n.t("workflow.separationBusy"), "error");
    return busy;
}
...
export async function collectMissingModels(workflow, targetNodeId) {
    let scope = null;
    ...
    if (msstNodes.length > 0) {
        ...
        for (const n of msstNodes) {
            const modelFile = n.params.modelFile ?? "";
            if (!modelFile || seen.has(modelFile))
                continue;
            seen.add(modelFile);
            ...
            if (!entry.has_onnx && !entry.has_fp16) {
                ...
                const detected = entry.architecture !== "unknown" ? entry.architecture : undefined;
                const arch = MSST_CATALOG.find((e) => e.filename === modelFile)?.architecture ?? detected;
                const precision = n.params.precision ??
    ...
    if (nodes.some((n) => n.nodeType === "rvc" || n.nodeType === "sovits")) {
        try {
            const packs = await invoke("asset_pack_status");
...
function graphErrorMessage(msg) {
    if (msg.includes("more than one input node"))
        return i18n.t("workflow.errGraphMultiInput");
    if (msg.includes("no input node"))
        return i18n.t("workflow.errGraphNoInput");
    if (msg.includes("no output nodes"))
        return i18n.t("workflow.errGraphNoOutput");
    if (msg.includes("cycle"))
        return i18n.t("workflow.errGraphCycle");
...
export async function preflightRun(segmentId, workflow, targetNodeId) {
    ...
    if (!(await waitVoiceDrain(segmentId)))
        return false;
    if (running()) { // a run may have started while we drained
        useAppStore.getState().showToast(i18n.t("workflow.runBusy"), "info");
        return false;
    }
    if (await rejectIfSeparationBusy(segmentId, workflow, targetNodeId))
        return false;
...
async function ensureRunDir(segmentId) {
    const raw = await invoke("ensure_cache_dir", {
...
function bypassPassThrough(gn, nodeType, dataMap) {
    const ins = [];
    for (const e of gn.inEdges) {
        const v = dataMap.get(e.fromNode)?.get(e.fromPort);
        if (v)
            ins.push({ toPort: e.toPort, path: v });
    }
...
export async function executeWorkflow(segmentId, segment, workflow) {
...
function ancestorSetOf(graph, targetNodeId) {
    const anc = new Set([targetNodeId]);
...
export async function executeSingleNode(segmentId, segment, workflow, targetNodeId) {
    const store = useWorkflowStore.getState();
    ...
    try {
        ...
        for (const nodeId of graph.sorted) {
            ...
            store.setNodeStatus(segmentId, nodeId, "running");
            activeNodeId = nodeId;
            const outputData = await executeNode(nodeId, gn.node.nodeType, gn.node.params, gn, dataMap, cacheDir, segmentId);
            dataMap.set(nodeId, outputData);
            if (outputData.size > 0) {
...
async function runSongNode(task, payload, nodeId, segmentId) {
...
function pickEnum(params, key, values, fallbackIdx) {
    const idx = Math.max(0, Math.min(values.length - 1, Math.round(params[key] ?? fallbackIdx)));
    return values[idx];
}
...
async function resolveMidiNotes(input, what) {
    ...
    if (trimmed.startsWith("[") || trimmed.startsWith("{")) {
        try {
            ...
            if (Array.isArray(rawNotes)) {
                const notes = rawNotes.filter((n) => n && typeof n.pitch === "number" && typeof n.tick === "number");
                if (notes.length > 0)
                    return { notes, ppq: typeof parsed?.ppq === "number" ? parsed.ppq : 480 };
            }
        }
    ...
    if (ext === "mid" || ext === "midi") {
        const res = await invoke("import_score_file", { path: input });
        const notes = (res?.tracks ?? []).flatMap((t) => (t.notes ?? []).map((n) => ({
            tick: n.tick, duration: n.duration, pitch: n.pitch, velocity: n.velocity ?? 100,
        })));
        if (notes.length > 0)
            return { notes, ppq: res?.ppq ?? 480 };
    }
...
function resolveChordSegments(input, what) {
    try {
        ...
        const segments = raw.flatMap((c, i) => {
...
async function executeNode(nodeId, nodeType, params, gn, dataMap, cacheDir, segmentId) {
    ...
    switch (nodeType) {
        ...
        case "sovits": {
            ...
            const modelPath = await healVoiceModelPath(nodeType, voiceName, params.modelPath);
            ...
            const unlisten = await listen("voice-progress", (e) => {
            ...
            try {
                ...
                await invoke(isRvc ? "run_rvc" : "run_sovits", {
                    voiceName,
                    modelPath,
        ...
        case "msstSeparation": {
            ...
            const modelFile = params.modelFile ?? "";
            const arch = MSST_CATALOG.find((e) => e.filename === modelFile)?.architecture ??
            ...
            const config = {
                ...
                modelPath: await healMsstModelPath(params.modelFile, params.modelPath ?? params.modelName ?? ""),
                ...
                useTta: params.useTta ?? false,
                shifts: params.shifts ?? 0,
                ...
                ...(params.numOverlap !== undefined ? { numOverlap: params.numOverlap } : {}),
                ...
                ...(params.aggression !== undefined ? { aggression: params.aggression } : {}),
                ...(params.postProcess !== undefined ? { postProcess: params.postProcess } : {}),
                ...(params.postProcessThreshold !== undefined ? { postProcessThreshold: params.postProcessThreshold } : {}),
            ...
            let status = await invoke("get_separation_status");
            ...
            if (typeof status.state === "object") {
                const errMsg = status.state.Error ?? "MSST separation failed";
        ...
        case "split": {
            const numOutputs = params.outputs ?? 2;
        ...
        case "complianceCheck": {
            ...
            const report = await invoke("compliance_check", { path: primaryInput });
            outputData.set(0, primaryInput);
        ...
        case "busEq": {
            ...
            const g = (i) => Math.max(-12, Math.min(12, params[`eqGain${i}`] ?? 0));
            const bands = [
                { type: "hpf", freq: 30, gain_db: 0, q: 0.707, enabled: (params.eqHpf ?? 1) > 0 },
        ...
        case "spectrogram": {
            ...
            const report = await invoke("analyze_spectrogram", {
                input: primaryInput,
                output: pngPath,
            });
            outputData.set(0, primaryInput);
        ...
        case "f0Curve": {
            ...
            const report = await invoke("track_f0", { input: primaryInput });
            outputData.set(0, primaryInput);
            outputData.set(1, JSON.stringify(report));
            break;
        }
        case "timbreMetrics": {
            ...
            const report = await invoke("analyze_timbre", { input: primaryInput });
            outputData.set(0, primaryInput);
            outputData.set(1, JSON.stringify(report));
            break;
        }
        case "harmonicityCheck": {
            ...
            const f0Hz = Math.max(0, params.f0Hz ?? 0);
            const report = await invoke("analyze_harmonicity", {
                input: primaryInput,
        ...
        case "spectralCompare": {
            ...
            if (!bInput)
                throw new Error(`Node "${nodeId}" (spectralCompare) has no B input connected`);
            const report = await invoke("compare_spectra", {
                inputA: primaryInput,
                inputB: bInput,
            });
            outputData.set(0, primaryInput);
        ...
        case "dtwAlign": {
            ...
            const bInput = inputPaths.get(1);
            if (!bInput)
                throw new Error(`Node "${nodeId}" (dtwAlign) has no B input connected`);
            const band = Math.max(0, Math.round(params.band ?? 0));
            const report = await invoke("dtw_compare", {
                inputA: primaryInput,
                inputB: bInput,
        ...
        case "abCompare": {
            ...
            const bInput = inputPaths.get(1);
            if (!bInput)
                throw new Error(`Node "${nodeId}" (abCompare) has no B input connected`);
            outputData.set(0, primaryInput);
        ...
        case "lufsAnalyze": {
            ...
            const targetLufs = params.targetLufs ?? -16;
            const report = await invoke("measure_loudness", {
                path: primaryInput,
            });
            const integrated = report.integrated_lufs;
            outputData.set(0, primaryInput);
        ...
        case "amtMidi": {
            ...
            const rawChain = params.muscriptorChain ?? "official";
            ...
            let lastProgressAt = Date.now();
            const unlisten = await listen("amt-progress", (e) => {
                if (e.payload.node_id && e.payload.node_id !== progressKey)
                    return;
            ...
            const watchdog = setInterval(() => {
                if (Date.now() - lastProgressAt <= AMT_STALL_TIMEOUT)
                    return;
                stalled = true;
            ...
            try {
                ...
                let slot = 1;
                if (res.merged_midi_path)
                    outputData.set(slot++, res.merged_midi_path);
                if (res.vocal_midi_path)
                    outputData.set(slot++, res.vocal_midi_path);
                if (res.accompaniment_midi_path)
                    outputData.set(slot++, res.accompaniment_midi_path);
                if (res.stem_midi_paths) {
                    for (const [, p] of Object.entries(res.stem_midi_paths)) {
                        if (p)
                            outputData.set(slot++, p);
                    }
                }
                ...
                if (isAudio) {
                    void (async () => {
                        try {
                            const tempo = await invoke("analyze_segment_tempo", { path: primaryInput, windowStartMs: 0, windowEndMs: 0, beats_per_bar: 4 });
                            ...
                            if (tempo && tempo.bpm > 30 && tempo.bpm < 300) {
                                ...
                                if (Math.abs(prev - tempo.bpm) > 0.5) {
                                    ...
                                    useAppStore.getState().showToast(`🎵 Beat-This: 检测 BPM ${Math.round(tempo.bpm * 10) / 10} (置信度 ${Math.round(tempo.confidence * 100)}%) — 工程已自动填入`, "info");
                                }
                            }
                        }
        ...
        case "speedShift": {
            ...
            try {
                const result = await invoke("stretch_segment_audio", { path: primaryInput, time_factor: timeFactor });
                ...
                useAppStore.getState().showToast(i18n.t("workflow.warnSpeedShiftFallback", { detail }), "warning");
            }
            break;
        }
        case "chordDetect": {
            if (isSourceNode)
                throw new Error("chordDetect needs audio/MIDI input");
            if (!primaryInput)
                throw new Error("chordDetect: no input connected");
            const ext = (primaryInput.split(".").pop() ?? "").toLowerCase();
            let notes = [];
            let ppq = 480;
            if (ext === "mid" || ext === "midi") {
                const res = await invoke("import_score_file", { path: primaryInput });
                notes = (res?.tracks ?? []).flatMap((t) => (t.notes ?? []).map((n) => ({ tick: n.tick, duration: n.duration, pitch: n.pitch, velocity: n.velocity })));
                ppq = res?.ppq ?? 480;
            }
            else {
            ...
            if (notes.length === 0)
                throw new Error("chordDetect: input has no notes");
        ...
        case "autoArrange": {
            if (isSourceNode)
                throw new Error("autoArrange needs MIDI or chord input");
            if (!primaryInput)
                throw new Error("autoArrange: no input connected");
            ...
            if (primaryInput.trim().startsWith("{")) {
                ...
                if (ext === "mid" || ext === "midi") {
                    const res = await invoke("import_score_file", { path: primaryInput });
                    notes = (res?.tracks ?? []).flatMap((t) => (t.notes ?? []).map((n) => ({ tick: n.tick, duration: n.duration, pitch: n.pitch, velocity: n.velocity ?? 100 })));
                    void (res?.ppq);
                }
                else {
            ...
            if (chordAnalysisNotes.length === 0)
                throw new Error("autoArrange: no notes or chord blocks to arrange");
            ...
            if (!res || res.bars === 0)
                throw new Error("autoArrange: arrange engine produced no output");
            ...
            const outDir = (await invoke("ensure_cache_dir", { segmentId: `${segmentId}/arrange_${nodeId}` })).replace(/\\/g, "/");
            ...
            const manifest = { style, mood, key: res.key.label, confidence: res.key.confidence, bars: res.bars, startTick: res.startTick, endTick: res.endTick };
        ...
        case "deepOriginal": {
            ...
            if (!isSourceNode && primaryInput && Math.abs(rate - 1.0) > 0.001) {
                try {
                    const s = await invoke("stretch_segment_audio", { path: primaryInput, time_factor: 1 / rate });
                    ...
                    useWorkflowStore.getState().setNodeStatus(segmentId, nodeId, "degraded");
                    useWorkflowStore.getState().setNodeError(segmentId, nodeId, detail);
                    useAppStore.getState().showToast(i18n.t("workflow.warnDeepOrigStretchFallback", { detail }), "warning");
                }
            }
            if (!isSourceNode && primaryInput && semitones !== 0) {
                try {
                    ...
                    useWorkflowStore.getState().setNodeStatus(segmentId, nodeId, "degraded");
                    useWorkflowStore.getState().setNodeError(segmentId, nodeId, detail);
                    useAppStore.getState().showToast(i18n.t("workflow.warnDeepOrigTransposeFallback", { detail }), "warning");
                }
            }
            for (let p = 0; p < 5; p++)
                outputData.set(p, outPath);
            break;
        }
        case "midiFileIn": {
            const filePath = params.filePath ?? "";
            if (!filePath)
                throw new Error("MIDI File In: pick a .mid file first (click the button)");
            outputData.set(0, filePath);
            break;
        }
        case "chordBlockIn": {
            const chordStr = params.chords ?? "C | Am | F | G";
            const bpm = params.bpm ?? 120;
            const rootMap2 = { C: 0, "C#": 1, Db: 1, D: 2, "D#": 3, Eb: 3, E: 4, F: 5, "F#": 6, Gb: 6, G: 7, "G#": 8, Ab: 8, A: 9, "A#": 10, Bb: 10, B: 11 };
        ...
        case "soundfontRender": {
            ...
            if (!primaryInput)
                throw new Error("soundfontRender: no input connected");
            ...
            if (primaryInput.trim().startsWith("{") || primaryInput.trim().startsWith("[")) {
                try {
                    ...
                    if (Array.isArray(parsed))
                        ...
                        sfNotes = parsed.notes;
                    if (!Array.isArray(parsed) && typeof parsed.bpm === "number")
                        sfBpm = parsed.bpm;
                }
                catch {
                ...
                const score = await invoke("import_score_file", { path: primaryInput });
                sfNotes = (score?.tracks ?? []).flatMap((tr) => (tr.notes ?? []).map((n) => ({
                ...
                if (typeof score?.bpm === "number" && score.bpm > 0)
                    sfBpm = score.bpm;
            }
            if (sfNotes.length === 0)
                throw new Error("soundfontRender: input has no notes");
            ...
            const sfWavPath = await invoke("render_soundfont_notes", {
                fontId,
                presetId,
                notes: renderNotes,
                sampleRate: params.sampleRate ?? 44100,
                backend: params.backend ?? "builtin",
            });
        ...
        case "melodySimilarity": {
            ...
            if (!primaryInput)
                throw new Error(`Node "${nodeId}" (melodySimilarity) has no A input connected`);
            const msBInput = inputPaths.get(1);
            if (!msBInput)
                throw new Error(`Node "${nodeId}" (melodySimilarity) has no B input connected`);
            ...
            const msThreshold = Math.max(0, Math.min(100, params.threshold ?? 35)) / 100;
        ...
        case "harmonizer": {
            if (isSourceNode)
                throw new Error("harmonizer needs MIDI or chord input");
            if (!primaryInput)
                throw new Error("harmonizer: no input connected");
            let notes = [];
            if (primaryInput.trim().startsWith("{")) {
                try {
                    const parsed = JSON.parse(primaryInput);
            ...
            if (notes.length === 0)
                throw new Error("harmonizer: no notes");
            const proj = useProjectStore.getState();
            const style = params.chordStyle ?? "POP_STANDARD";
            const chordsPerBar = params.chordsPerBar ?? 1;
            const keyName = params.key ?? "auto";
            ...
            if (!res)
                throw new Error("harmonizer: no chord output");
        ...
        case "melodyGen": {
            if (isSourceNode)
                throw new Error("melodyGen needs chord input");
            if (!primaryInput)
                throw new Error("melodyGen: no chord input connected");
            ...
            const style = params.style ?? "pop";
            const mood = params.mood ?? "neutral";
            const stubNotes = chordSegs.map((c) => ({
                tick: c.startTick,
            ...
            if (!res)
                throw new Error("melodyGen: arrange failed");
        ...
        case "songLyrics": {
            ...
            if (!text)
                throw new Error(i18n.t("songNode.errEmptyLyrics"));
        ...
        case "songPrompt": {
            const text = (params.text ?? "").trim();
            if (!text)
                throw new Error(i18n.t("songNode.errEmptyPrompt"));
            // 同用 lyrics:// 前缀标记"虚拟文本"（11.3）；songGen 按端口序区分歌词/提示词
            outputData.set(0, `lyrics://${text}`);
            break;
        }
        case "songGen": {
            const task = params.songTask === "instrumental" ? "instrumental" : "generate";
            const lyrics = songPortText(inputPaths.get(0)) ?? params.lyrics ?? "";
            const prompt = songPortText(inputPaths.get(1)) ?? params.prompt ?? "";
            const refAudio = inputPaths.get(2);
            const payload = {
                model: params.model ?? "acestep-v1.5",
                lyrics,
                prompt,
                durationSec: params.durationSec ?? 0,
                seed: params.seed ?? undefined,
                wantStems: params.wantStems ?? false,
                wantMidi: params.wantMidi ?? false,
                wantLrc: params.wantLrc ?? false,
                extra: {
                    ...(params.cot ? { cot: params.cot } : {}),
            ...
            if (audio)
                outputData.set(0, audio);
            if (midi)
                outputData.set(1, midi);
            if (lrc)
                outputData.set(2, lrc);
        ...
        case "songCover": {
            ...
            const payload = {
                songName: params.songName || undefined,
                srcAudioPath: primaryInput,
                prompt,
                coverStrength: params.coverStrength ?? 0.5,
        ...
        case "songRepaint": {
            ...
            const payload = {
                ...
                srcAudioPath: primaryInput,
                prompt,
                repaintStart: params.repaintStart ?? 0,
                repaintEnd: params.repaintEnd ?? 0,
            };
        ...
        case "songComplete": {
            ...
            if (stems) {
                ...
                classes.forEach((cls, i) => {
                    const p = stems[cls];
                    if (p)
                        outputData.set(1 + i, p);
                });
            }
            break;
        }
        case "songExtract": {
            const classes = params.trackClasses ?? [...ACE_TRACK_CLASSES];
            const payload = {
                srcAudioPath: primaryInput,
                trackClasses: classes,
            };
        ...
        case "songLego": {
            const payload = {
                srcAudioPath: primaryInput,
                prompt: songPortText(inputPaths.get(1)) ?? "",
                trackName: params.trackName ?? "guitar",
            };
        ...
        case "midiHumanize": {
            ...
            const out = humanizeNotes(notes, {
                timingMs: params.timingMs ?? 12,
                velocityJitter: params.velocityJitter ?? 8,
                tempo: useProjectStore.getState().tempo,
                seed: params.seed ?? DEFAULT_SYMBOL_SEED,
            });
        ...
        case "velocityCurve": {
            ...
            const curve = pickEnum(params, "curve", ["crescendo", "decrescendo", "arch", "custom"], 2);
            // custom 曲线：0-100 采样值序列（逗号/空白分隔），归一到 0..1（applyVelocityCurve 的 shape 约定）
            const shape = (params.customShape ?? "")
                .split(/[\s,;]+/)
                .filter((s) => s !== "")
                .map((s) => Number(s))
        ...
        case "swingQuantize": {
            ...
            const out = swingQuantizeNotes(notes, {
                grid: params.grid === 1 ? 16 : 8,
                swing: params.swing ?? 55,
                quantize: params.quantize ?? 0,
            });
            outputData.set(0, JSON.stringify(out));
            break;
        }
        ...
        case "rhythmRestructure": {
            ...
            const out = rhythmRestructure(notes, {
                strength: params.strength ?? 60,
                beatsPerBar: params.beatsPerBar ?? proj.timeSignature[0] ?? 4,
                seed: params.seed ?? DEFAULT_SYMBOL_SEED,
            });
            outputData.set(0, JSON.stringify(out));
            break;
        ...
        case "contourMorph": {
            ...
            const out = contourMorph(notes, {
                strength: params.strength ?? 60,
                keepClimax: params.keepClimax !== 0,
                seed: params.seed ?? DEFAULT_SYMBOL_SEED,
            });
            outputData.set(0, JSON.stringify(out));
            break;
        ...
        case "motifDevelop": {
            ...
            const out = motifDevelop(notes, {
                technique: pickEnum(params, "technique", ["sequence", "invert", "retrograde", "augment", "diminish", "mixed"], 5),
                motifBars: Math.min(4, Math.max(2, params.motifBars ?? 2)),
        ...
        case "reharmonize": {
            ...
            const out = reharmonizeSegments(chordSegs, {
                strategies: [pickEnum(params, "strategy", ["diatonic", "borrowed", "tritone", "extension"], 0)],
        ...
        case "rhythmVariation": {
            ...
            const out = varyRhythm(notes, {
                mode: pickEnum(params, "mode", ["push", "layBack", "syncopate", "sparse"], 0),
        ...
        case "structureEdit": {
            ...
            const out = editStructure(chordSegs, {
                op: pickEnum(params, "op", ["repeatTail", "dropTail", "transposeTail", "lengthenTail"], 0),
                sectionBars: Math.min(8, Math.max(1, params.sectionBars ?? 4)),
                beatsPerBar,
                semitones: Math.min(12, Math.max(-12, params.semitones ?? 2)),
                ppq: TICKS_PER_BEAT,
            });
            outputData.set(0, JSON.stringify(segmentsToChordBlock(out, bpm)));
            outputData.set(1, JSON.stringify({
        ...
        case "breathPlanner": {
            ...
            const points = planBreathPoints(notes, {
                minGapBeats: params.minGapBeats ?? 1,
                ppq,
                ...(lyrics ? { lyrics } : {}),
            });
...
async function materializeAmtMidiTracks(nodeId, midiPath, trackMode = "multi_track", sourceAudioPath, playbackDir) {
    ...
    try {
        ...
        for (const t of [...project.tracks]) {
            if (t.amtNodeId === nodeId)
                useProjectStore.getState().removeTrack(t.id);
        }
        let score;
        try {
            score = await invoke("import_score_file", { path: midiPath });
        }
        catch (e) {
        ...
        if (tracks.length === 0)
            return;
        const base = midiPath.split(/[/\\]/).pop()?.replace(/\.[^.]+$/, "") || nodeId;
        ...
        let metaTracks = [];
        try {
            const meta = await invoke("amt_midi_metadata", { midiPath });
            metaTracks = meta?.tracks ?? [];
        }
        catch { /* best-effort */ }
        const metaFor = (name) => metaTracks.find((m) => m.name === name) || undefined;
        /** Build a playable audio lane for one track's WAV (peaks + duration). */
        const buildLane = async (wav, segmentId, label, maxTick) => {
        ...
        if (trackMode === "single_track") {
            const allNotes = tracks.flatMap((it) => it.notes.map((n) => ({
        ...
        for (let i = 0; i < tracks.length; i++) {
            ...
            const maxTick = it.notes.reduce((m, n) => Math.max(m, n.tick + n.duration), it.start_tick ?? 0);
...
function localizeTrackName(raw, index) {
    if (!raw)
        return `轨道 ${index + 1}`;
    const trimmed = raw.trim();
    // 先精确匹配
    if (STEM_ZH_MAP[trimmed])
        return STEM_ZH_MAP[trimmed];
...
function laneStem(graph, inEdgeCount, edge) {
    const stems = graph.nodes.get(edge.fromNode)?.node.params
        ?.stemLabels;
...
function laneLabelFor(graph, base, inEdgeCount, edge) {
...
function laneIdFor(outputNodeId, edge) {
...
function countOutputLanes(graph, dataMap) {
    let count = 0;
    for (const outId of graph.outputNodeIds) {
        const gn = graph.nodes.get(outId);
        const base = gn.node.params.laneLabel ?? DEFAULT_OUTPUT_GROUP;
        for (const edge of gn.inEdges) {
...
const cacheDecodeFailures = new Map();
const DECODE_GIVE_UP = 3;
function noteDecodeFailure(segmentId, audioPath) {
    const k = `${segmentId}|${audioPath}`;
    cacheDecodeFailures.set(k, (cacheDecodeFailures.get(k) ?? 0) + 1);
}
function decodeGivenUp(segmentId, audioPath) {
 ...
 */
export function collectCachedPaths(segmentId, outputNodeId, workflow) {
    const graph = parseWorkflowGraph(workflow);
 ...
 */
export async function depositFromCache(trackId, segmentId, workflow) {
    ...
    let graph = null;
    try {
        graph = parseWorkflowGraph(workflow);
    }
    catch { /* broken/incomplete graph — still prune below */ }
    let changed = false;
    if (graph) {
        ...
        const alreadyAt = new Map((segBefore?.processedOutputs ?? []).filter((o) => !o.loading).map((o) => [o.laneId, o.audioPath]));
        for (const outId of graph.outputNodeIds) {
            ...
            if (decoded.length > 0) {
                ...
                const kept = (segBefore?.processedOutputs ?? []).filter((o) => o.outputNodeId === outId && !o.loading
    ...
    if (runningNow())
        return changed; // a new run owns the placeholders now — never prune them
...
export async function loadCachedOutput(p) {
    const info = await invoke("load_audio_file", { path: p.audioPath });
    return {
        laneId: p.laneId,
...
export function outputLanes(workflow, outputNodeId) {
    const graph = parseWorkflowGraph(workflow);
    const gn = graph.nodes.get(outputNodeId);
    if (!gn)
        return [];
...
export function hasUndepositedCache(segmentId, workflow, outs) {
    if (!workflow)
        return false;
    ...
    const deposited = new Map((outs ?? []).filter((o) => !o.loading).map((o) => [o.laneId, o.audioPath]));
    for (const outId of graph.outputNodeIds) {
        const { paths } = collectCachedPaths(segmentId, outId, workflow);
        for (const p of paths) {
            ...
            if (deposited.get(p.laneId) !== p.audioPath && !decodeGivenUp(segmentId, p.audioPath))
                return true;
        }
    }
    return false;
...
export function collectGroupNames(tracks, extra = []) {
    const names = new Set([DEFAULT_OUTPUT_GROUP, ...extra.filter(Boolean)]);
 ...
 */
export function planDetachGroup(workflow, outputNodeId) {
    let graph;
    try {
        graph = parseWorkflowGraph(workflow);
    }
 ...
 */
export function rehydrateRenderState(segmentId, segment) {
    const wf = segment.workflow;
    const outs = (segment.processedOutputs ?? []).filter((o) => !o.loading && o.outputNodeId && o.audioPath && !o.audioPath.startsWith("__pending"));
    if (!wf || outs.length === 0)
        return;
    const store = useWorkflowStore.getState();
    const warm = store.nodeOutputs[segmentId];
    if (warm && Object.keys(warm).length > 0)
        return; // already warm (live / just-run) — don't clobber
    let graph;
    try {
    ...
    const byLaneId = new Map(outs.map((o) => [o.laneId, o]));
    const nodeOutputs = {};
    const outputIds = new Set();
    for (const outId of new Set(outs.map((o) => o.outputNodeId))) {
        const gn = graph.nodes.get(outId);
        if (!gn)
            continue;
        for (const edge of gn.inEdges) {
