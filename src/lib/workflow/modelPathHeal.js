...
export async function healVoiceModelPath(voiceType, voiceName, modelPath) {
    if (!voiceName)
        return modelPath;
...
export function msstOnnxPath(modelsDir, filename) {
...
export async function healMsstModelPath(modelFile, modelPath) {
    if (!modelFile)
        return modelPath;
...
export async function healLoadedTrackAvatars(tracks) {
    if (!tracks.some((tr) => tr.voiceModel))
        return;
    const store = useVoiceModelStore.getState();
    ...
    for (const tr of tracks) {
        ...
        if (entry)
            tr.voiceModelAvatar = entry.avatar_path ?? undefined;
    }
}
