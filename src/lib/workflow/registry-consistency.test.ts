...
describe("四张注册表一致性回归测试（防止 Bug #1 lufsAnalyze 类似漏洞再现）", () => {
  ...
  it("rfTypeToWfType 的所有值必须是合法的 WorkflowNodeType", () => {
    ...
    const invalidMappings = rfKeys
      .map(rf => ({ rf, wf: rfTypeToWfType[rf] as string }))
      .filter(({ wf }) => !validWfTypes.has(wf));
    
    expect(
      invalidMappings,
      `rfTypeToWfType 映射到不存在的 WorkflowNodeType: ${invalidMappings.map(m => `${m.rf} -> ${m.wf}`).join(", ")}`
    ).toEqual([]);
  });
  ...
  it("双向映射必须可逆（wf -> rf -> wf 往返不变）", () => {
    ...
    expect(
      broken,
      `双向映射破损: ${broken.map(b => `${b.wf} -> ${b.rf} -> ${b.back}`).join(", ")}`
    ).toEqual([]);
  });

  it("特定分析节点必须同时出现在四张表（防止 lufsAnalyze 类似遗漏）", () => {
    const criticalNodes: WorkflowNodeType[] = [
      "lufsAnalyze",
      "spectrogram",
      "f0Curve",
      "timbreMetrics",
  ...
  it("NODE_PORTS 声明的节点都应该能在 wfTypeToRfType 找到（除 input/output）", () => {
    ...
    expect(
      unmapped,
      `这些节点有端口和损伤表但缺 rfTypes 映射: ${unmapped.join(", ")}`
    ).toEqual([]);
  });
});
