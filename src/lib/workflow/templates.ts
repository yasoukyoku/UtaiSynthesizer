...
export type WizardTemplateId =
  | "transcribe" | "clone" | "original" | "rebuild" | "repaint"
  // P2-14 歌曲制作模板（规划 11.6）
  | "lyrics2song" | "segRepaint" | "vocal2acc" | "stemsRebuild";
...
export interface WizardTemplateContext {
  ...
  amtBackend?: string;
  /** 规划 12.2 路线 B：伴奏变速系数（0.9~1.1；1 = 不变速，speedShift 节点省略）。 */
  speedFactor?: number;
}
...
export function buildRebuildWorkflow(ctx: WizardTemplateContext): Workflow {
...
export function buildRepaintWorkflow(ctx: WizardTemplateContext): Workflow {
  const { vocalPort, instPort } = resolveVocalPorts(ctx.vocalStemNames);
  const vm = ctx.voiceModel;
  const semis = ctx.transposeSemitones ?? 2;
...
export const templateNeedsVoiceModel = (t: WizardTemplateId): boolean =>
  t === "clone" || t === "original" || t === "rebuild" || t === "repaint";

/** 模板是否需要 AMT(audio→MIDI) 转谱后端:只有图里含 amtMidi 节点的模板才检查
 *  (扒带/深度原创/原创重建);歌曲模板与纯换声模板不因缺 AMT 而被拦(规划 6-6)。 */
export const templateNeedsAmt = (t: WizardTemplateId): boolean =>
  t === "transcribe" || t === "original" || t === "rebuild";
...
export function buildLyrics2SongWorkflow(_ctx: WizardTemplateContext): Workflow {
  return {
    nodes: [
...
export function buildSegRepaintWorkflow(_ctx: WizardTemplateContext): Workflow {
  return {
    nodes: [
      node("input", "input", 0, 0, {}),
      node("sty", "songPrompt", 0, 180, {}),
...
export function buildVocal2AccWorkflow(_ctx: WizardTemplateContext): Workflow {
  return {
    nodes: [
      node("input", "input", 0, 0, {}),
...
export function buildStemsRebuildWorkflow(_ctx: WizardTemplateContext): Workflow {
  return {
    nodes: [
      node("input", "input", 0, 0, {}),
