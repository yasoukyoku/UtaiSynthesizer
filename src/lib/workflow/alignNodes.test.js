...
import { alignNodes, distributeNodes, MIN_ALIGN_NODES, MIN_DISTRIBUTE_NODES, } from "./alignNodes";
function n(id, x, y, size) {
...
describe("alignNodes", () => {
    ...
    it("aligns right by trailing edge, honouring per-node widths", () => {
        ...
        const r = alignNodes([n("a", 0, 0, { width: 100, height: 50 }), n("b", 40, 60, { width: 300, height: 50 })], "right");
    ...
    it("centers horizontally on the selection bounding box", () => {
        ...
        const r = alignNodes([n("a", 0, 0, { width: 300, height: 50 }), n("b", 0, 60, { width: 100, height: 50 })], "hcenter");
    ...
    it("ignores a zero or negative measured size", () => {
        const r = alignNodes([n("a", 0, 0, { width: 0, height: 0 }), n("b", 100, 0, { width: 240, height: 120 })], "right");
        expect(r.positions.get("a")?.x).toBe(100);
    });
    it("is idempotent", () => {
    ...
    it("handles every align mode without throwing", () => {
        const modes = ["left", "right", "hcenter", "top", "bottom", "vcenter"];
