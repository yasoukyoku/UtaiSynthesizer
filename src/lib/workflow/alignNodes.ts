...
function finalize(nodes: LayoutNodeInput[], next: Map<string, { x: number; y: number }>): AlignResult {
 ...
 */
export function alignNodes(nodes: LayoutNodeInput[], mode: AlignMode): AlignResult {
  if (nodes.length < MIN_ALIGN_NODES) return { positions: new Map(), moved: 0 };
 ...
 */
export function distributeNodes(nodes: LayoutNodeInput[], mode: DistributeMode): AlignResult {
  if (nodes.length < MIN_DISTRIBUTE_NODES) return { positions: new Map(), moved: 0 };
  ...
  const sizeOn = (n: LayoutNodeInput) => (horizontal ? sizeOf(n).w : sizeOf(n).h);
  const startOf = (n: LayoutNodeInput) => (horizontal ? n.position.x : n.position.y);
