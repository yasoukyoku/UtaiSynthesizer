...
export interface AutoLayoutResult {
  ...
  /** 位置真的变了的节点数。0 = 图已经是整理后的样子,调用方可以据此跳过写入,不脏一步撤销。 */
  moved: number;
}
...
function sizeOf(n: LayoutNodeInput): { w: number; h: number } {
...
function normalizeEdges(ids: Set<string>, edges: LayoutEdgeInput[]): LayoutEdgeInput[] {
...
function assignLayers(
  ...
  for (const id of order) if (!settled.has(id)) topo.push(id);
  ...
  for (const id of topo) {
    ...
    for (const p of preds.get(id) ?? []) {
      ...
      if (pl !== undefined && pl + 1 > lv) lv = pl + 1;
    }
    layer.set(id, lv);
  }
  return layer;
...
function findComponents(ids: string[], preds: Map<string, string[]>, succs: Map<string, string[]>): string[][] {
  const seen = new Set<string>();
  const comps: string[][] = [];
...
export function computeAutoLayout(
  ...
  if (nodes.length === 0) return { positions, moved: 0 };
  ...
  for (const n of nodes) {
    ...
    const { w } = sizeOf(n);
    if (w > (colWidth.get(lv) ?? 0)) colWidth.set(lv, w);
  }
  ...
  const refY = (id: string): number => {
    const ps = preds.get(id) ?? [];
    if (ps.length === 0) return byId.get(id)?.position.y ?? 0;
    let sum = 0;
    let cnt = 0;
    for (const p of ps) {
  ...
  for (const comp of findComponents(ids, preds, succs)) {
    ...
    for (const [lv, bucket] of rows) {
      ...
      if (y - ROW_GAP > compBottom) compBottom = y - ROW_GAP;
    }
  ...
  for (const n of nodes) {
    ...
    if (!p) continue;
    if (Math.abs(p.x - n.position.x) > MOVE_EPSILON || Math.abs(p.y - n.position.y) > MOVE_EPSILON) moved += 1;
  }
