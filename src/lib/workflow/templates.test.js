...
import { buildWorkflow, buildTranscribeWorkflow, buildCloneWorkflow, buildOriginalWorkflow, resolveVocalPorts, stemCount, templateNeedsSeparation, templateNeedsVoiceModel, templateStemCount, } from "./templates";
...
describe("buildWorkflow", () => {
    ...
    it("三个模板都能被引擎的图解析器接受（无环/连通）", () => {
        for (const t of ["transcribe", "clone", "original"]) {
    ...
    it("P2-14 歌曲模板：结构正确且能被图解析器接受", () => {
        ...
        // 四个模板都能被图解析器接受（无环/连通）
        for (const t of ["lyrics2song", "segRepaint", "vocal2acc", "stemsRebuild"]) {
