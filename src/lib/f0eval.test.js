...
describe("interpolateShape", () => {
    it("easings hit boundaries + known midpoints, and clamp out-of-range t", () => {
        for (const s of ["linear", "sineIn", "sineOut", "sineInOut"]) {
...
describe("evalF0Cents (SynthV-aligned layering §10.3)", () => {
    const mk = (id, tick, dur, pitch, extra = {}) => ({ id, tick, duration: dur, pitch, lyric: "あ", velocity: 100, ...extra });
    ...
    const flat = { offsetMs: 0, durLeftMs: 0, durRightMs: 0, depthLeftCents: 0, depthRightCents: 0, openEdgeCents: 0 };
    const smooth = { offsetMs: 0, durLeftMs: 100, durRightMs: 70, depthLeftCents: 15, depthRightCents: 15, openEdgeCents: 0 };
    ...
    const scoop = { offsetMs: 0, durLeftMs: 100, durRightMs: 100, depthLeftCents: 15, depthRightCents: 15, openEdgeCents: 100 };
    ...
    it("same-pitch abutting notes have NO transition wobble (flat through the seam — no phantom bump)", () => {
        ...
        for (const at of [430, 480, 540])
            expect(evalF0CentsAt(same, undefined, at, optsSmooth).cents).toBeCloseTo(6000, 6);
    });
