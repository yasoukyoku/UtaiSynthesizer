...
export function resumeLockedFields(backend) {
    ...
    if (backend === "sovits_diff")
        out.push({ id: "kStepMax", tier: "locked", scope: "run" });
 ...
 */
export function resumeWouldBeGuarded(backend, info) {
    if (!info || !info.exists)
        return false;
    if (backend === "sovits_diff")
        return info.diff_steps > 0;
...
export function lockedFieldIds(backend, tier) {
    return new Set(resumeLockedFields(backend)
...
export function poolInvalidatingIds(backend) {
    return new Set(resumeLockedFields(backend)
