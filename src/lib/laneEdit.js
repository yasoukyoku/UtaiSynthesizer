...
import { sliceClips, deleteClip, isWholeClips, ticksToMs, clipIndexAtMs, materializeClips, laneGroupId, segStretch, } from "./audio/laneOps";
/**
 ...
 */
function findSeg(trackId, segmentId) {
...
function resyncSelectedClip(trackId, segmentId, outputNodeId, stemDurMs, anchorMs) {
    const lane = useAppStore.getState().selectedLane;
    if (!lane || lane.trackId !== trackId || lane.segmentId !== segmentId || lane.outputNodeId !== outputNodeId)
        return;
...
export function sliceLaneGroupAtPlayhead(trackId, segmentId, outputNodeId) {
    const st = useProjectStore.getState();
    const seg = findSeg(trackId, segmentId);
    if (!seg || seg.content.type !== "audioClip")
        return;
    const ph = st.playheadTick;
    if (ph <= seg.startTick || ph >= seg.startTick + seg.durationTicks)
        return;
...
export function deleteLanePiece(trackId, segmentId, outputNodeId, clipIndex) {
    const seg = findSeg(trackId, segmentId);
    if (!seg || seg.content.type !== "audioClip")
        return;
    const stemDurMs = seg.content.totalDurationMs;
...
export function liveSelectedLane() {
    const lane = useAppStore.getState().selectedLane;
    if (!lane)
        return null;
    ...
    if (!live || !seg || seg.content.type !== "audioClip") {
        if (lane)
            useAppStore.setState({ selectedLane: null });
        return null;
    }
