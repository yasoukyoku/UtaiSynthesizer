...
export function resolveSegmentSourceAudio(seg) {
    if (!seg)
        return {};
    let path;
    let name;
...
export function resolveTrackSourceAudio(track) {
    if (!track)
        return {};
    ...
    for (const seg of segs) {
        ...
        for (const o of outs) {
            ...
            if (!best || durMs > best.durMs)
                best = { path: p, name: o?.laneLabel || o?.group, durMs };
        }
    }
    if (best)
        return { path: best.path, name: best.name || track.name };
    // 3. any content-level audio path (e.g. vocal renders), longest-duration first.
    for (const seg of segs) {
        const r = resolveSegmentSourceAudio(seg);
        if (r.path)
            return { path: r.path, name: r.name || track.name };
    }
 ...
 */
export function amtInstrumentKey(program, channel) {
    if (channel === 9)
        return "drums";
 ...
 */
export function matchInstrumentWav(wavs, trackName, program, channel) {
    if (!wavs)
        return undefined;
    const key = amtInstrumentKey(program, channel);
    if (key && wavs[key])
        return wavs[key];
    if (trackName && wavs[trackName])
        return wavs[trackName];
    const lower = (trackName || "").toLowerCase();
    if (lower) {
        const direct = Object.entries(wavs).find(([k]) => k.toLowerCase() === lower);
        if (direct)
            return direct[1];
        if (lower.includes("drum") && wavs["drums"])
            return wavs["drums"];
    }
    return undefined;
}
