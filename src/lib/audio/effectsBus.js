...
const trackAnalysers = new WeakMap();
...
function getFxChain(ctx, cfg) {
    ...
    const chain = { dry, masterGain, analyser, reverbInput, delayInput, delayNode, delayFb, delayReturn, reverbReturn, cfgKey: key };
 ...
 * to pre-bus projects.
 */
export function connectTrackOutput(ctx, panner, sends, cfg, 
...
export function roundSend(v) {
    return Math.round(Math.max(0, Math.min(1, v)) * 100) / 100;
}
// ── 全局总线参数(单例)。UI 经 setFxBusConfig 调整并持久化;playback/export 均在建图时读取
//   同一份,保证「听导一致」。 ──
let fxBusConfig = { ...DEFAULT_FX_BUS };
/** 当前总线参数——播放与导出的共同快照。 */
export function getFxBusConfig() {
    return fxBusConfig;
}
/** 更新总线参数(带钳制)。调用方负责持久化。 */
export function setFxBusConfig(c) {
    fxBusConfig = {
