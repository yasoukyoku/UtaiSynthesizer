...
export function canonStretch(r) {
    return Math.round(r * 1e6) / 1e6;
}
const resolved = new Map(); // `${path}::${r}` → artifact path
...
export function stretchedArtifactPaths() {
...
export function stretchInFlight() {
...
export async function applySegmentStretch(trackId, segmentId, rNew) {
    ...
    if (!seg || seg.content.type !== "audioClip" || seg.loading)
        return;
    const c = seg.content;
    if (Math.abs(factor - (c.stretch ?? 1)) < 1e-9)
        return;
    ...
    if (`${getLoadEpoch()}:${c2.sourcePath}:${c2.offsetMs}:${seg2.durationTicks}:${c2.stretch ?? 1}` !== winSig)
        return;
...
export async function ensureStretched(path, r) {
    ...
    if (!p) {
        p = invoke("stretch_segment_audio", { path, timeFactor: factor }).then((res) => {
