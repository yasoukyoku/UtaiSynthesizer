...
export async function detectSegmentTempo(trackId, segmentId, beatsPerBar) {
    if (inFlight.has(segmentId))
        return;
    const p = useProjectStore.getState();
    const seg = p.tracks.find((t) => t.id === trackId)?.segments.find((s) => s.id === segmentId);
    if (!seg || seg.content.type !== "audioClip" || seg.loading)
        return;
    const c = seg.content;
    ...
    try {
        ...
        if (`${getLoadEpoch()}:${c2.sourcePath}:${c2.offsetMs}:${seg2.durationTicks}:${c2.stretch ?? 1}:${p2.tempo}` !== winSig)
            return;
        p2.setSegmentTempoDetect(trackId, segmentId, {
            ...
            conf: res.confidence,
            ...(res.not_constant ? { notConstant: true } : {}),
        });
        if (res.not_constant)
            useAppStore.getState().showToast(i18n.t("tempo.notConstant"), "info");
    }
    catch (e) {
...
export function doubleTempoDetect(td, beatsPerBar) {
    const bpm = td.bpm * 2;
...
export function halveTempoDetect(td, beatsPerBar) {
    const bpm = td.bpm / 2;
...
export function nudgeDownbeat(td, beatsPerBar) {
    const bpb = Math.max(1, beatsPerBar);
...
export function tempoErrorMessage(e) {
    const msg = e instanceof Error ? e.message : String(e);
    if (msg.includes("TEMPO_TOO_SHORT"))
        return i18n.t("tempo.errTooShort");
    if (msg.includes("TEMPO_NO_BEAT"))
        return i18n.t("tempo.errNoBeat");
    if (msg.includes("STRETCH_INPUT_MISSING"))
        return i18n.t("tempo.errStretchMissing");
    if (msg.includes("STRETCH_"))
        return `${i18n.t("tempo.errStretchFailed")}: ${msg}`;
