...
export function ticksToMs(ticks, tempo) {
...
export function flooredDurationTicks(ms, tempo) {
...
export function segStretch(seg) {
...
export function segmentSourceWindowMs(seg, tempo) {
...
export function laneGroupId(out) {
...
export function laneLabelParts(label) {
...
export function laneGroupName(out) {
...
export function laneRowKey(out) {
...
export function rowKeyLaneId(rowKey) {
...
export function materializeClips(stored, stemDurMs) {
    if (stored === undefined)
        return [{ start: 0, end: Math.max(0, stemDurMs) }];
...
export function laneVisiblePieces(seg, stored, stemDurMs, tempo, bakeOffsetMs = 0) {
...
export function clipIndexAtMs(stored, stemDurMs, ms) {
    const clips = materializeClips(stored, stemDurMs);
    if (clips.length === 0)
        return -1;
    for (let i = 0; i < clips.length; i++) {
        if (ms >= clips[i].start && ms <= clips[i].end)
            return i;
    }
    let best = 0;
...
export function sliceClips(stored, stemDurMs, cutMs) {
    const clips = materializeClips(stored, stemDurMs);
    const out = [];
    for (const c of clips) {
...
export function trimClip(stored, stemDurMs, index, edge, newMs, winStartMs, winEndMs) {
    const clips = materializeClips(stored, stemDurMs);
    const c = clips[index];
    if (!c)
        return clips;
    if (edge === "start") {
        ...
        if (index === 0 && v <= winStartMs + EPS_MS)
            v = 0; // un-trimmed front → follow the window again
        c.start = v;
    }
    else {
        const hi = index < clips.length - 1 ? clips[index + 1].start : winEndMs;
        let v = Math.max(Math.min(newMs, hi), c.start + EPS_MS);
        if (index === clips.length - 1 && v >= winEndMs - EPS_MS)
            v = stemDurMs; // un-trimmed end → follow window
        c.end = v;
    }
    return clips;
}
/** Delete clip `index` → a silent gap there. Deleting the only clip yields `[]` (whole lane silent). */
export function deleteClip(stored, stemDurMs, index) {
    const clips = materializeClips(stored, stemDurMs);
...
export function isWholeClips(clips, stemDurMs) {
...
export function laneReachesSeam(seg, rowKey, tempo, edge) {
    ...
    if (edge === "start")
        return pieces.some((p) => Math.abs(p.startTick - seg.startTick) < 1);
...
export function recordDetachLineage(newNodeId, oldNodeId) {
...
export function resolveDetachAncestor(nodeId, snapSeq, has) {
    const seen = new Set([nodeId]);
    let entry = detachLineage.get(nodeId);
    while (entry && !seen.has(entry.oldId)) {
        if (snapSeq > entry.seq)
            return null;
        if (has(entry.oldId))
            return entry.oldId;
...
export function laneOpsSig(laneOps) {
    if (!laneOps)
        return "";
