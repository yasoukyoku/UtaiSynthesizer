...
class Biquad {
  ...
  static lowShelf(sr: number, f0: number, gainDb: number, s = 1): Biquad {
  ...
  static peaking(sr: number, f0: number, q: number, gainDb: number): Biquad {
    const A = Math.pow(10, gainDb / 40);
    const w0 = (2 * Math.PI * Math.min(f0, sr * 0.49)) / sr;
    const cw = Math.cos(w0);
    const sw = Math.sin(w0);
  ...
  static highShelf(sr: number, f0: number, gainDb: number, s = 1): Biquad {
    const A = Math.pow(10, gainDb / 40);
    const w0 = (2 * Math.PI * Math.min(f0, sr * 0.49)) / sr;
    const cw = Math.cos(w0);
    const sw = Math.sin(w0);
 ...
 */
export function applyMasterChain(
  L: Float32Array,
  R: Float32Array,
  sampleRate: number,
  opts: MasterChainOptions = {},
): number {
  ...
  const envToGainDb = (envLin: number) => {
    if (envLin <= 1e-6) return 0;
    const envDb = 20 * Math.log10(envLin);
    if (envDb <= compThreshDb) return 0;
  ...
  const qIdx: number[] = []; // 单调队列（下标，值从 rect 只读取）
  const qVal: number[] = [];
  let gainLin = 1;
