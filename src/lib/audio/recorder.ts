...
interface RecordingState {
  ...
  start: (trackId: string) => Promise<void>;
  stop: () => Promise<void>;
...
export const useRecording = create<RecordingState>((set, get) => ({
  ...
  stop: async () => {
    ...
    if (!mediaRecorder || !trackId) return;
    ...
    let filePath: string;
    if (isTauri()) {
      filePath = await invoke<string>("save_recording", { data: Array.from(new Uint8Array(wav)) });
    } else {
  ...
  toggle: async (trackId) => {
    if (get().recordingTrackId === trackId) {
      ...
      if (get().recordingTrackId) await get().stop();
