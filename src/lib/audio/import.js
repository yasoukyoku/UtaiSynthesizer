...
export async function probeAudioDuration(filePath) {
    try {
        const ms = await invoke("probe_audio_duration", { path: filePath });
 ...
 */
export async function importAudioToNewTrack(filePath, startTick, knownDurationMs, insertIndex) {
 ...
 */
export async function importAudioToExistingTrack(filePath, trackId, startTick, knownDurationMs) {
    const durMs = knownDurationMs ?? (await probeAudioDuration(filePath));
 ...
 */
async function finalizeSegment(filePath, trackId, segId, removeTrackOnFail) {
    ...
    try {
        ...
        useHistoryStore.getState().runSilent(() => updateTrack(trackId, {
            segments: track.segments.map((s) => s.id === segId
                ? {
                    ...s,
                    durationTicks,
                    loading: false,
                    content: s.content.type === "audioClip"
