...
import { applyFadeIn, applyFadeOut, clampPan, dbToLinear, loudnessEnvNode, ticksToSeconds, } from "./playback";
...
function playableSegments(track) {
 ...
 */
export async function renderMixdown(tracks, audioFiles, tempo, sampleRate, onProgress, options) {
    const hasSolo = tracks.some((t) => t.solo);
    const trackAudible = (t) => !t.muted && (!hasSolo || t.solo);
    ...
    for (const track of tracks) {
        ...
        for (const seg of track.segments) {
            if (seg.loading)
                throw new Error("EXPORT_SOURCE_LOADING");
    ...
    if (durationSec > 3600)
        throw new Error("EXPORT_TOO_LONG");
    ...
    let stretchQueue = Promise.resolve();
    const stretchSeq = (path, r) => {
    ...
    for (const track of tracks) {
        if (!trackAudible(track))
            continue; // delta #4 — constant-0 gain, no contribution
        for (const seg of playableSegments(track)) {
            ...
            if (segmentPlaysLanes(track, seg)) {
                for (const out of seg.processedOutputs ?? []) {
                    if (out.loading || isLaneRowMuted(track, laneRowKey(out), out.laneId))
                        continue;
                ...
                if (meta)
                    want(meta.playbackPath || seg.content.sourcePath, r);
            }
        }
    }
        ...
        for (let si = 0; si < sorted.length; si++) {
            ...
            if (segmentPlaysLanes(track, seg)) {
                for (const out of seg.processedOutputs) {
                    ...
                    if (isLaneRowMuted(track, rowKey, out.laneId))
                        continue; // delta #4 (lane row)
                    ...
                    if (!buf)
                        throw new Error(`EXPORT_DECODE_FAIL: ${out.audioPath}`);
            ...
            if (seg.content.type !== "audioClip")
                continue; // defensive mirror of playback.ts
            const meta = audioFiles[seg.content.sourcePath];
            if (!meta)
                throw new Error(`EXPORT_SOURCE_MISSING: ${seg.content.sourcePath}`);
            ...
            if (!buf)
                throw new Error(`EXPORT_DECODE_FAIL: ${srcPath}`);
