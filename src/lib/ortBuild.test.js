...
import { restartWouldChangeOrtBuild } from "./ortBuild";
const importFs = () => {
    const spec = "node:fs";
...
const hint = (o) => restartWouldChangeOrtBuild({ ...base, ...o });
describe("restartWouldChangeOrtBuild", () => {
    it("★the S116 regression: a card our CUDA package cannot run is never promised a restart", () => {
        ...
        expect(hint({ device: "auto", autoVendor: "nvidia", ortBuild: "DirectML", cudaSupported: false, cudaReady: true }), "Auto + NVIDIA pick on an unsupported card must NOT promise a restart").toBe(false);
        expect(hint({ device: "cuda", ortBuild: "DirectML", cudaSupported: false, cudaReady: true }), "the explicit-CUDA leg has the same false promise and must be closed too").toBe(false);
    });
...
describe("buildMismatch ↔ init_ort_runtime 对拍", () => {
    ...
    it("the build gate keys on cuda_pkg_supported — the term `cudaSupported` mirrors", async () => {
        expect(await gateBody(), "the ORT build gate stopped using cuda_pkg_supported; lib/ortBuild.ts mirrors it and must be re-derived").toContain("cuda_pkg_supported");
    });
    ...
    it("the Auto arm requires the provider dependency set — the term `cudaReady` covers", async () => {
        expect(await gateBody(), "cuda_provider_deps_resolvable left the Auto arm; is_cuda_runtime_ready is no longer a superset of it").toContain("cuda_provider_deps_resolvable");
    });
