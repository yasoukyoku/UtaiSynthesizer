...
export const tickToX = (tick, v) => tick * v.ppt - v.scrollX;
export const xToTick = (x, v) => (x + v.scrollX) / v.ppt;
// ── part-local ↔ absolute (the ONLY 2 adapters; note ticks are segment-relative, the grid is absolute) ──
export const noteTickToX = (relTick, partStartTick, v) => tickToX(relTick + partStartTick, v);
export const xToNoteTick = (x, partStartTick, v) => xToTick(x, v) - partStartTick;
...
export const pitchToY = (pitch, v) => (v.top ?? 0) + (V_PITCH_MAX - pitch) * v.rowH - v.scrollY;
/** The whole-semitone pitch whose row contains screen `y` (clamped to MIDI range). */
export const yToPitch = (y, v) => Math.min(V_PITCH_MAX, Math.max(V_PITCH_MIN, V_PITCH_MAX - Math.floor((y - (v.top ?? 0) + v.scrollY) / v.rowH)));
...
export const centsToY = (absCents, v) => (v.top ?? 0) + (V_PITCH_MAX + 0.5 - absCents / 100) * v.rowH - v.scrollY;
/** Inverse of centsToY — the continuous absolute-cents value at screen `y` (for dragging a pitch point). */
export const yToCents = (y, v) => (V_PITCH_MAX + 0.5 - (y - (v.top ?? 0) + v.scrollY) / v.rowH) * 100;
/** Total content height of all 128 rows (for vertical scroll clamps). */
export const rowsContentHeight = (rowH) => (V_PITCH_MAX - V_PITCH_MIN + 1) * rowH;
...
export const paramToY = (value, min, max, laneTop, laneH) => {
...
export const yToParam = (y, min, max, laneTop, laneH) => {
...
export const snapFloor = (relTick, snapTicks) => snapTicks > 0 ? Math.floor(relTick / snapTicks) * snapTicks : Math.round(relTick);
export const snapRound = (relTick, snapTicks) => snapTicks > 0 ? Math.round(relTick / snapTicks) * snapTicks : Math.round(relTick);
/**
...
export const snapPlaceTick = (relTick, partStart, unit, on) => on ? snapFloor(partStart + relTick, unit) - partStart : Math.round(relTick);
/** Adjustment (moving / resizing an EXISTING note, or dragging a new note's end): ROUND to the nearest line. */
export const snapEdgeTick = (relTick, partStart, unit, on) => on ? snapRound(partStart + relTick, unit) - partStart : Math.round(relTick);
/**
 ...
 */
export const snapMoveDelta = (anchorRel, cursorDelta, partStart, unit, on) => snapEdgeTick(anchorRel + cursorDelta, partStart, unit, on) - anchorRel;
/**
 ...
 */
export const resizeEndTick = (origTick, origDuration, cursorRel, grabRel, partStart, unit, minLen, on) => Math.max(origTick + minLen, on ? snapEdgeTick(cursorRel, partStart, unit, true) : origTick + origDuration + Math.round(cursorRel - grabRel));
...
export const isBlackKey = (pitch) => BLACK.has(((pitch % 12) + 12) % 12);
/** Scientific pitch name, C4 = 60 (e.g. 60→"C4", 0→"C-1", 127→"G9"). */
export const pitchName = (pitch) => `${NAMES[((pitch % 12) + 12) % 12]}${Math.floor(pitch / 12) - 1}`;
/** MIDI pitch → Hz (A4=440), for the light WebAudio preview oscillator (§9.7). */
export const pitchToHz = (pitch) => 440 * Math.pow(2, (pitch - 69) / 12);
/** Absolute-cents → Hz (continuous, for the f0-line preview). */
export const centsToHz = (absCents) => 440 * Math.pow(2, (absCents - 6900) / 1200);
