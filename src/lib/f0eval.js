...
export function evalCurveAt(c, x) {
    if (!c || c.xs.length === 0)
        return 0;
    const { xs, ys } = c;
    const n = xs.length;
...
function findNoteAt(notes, relTick) {
    let found = -1;
    for (let i = 0; i < notes.length; i++) {
        const n = notes[i];
        if (relTick >= n.tick && relTick < n.tick + n.duration)
            found = i;
    }
    return found;
}
/** Note's WRITTEN base pitch in cents (tone + fine detune). */
const noteCents = (n) => n.pitch * 100 + (n.detune ?? 0);
...
export function effTransition(n, def) {
...
const arriveBump = (s) => Math.sin(Math.PI * s * s); // peak ≈ s=0.707 (toward B)
const departBump = (s) => Math.sin(Math.PI * (1 - s) * (1 - s)); // peak ≈ s=0.293 (toward A)
/**
 ...
 */
function crossCents(A, B, relTick, opts) {
    ...
    const span = t1 - t0;
    if (span <= 0 || relTick < t0 || relTick > t1)
        return null; // durLeft=durRight=0 → pure staircase (parity)
 ...
 */
function leadCents(note, relTick, opts) {
    ...
    if (durL <= 0 || t.openEdgeCents <= 0)
        return null;
    const t1 = note.tick + durL;
    if (relTick < note.tick || relTick > t1)
        return null;
...
function evalVibrato(note, noteRel, tempo) {
    const v = note.vibrato;
    if (v.depthCents <= 0 || v.freqHz <= 0)
        return 0;
    ...
    if (inSpan < 0 || activeTicks <= 0)
        return 0; // before onset delay / no room
    ...
    let env = 1;
    if (easeIn > 0 && inSpan < easeIn)
        env = inSpan / easeIn; // fade-in
    const remain = activeTicks - inSpan; // ticks to the note end
    if (easeOut > 0 && remain < easeOut)
        env = Math.min(env, remain / easeOut); // fade-out
 ...
 */
export function evalF0CentsAt(notes, pitchDev, relTick, opts) {
    ...
    let cents = prevAbut ? crossCents(notes[idx - 1], note, relTick, opts) : leadCents(note, relTick, opts);
    if (cents === null)
        cents = nextAbut ? crossCents(note, notes[idx + 1], relTick, opts) : releaseCents(note, relTick, opts);
    if (cents === null)
        cents = noteCents(note); // ① stable staircase
    cents += dev; // ③
    if (note.vibrato)
        cents += evalVibrato(note, relTick - note.tick, opts.tempo); // ④
 ...
 */
export function evalF0CentsFrames(notes, pitchDev, frame, opts) {
 ...
 */
export function paintedDev(notes, paint, base, opts) {
 ...
 */
export function paintedParamCurve(paint, base) {
    const map = new Map();
    for (let i = 0; i < paint.xs.length; i++)
        map.set(Math.round(paint.xs[i]), paint.ys[i]);
    return mergePaintedInterval(map, base);
}
/**
 ...
 */
export function sliceCurveAtTick(curve, boundary) {
    ...
    for (let i = 0; i < curve.xs.length; i++) {
        ...
        if (x < boundary) {
            ...
            rxs.push(0);
            rys.push(y);
        } // exactly on the seam → both halves
    }
    ...
    if (!lxs.length || lxs[lxs.length - 1] !== boundary) {
        lxs.push(boundary);
        lys.push(bVal);
    }
    if (!rxs.length || rxs[0] !== 0) {
        rxs.unshift(0);
        rys.unshift(bVal);
    }
