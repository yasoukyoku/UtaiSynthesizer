...
import { t18 } from "./msst-catalog";
export const SONG_FAMILY_LABELS = {
...
export function songModelTotalSize(model) {
...
export function getSongInstallStatus(installedFiles, model) {
    ...
    for (const f of model.files) {
        ...
        if (!LISTED_EXTS.includes(ext))
            continue; // 不可见的文件不参与校验
        ...
        if (listed.has(prefix + f.path) || hasAlt)
            ready += 1;
        else
            missing.push(f.path);
    }
...
export function songModelById(id) {
    return SONG_MODEL_CATALOG.find((m) => m.id === id);
}
export function songCatalogLabel(model, lang) {
