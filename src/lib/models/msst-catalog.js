...
export const MSST_DEFAULT_NUM_OVERLAP = {
    bs_roformer: 4,
    mel_band_roformer: 4,
    mdx23c: 4,
    htdemucs: 2,
...
export const MSST_DEFAULT_PRECISION = {
    bs_roformer: "fp32",
...
export const MSST_FP16_ARCHS = new Set(["bs_roformer", "mel_band_roformer", "mdx23c", "htdemucs"]);
...
export function applyMirror(url, mirror) {
    if (mirror.type === "huggingface" || !url.includes("huggingface.co"))
        return url;
    if (mirror.type === "hf-mirror")
        return url.replace("https://huggingface.co", "https://hf-mirror.com");
    if (mirror.type === "custom" && mirror.customUrl)
        return url.replace("https://huggingface.co", mirror.customUrl);
    return url;
}
export const DEFAULT_MIRROR = { type: "huggingface", customUrl: "" };
...
export function hfBaseForMirror(mirror) {
    if (mirror.type === "hf-mirror")
        return "https://hf-mirror.com";
    if (mirror.type === "custom" && mirror.customUrl.trim())
        return mirror.customUrl.trim();
    return null;
}
 ...
 *  mainland testers). */
export const BUILTIN_GH_PRESETS = [
...
export const DEFAULT_GH_MIRROR = { type: "direct", customUrl: "" };
...
export function migrateGhMirror(gh) {
    ...
    if (g.type === "ghfast")
        return { type: "preset", presetId: "ghfast.top", customUrl };
    if (g.type === "ghproxy")
        return { type: "preset", presetId: "gh-proxy.com", customUrl };
...
export function ghProxyPrefix(gh, presets = BUILTIN_GH_PRESETS) {
    if (gh.type === "preset") {
...
export function applyGhMirror(url, gh, presets = BUILTIN_GH_PRESETS) {
    const prefix = ghProxyPrefix(gh, presets);
    if (!prefix || !isGhUrl(url))
        return url;
    return `${prefix}/${url}`;
}
...
export function ghRouteOrder(gh, presets = BUILTIN_GH_PRESETS) {
    const out = [];
...
export function ghTrustedRoutes(gh, presets = BUILTIN_GH_PRESETS) {
    const chosen = ghProxyPrefix(gh, presets);
...
export function ghMirrorCandidates(url, gh, presets = BUILTIN_GH_PRESETS, presetFallbacks = false) {
    if (!isGhUrl(url))
        return [url];
    const out = [];
    const push = (u) => {
