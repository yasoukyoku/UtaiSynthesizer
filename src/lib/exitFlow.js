...
async function getRunningTasks() {
    ...
    let rustTasks = [];
    try {
        rustTasks = await invoke("running_tasks"); // training / separation / cuda_download / convert
    }
    catch {
        /* best-effort guard */
    }
    ...
    if (wfRunning && !rustTasks.includes("separation"))
        tasks.push(i18n.t("close.task_workflow"));
    for (const id of rustTasks)
        tasks.push(i18n.t(`close.task_${id}`));
    if (Object.keys(useMsstModelStore.getState().downloading).length > 0)
        tasks.push(i18n.t("close.task_download"));
    return tasks;
}
...
export async function runExitFlow(mode = "quit") {
    if (useAppStore.getState().confirm)
        return; // a dialog is already open (recovery / another close) — don't stack
    ...
    try {
        ...
        if (hasUnsavedWork()) {
            ...
            if (c === "save") {
                ...
                if (!ok)
                    return; // save cancelled/failed → abort quit
            }
            else if (c !== "dontSave") {
