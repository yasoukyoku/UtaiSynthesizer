...
setDictionarySig("d0");
import { clearClipboard, clipboardKind, copySelectedSegments, copyTrackToClipboard, cutSelectedSegments, pasteClipboard, } from "./clipboard";
import { isVocalDirty, vocalRenderSig, VOCAL_LANE_ID } from "./vocal/vocalRender";
function audioSeg(id, startTick, durationTicks = 960) {
    return {
        id, startTick, durationTicks,
        content: { type: "audioClip", sourcePath: `C:/audio/${id}.wav`, offsetMs: 0, totalDurationMs: 2000 },
    };
}
function audioTrack(id, segs) {
    return {
...
function vocalTrack(id, segs, voiceModel) {
    return {
...
function seed(tracks, playhead = 0) {
...
function select(trackId, segmentId) {
    useAppStore.getState().selectSegment(trackId, segmentId);
}
let uninstall = null;
beforeEach(() => {
    clearClipboard();
    useAppStore.setState({ selectedSegment: null, selectedSegments: [], selectedLane: null, activeTrackId: null });
    useAudioStore.setState({ isPlaying: false });
    useWorkflowStore.setState({ executions: {}, nodeStatuses: {}, nodeOutputs: {}, nodeProgress: {}, nodeErrors: {}, renderLinks: {} });
    useVoiceModelStore.setState({ models: { sovits: [{ name: "singerA", path: "C:/m/a.onnx" }, { name: "singerB", path: "C:/m/b.onnx" }], rvc: [] } });
    uninstall?.();
...
describe("S61 clipboard — audio segments", () => {
    ...
    it("C: single-clip paste onto a mismatching track type is a no-op", () => {
        ...
        useAppStore.setState({ selectedSegment: { trackId: "V", segmentId: "x" }, selectedSegments: [] });
...
describe("S61 clipboard — vocal bake dual-sig", () => {
    function bakedVocal(id, voiceModel) {
        const seg = {
    ...
    it("E1: same-config paste keeps the bake CLEAN (windowSig stamped for fresh note ids)", () => {
        ...
        select("V1", "V1-seg");
        copySelectedSegments();
        useAppStore.setState({ selectedSegment: { trackId: "V2", segmentId: "x" }, selectedSegments: [] });
        expect(pasteClipboard()).toBe("ok");
