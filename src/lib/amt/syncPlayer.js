...
export class SynchronizedPlayer {
    constructor() {
        ...
        Object.defineProperty(this, "midiMix", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "origGain", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        });
        Object.defineProperty(this, "midiGain", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: null
        ...
        Object.defineProperty(this, "startCtxTime", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        ...
        Object.defineProperty(this, "endFired", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
    ...
    get duration() {
        let d = 0;
        if (this.original)
            d = Math.max(d, this.original.duration);
        if (this.midiMix)
            d = Math.max(d, this.midiMix.duration);
        for (const b of this.instruments.values())
    ...
    get position() {
        ...
        return Math.min(this.duration, this.startOffset + Math.max(0, ctx.currentTime - this.startCtxTime) * this.rate);
    }
    static async decode(path) {
        const bytes = await readFile(path);
        const ab = bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength);
        return await SynchronizedPlayer.ctx().decodeAudioData(ab);
    }
    ...
    async configure(src) {
        ...
        const jobs = [];
        if (src.originalPath) {
            jobs.push(SynchronizedPlayer.decode(src.originalPath)
                .then((b) => { this.original = b; })
                .catch((e) => { console.warn("[syncPlayer] original decode failed:", e); }));
        }
        if (src.midiMixPath) {
            jobs.push(SynchronizedPlayer.decode(src.midiMixPath)
        ...
        for (const [name, path] of Object.entries(src.instrumentPaths)) {
            if (!path)
                continue;
            jobs.push(SynchronizedPlayer.decode(path)
    ...
    setMixState(state) {
        ...
        if (!this.playing || !this.origGain || !this.midiGain)
            return; // wiring is rebuilt on play anyway
...
Object.defineProperty(SynchronizedPlayer, "_ctx", {
    enumerable: true,
    configurable: true,
    writable: true,
    value: null
