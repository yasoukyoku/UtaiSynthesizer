...
export function analyzeTrackChords(trackId) {
    ...
    useAppStore.getState().showToast(i18n.t("chordTrack.done", { count: res.segments.length, key: res.key.label }), "success");
}
/**
 * 识别该轨音频的鼓点并生成 GM 鼓音符轨。
 * @param audioPath 源音频文件（resolveTrackSourceAudio 解出的可播放路径）。
 */
export async function detectTrackDrums(trackId, audioPath) {
    const proj = useProjectStore.getState();
    const track = proj.tracks.find((t) => t.id === trackId);
    if (!track || !audioPath)
        return;
    try {
        ...
        useAppStore.getState().showToast(i18n.t("chordTrack.drumsDone", { count: notes.length, track: drumTrack.name }), "success");
    }
    catch (e) {
        useAppStore.getState().showToast(i18n.t("chordTrack.drumsFailed", { msg: e instanceof Error ? e.message : String(e) }), "error");
    }
}
