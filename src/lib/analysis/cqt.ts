...
export interface CqtOptions {
  ...
  /** 帧间隔（秒）。默认 0.02。 */
  hopSec?: number;
  /** 每半音的 Q 值（周期数）。默认 1/(2^(1/12)−1) ≈ 16.82（半音分辨率）。 */
  q?: number;
}

/** MIDI 音高 → 频率（Hz）。 */
export function midiPitchToFreq(p: number): number {
 ...
 */
export function analyzeCqt(samples: Float32Array, sampleRate: number, opts: CqtOptions = {}): CqtResult {
  ...
  const magnitudes: Float32Array[] = pitches.map(() => new Float32Array(frameCount));
  const times: number[] = [];
...
export interface PitchSalience {
  time: number;
  /** 每帧的最显著音高（局部最大列中的最大者）。 */
  topPitches: Array<{ pitch: number; magnitude: number }>;
}

/** 从 CQT 结果提取逐帧音高显著性 top-K（NoteDigger 扒谱的候选音符来源）。 */
export function pitchSalience(result: CqtResult, topK = 4): PitchSalience[] {
  const frames = result.times.length;
  const out: PitchSalience[] = [];
  for (let fr = 0; fr < frames; fr++) {
    const cands: Array<{ pitch: number; magnitude: number }> = [];
