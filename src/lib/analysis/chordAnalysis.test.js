...
import { analyzeChords, estimateKey, chordLabel, analyzeChordsFromCqt } from "./chordAnalysis";
import { analyzeCqt } from "./cqt";
const PPQ = 480;
/** 以根音为低音的和弦块：pitch = MIDI 音级 + 八度，持续 durTicks。 */
function chordBlock(startTick, durTicks, pitches, velocity = 100) {
    return pitches.map((p) => ({ tick: startTick, duration: durTicks, pitch: p, velocity }));
}
/** C 大调音阶顺阶音符（时长加权直方图）。 */
function scaleNotes(roots, dur = PPQ) {
...
describe("analyzeChordsFromCqt (FFT/CQT 音频和弦识别)", () => {
    const SR = 44100;
    const sine = (freq, durSec) => {
        const n = Math.floor(SR * durSec);
        const buf = new Float32Array(n);
        for (let i = 0; i < n; i++)
            buf[i] = Math.sin((2 * Math.PI * freq * i) / SR) * 0.6;
        return buf;
    };
    // MIDI pitch → Hz
    const pitchHz = (p) => 440 * Math.pow(2, (p - 69) / 12);
    it("音频输入 → 产生和弦段 + 调性 (结构正确性)", () => {
        // 正弦波叠加大三和弦: 纯音 + 谐波, CQT 能稳定检测到能量集中在 C/E/G 音级附近
        const chordMix = (pitches, durSec = 1.5) => {
    ...
    it("静音输入 → 返回空 segments + Cmaj key", () => {
        const silent = { times: [], magnitudes: [], pitches: [] };
