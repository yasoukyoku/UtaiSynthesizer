...
import { detectDrums, drumHitsToNotes, GM_DRUM_NOTE, type DrumHit } from "./drumDetection";

const SR = 44100;

/** 合成敲击：t 秒处一个 attack 极快的衰减正弦（模拟鼓的瞬态）。 */
function synthHit(buf: Float32Array, atSec: number, freq: number, decaySec = 0.08): void {
...
function near(hits: DrumHit[], type: DrumHit["type"], timeSec: number, tolSec = 0.05): boolean {
...
describe("detectDrums", () => {
  ...
  it("双触发抑制：10Hz 连续低频敲击不会每 50ms 报一次", () => {
    const dur = 2;
    const buf = new Float32Array(SR * dur);
    for (let k = 0; k < 20; k++) synthHit(buf, 0.1 * k, 60, 0.02); // 100ms 间隔
