...
const BARS = 8;
function note(tick, duration, pitch, velocity) {
...
function BAR_END(beat, pitches) {
    if (pitches[0] === 60 && pitches[1] === -2)
        return TICKS_PER_BAR; // 全音符小节
    ...
    for (let bar = 0; bar < BARS; bar++) {
        ...
        for (const half of [0, 960]) {
            for (const p of chord)
                notes.push(note(base + half, 940, p, 70));
        }
    }
    return notes;
}
...
function demoDrums() {
    ...
    for (let bar = 0; bar < BARS; bar++) {
        ...
        for (let beat = 0; beat < 4; beat++) {
            ...
            if (beat % 2 === 0)
                notes.push(note(t, 240, KICK, 104));
            else
                notes.push(note(t, 240, SNARE, 94));
        }
    }
    // 末小节起点叠一枚镲片（4 拍延音）收尾。
