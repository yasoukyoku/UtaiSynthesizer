...
let savedJson = null; // serialized doc as of the last REAL save / open / new (clean baseline)
let recoveryPending = false; // true while the startup recover prompt is open — don't clobber the slot
let current = null; // the in-flight flush chain (single-flight guard + awaitable handle)
...
function buildEnvelope(projectJson) {
    const s = useProjectStore.getState();
    const env = { filePath: s.filePath, name: s.name, savedAt: Date.now(), projectJson };
...
function flush() {
    if (recoveryPending)
        return Promise.resolve(); // recovery decision pending — don't clobber the slot
    if (current) {
...
export async function markAutosaveBaseline() {
...
export function setRecoveryPending(v) {
    recoveryPending = v;
}
...
export async function readAutosave() {
    try {
        const raw = await invoke("read_autosave");
        if (!raw)
            return null;
