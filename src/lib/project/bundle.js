 ...
 */
function serializeProject(name, tracks, tempo, timeSignature, mapPath, stripView = false) {
    const outTracks = tracks.map((t) => {
        ...
        if (!track.playOriginal)
            delete track.playOriginal;
        ...
        if (!track.reverbSend)
            delete track.reverbSend;
        if (!track.delaySend)
            delete track.delaySend;
        ...
        if (track.laneMutes) {
            const kept = Object.fromEntries(Object.entries(track.laneMutes).filter(([k, v]) => v || track.laneControls[rowKeyLaneId(k)]?.muted === true));
            if (Object.keys(kept).length > 0)
                track.laneMutes = kept;
            else
                delete track.laneMutes;
        }
        ...
        if (stripView || !track.loudnessLaneOpen)
            delete track.loudnessLaneOpen;
...
export function buildSaveBundle(name, tracks, tempo, timeSignature) {
    const copies = [];
    ...
    const consolidate = (absPath, subdir) => {
...
export function buildAutosaveJson(name, tracks, tempo, timeSignature) {
...
export function parseLoadedBundle(projectJson, dir) {
    const data = JSON.parse(projectJson);
    const base = dir.replace(/\\/g, "/").replace(/\/+$/, "");
    const resolve = (p) => p.startsWith("media/") || p.startsWith("renders/") ? `${base}/${p}` : p; // else external/absolute
    const tracks = (data.tracks ?? []).map((t) => {
        const segments = (t.segments ?? []).map((s) => {
            ...
            if (rest.workflow?.nodes.some((n) => ["pitchShift", "formantShift", "audioEnhance"].includes(n.nodeType))) {
                rest.workflow = {
                    ...rest.workflow,
                    nodes: rest.workflow.nodes.map((n) => {
                        if (!["pitchShift", "formantShift", "audioEnhance"].includes(n.nodeType))
                            return n;
                        ...
                        return { ...n, nodeType: "transpose", params: { semitones } };
                    }),
                };
            }
            ...
            if (rest.workflow?.nodes.some((n) => ["songGenYue2", "songGenAceStep"].includes(n.nodeType))) {
                rest.workflow = {
                    ...rest.workflow,
                    nodes: rest.workflow.nodes.map((n) => {
                        if (!["songGenYue2", "songGenAceStep"].includes(n.nodeType))
                            return n;
                        return {
                            ...n,
                            nodeType: "songGen",
            ...
            if (content.type === "audioClip") {
                ...
                if (content.stretch !== undefined) {
                    ...
                    if (!Number.isFinite(r) || r <= 0 || Math.abs(r - 1) < 1e-9)
                        delete content.stretch;
                    else
                        content.stretch = Math.min(4, Math.max(0.25, r));
                }
                if (content.tempoDetect !== undefined) {
                    const td = content.tempoDetect;
                    ...
                    if (Number.isFinite(bpm) && bpm >= 20 && bpm <= 400 && Number.isFinite(anchorMs) && anchorMs >= 0) {
                        content.tempoDetect = {
                            ...
                            ...(td?.notConstant === true ? { notConstant: true } : {}),
                        };
                    }
                    else {
            ...
            if (rest.laneLoudness !== undefined) {
                ...
                if (laneLoudness)
                    rest.laneLoudness = laneLoudness;
                else
                    delete rest.laneLoudness;
            }
            let processedOutputs = rest.processedOutputs;
            if (processedOutputs) {
                ...
                const wfEdges = new Map();
                for (const c of rest.workflow?.connections ?? []) {
                    if (!wfNodeGroup.has(c.toNode))
                        continue;
                ...
                processedOutputs = processedOutputs
                    ...
                    if (laneId === undefined && o.outputNodeId) {
                        ...
                        if (edges && edges.length === lanelessCount.get(o.outputNodeId)) {
                            ...
                            const e = edges[n];
                            if (e)
                                laneId = `${o.outputNodeId}::${e.fromNode}:${e.fromPort}`;
                        }
                    }
                    laneId = laneId ?? o.outputNodeId ?? o.laneLabel;
                    if (usedLaneIds.has(laneId)) {
                        let i = 2;
                        while (usedLaneIds.has(`${laneId}#${i}`))
                            i++;
                        laneId = `${laneId}#${i}`;
                    }
        ...
        // (isLaneRowMuted still reads the per-laneId legacy muted flag; laneControlFor prefers the group key).
        {
            ...
            for (const [key, ctrl] of Object.entries(laneControls)) {
                ...
                if (promoted[groupKey])
                    continue; // a real group entry / an earlier promotion wins (first-seen)
        ...
        const track = { ...t, segments, laneControls };
        ...
        if (rs && rs > 0)
            track.reverbSend = Math.round(rs * 100) / 100;
        else
            delete track.reverbSend;
        if (ds && ds > 0)
            track.delaySend = Math.round(ds * 100) / 100;
        else
            delete track.delaySend;
        const vp = sanitizeVocalParams(t.vocalParams);
        if (vp)
            track.vocalParams = vp;
        else
            delete track.vocalParams;
        ...
        if (track.soundfont !== undefined) {
            ...
            if (typeof sfFontId === "string" && sfFontId && typeof sfPresetId === "string" && sfPresetId) {
                ...
                track.soundfont = {
                    ...
                    ...(bk === "fluidsynth" ? { backend: "fluidsynth" } : {}),
                };
            }
            else {
                delete track.soundfont;
            }
        }
        return track;
    });
