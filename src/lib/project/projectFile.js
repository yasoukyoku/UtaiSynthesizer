...
import { buildSaveBundle, parseLoadedBundle } from "./bundle";
...
function loadOriginalPeaks(tracks) {
    const seen = new Set();
...
async function confirmDiscardIfDirty() {
    if (!useProjectStore.getState().dirty && !hasUnsavedWork())
        return true;
...
export async function loadDemoProject() {
    ...
    try {
        ...
        const kindByTrack = ["piano", "chords", "bass", "drums"];
        const assigns = [];
        try {
        ...
        tracks.forEach((tr, i) => { const sf = assigns[i]; if (sf)
            tr.soundfont = sf; });
        useProjectStore.setState({
...
export async function openProjectFromPath(uspPath) {
    if (busy)
        return false;
    busy = true;
    try {
        ...
        const opened = await invoke("open_project_archive", { uspPath });
...
export async function saveProjectFile() {
    ...
    if (!(await invoke("path_exists", { path: fp })))
        return saveProjectFileAs();
    busy = true;
    try {
...
async function writeArchive(uspPath, rename) {
    const s = useProjectStore.getState();
    try {
        const { projectJson, copies } = buildSaveBundle(s.name || "Untitled", s.tracks, s.tempo, s.timeSignature);
        const missing = await invoke("save_project_archive", { uspPath, projectJson, copies });
...
export async function restoreAutosave(env) {
    try {
