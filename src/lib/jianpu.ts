...
const DEGREE_TEXT: readonly string[] = [
...
const clampDots = (n: number): number => Math.max(0, Math.min(3, n));

/**
 * pitch（MIDI 0-127）在主音 tonic（0-11，C=0）下的简谱唱名。
 * 越界的 tonic/异常 pitch 都收敛到安全值（不抛错——绘制路径每帧调用）。
 */
export function jianpuForPitch(pitch: number, tonic: number): JianpuLabel {
