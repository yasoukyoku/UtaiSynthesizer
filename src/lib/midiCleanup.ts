...
export interface CleanableNote {
  tick: number;
  duration: number;
  pitch: number;
  velocity?: number;
  /** 可选音符 id——有它时 smartCleanStems 会记录“被修过且留存”的 id 进 stats.touched（差异高亮用）。 */
  id?: string;
}

export interface CleanableStem<N extends CleanableNote = CleanableNote> {
  notes: N[];
  /** 首个 MIDI 通道（0-15；9 = 鼓）。 */
  channel?: number | null;
}
...
export interface CleanStats {
  ...
  /** v3：拖尾音符被截断的数量。 */
  trimmedHang: number;
  /** v3：起点被吸附到 1/32 网格的数量。 */
  snapped: number;
  /** 被修过且保留下来的音符 id（合并存留、力度修正、补漏新增）——
   *  调用方用它做“修复差异高亮”，让用户看清 AI 动了哪些音符。 */
  touched: string[];
}

export const emptyCleanStats = (): CleanStats => ({
...
export const cleanChangedTotal = (s: CleanStats): number =>
  s.removedShort + s.removedGhost + s.removedKey + s.removedRange +
...
export function detectKeyPitchClasses(all: CleanableStem[]): Set<number> | null {
  const hist = new Array<number>(12).fill(0);
  let count = 0;
  for (const s of all) {
    if (s.channel === 9) continue; // 鼓不携带调性信息
...
function barSimilarity(a: CleanableNote[], b: CleanableNote[]): number {
...
export interface SmartCleanOptions {
  bpm: number;
  ppq: number;
  /** 音乐总长（tick，480-ppq 空间）；补漏小节的边界。 */
  durationTicks: number;
  /** 新音符 id 工厂（补漏小节会新增音符）。默认 crypto.randomUUID。 */
  makeId?: () => string;
}
 ...
 */
export function smartCleanStems<N extends CleanableNote>(
  stems: CleanableStem<N>[],
  opts: SmartCleanOptions,
): CleanStats {
  const stats = emptyCleanStats();
  const safePpq = opts.ppq > 0 ? opts.ppq : 480;
  const safeBpm = opts.bpm > 0 ? opts.bpm : 120;
  const msToTicks = (ms: number) => Math.max(1, Math.round((ms / 1000) * (safeBpm / 60) * safePpq));
  ...
  stems.forEach((s) => {
    ...
    const byPitch = new Map<number, N[]>();
    for (const n of s.notes) {
    ...
    for (const list of byPitch.values()) {
      ...
      for (const n of list) {
        ...
        if (last && n.tick < last.tick + last.duration + gapTicks) {
          ...
          stats.mergedOverlap++;
          if (last.id) stats.touched.push(last.id);
          continue;
        }
        out.push(n);
      }
    ...
    if (!isDrum) {
      const dead = new Set<N>();
      for (let i = 0; i < kept.length; i++) {
        const a = kept[i];
        if (!a || dead.has(a)) continue;
        for (let j = i + 1; j < kept.length; j++) {
          const c = kept[j];
          if (!c) continue;
          if (c.tick - a.tick >= gapTicks) break; // 已排序 —— 超出起点窗口
      ...
      if (dead.size > 0) kept = kept.filter((n) => !dead.has(n));
    }
    ...
    if (vels.length > 0) {
      ...
      for (const n of kept) {
        if ((n.velocity ?? 100) < floor) {
          n.velocity = median;
          stats.fixedVelocity++;
          if (n.id) stats.touched.push(n.id);
        }
      }
    }
    if (kept.length >= 5) {
    ...
    if (!isDrum && keySet && vels.length > 0) {
      ...
      kept = kept.filter((n) => {
        if (keySet.has(((n.pitch % 12) + 12) % 12)) return true;
    ...
    if (kept.length > 0 && durationTicks > 0) {
      ...
      if (!isDrum) {
        ...
        for (let i = 0; i + 1 < kept.length; i++) {
          const cur = kept[i]!;
          const next = kept[i + 1]!;
          if (next.tick < cur.tick + cur.duration) overlapCount++;
        }
        if (kept.length >= 4 && overlapCount / kept.length <= 0.15) {
          for (let i = 0; i + 1 < kept.length; i++) {
            ...
            if (span >= minTicks && cur.duration > span) {
              cur.duration = span;
              stats.trimmedHang++;
              if (cur.id) stats.touched.push(cur.id);
            }
          }
        }
      }
  ...
  if (durationTicks > 0) {
    ...
    if (totalBars > 0) {
      const buckets: CleanableNote[][][] = stems.map((s) => {
        const bars: CleanableNote[][] = Array.from({ length: totalBars }, () => []);
        for (const n of s.notes) {
      ...
      for (const bars of buckets) {
        bars.forEach((list, b) => {
          if (list.length > 0) {
            if (b < firstBar) firstBar = b;
            if (b > lastBar) lastBar = b;
          }
        });
      }
      if (lastBar >= firstBar) {
        ...
        stems.forEach((s, si) => {
          ...
          const otherBar = new Array<boolean>(totalBars).fill(false);
          if (!singleStem) {
            for (let b = firstBar; b <= lastBar; b++) {
              otherBar[b] = buckets.some(
                (ob, oj) => oj !== si && ((ob?.[b]?.length ?? 0) > 0),
              );
            }
          }
          // v3：自相似周期强度 R(d) = mean barSimilarity(bars[i], bars[i-d])。
          const periodScore = (d: number) => {
            let sum = 0;
            let n = 0;
