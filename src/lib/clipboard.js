...
import { VOCAL_LANE_ID, isVocalDirty, resolveTrackVoice, stampSplitWindowSigs, vocalTrackSig, } from "./vocal/vocalRender";
import i18n from "../i18n";
let clipboard = null;
export function clipboardKind() {
...
export function clipboardReferencedPaths() {
    if (!clipboard)
        return [];
    const out = new Set();
    const addSeg = (seg) => {
    ...
    if (clipboard.kind === "segments") {
        ...
        for (const seg of clipboard.track.segments)
            addSeg(seg);
        for (const snap of Object.values(clipboard.renderStates))
            addSnap(snap);
    }
    return [...out];
}
...
export function copySelectedSegments() {
...
function copySelection() {
    ...
    for (const sel of selectionTargets()) {
        ...
        if (!track || !seg || seg.loading)
            continue; // mid-decode clips have no copyable content yet
        ...
        if (clone.processedOutputs?.length === 0)
            delete clone.processedOutputs;
        const laneControls = {};
        const laneMutes = {};
        for (const o of clone.processedOutputs ?? []) {
            const gid = laneGroupId(o);
            const ctrl = laneControlFor(track, gid, o.laneId);
            if (ctrl && !(gid in laneControls))
                laneControls[gid] = structuredClone(ctrl);
            const rk = laneRowKey(o);
            if (track.laneMutes?.[rk])
                laneMutes[rk] = true;
        }
        ...
        const item = {
            ...
            seg: clone,
            renderState: seg.content.type === "audioClip"
...
export function copyTrackToClipboard(trackId) {
    const st = useProjectStore.getState();
    const tempo = st.tempo;
    ...
    for (const seg of clone.segments) {
        ...
        if (seg.processedOutputs?.length === 0)
            delete seg.processedOutputs;
        if (seg.content.type === "audioClip") {
            const snap = useWorkflowStore.getState().snapshotSegmentState(seg.id);
            if (snap)
                renderStates[seg.id] = structuredClone(snap);
        }
...
export function cutSelectedSegments() {
...
function materializeSegment(item, playhead, anchorTick) {
    ...
    if (seg.content.type === "notes") {
        seg.content.notes = seg.content.notes.map((n) => ({ ...n, id: crypto.randomUUID() }));
    }
    return seg;
}
...
function insertSegmentIntoTrack(trackId, seg, item) {
    const st = useProjectStore.getState();
    const track = st.tracks.find((t) => t.id === trackId);
    if (!track)
        return;
    const patch = { segments: [...track.segments, seg] };
    ...
    if (ctrlKeys.length > 0) {
        ...
        for (const k of ctrlKeys)
            laneControls[k] = structuredClone(item.laneControls[k]);
...
function finalizePastedSegment(trackId, segId, renderState, vocal) {
    if (renderState)
        useWorkflowStore.getState().installSegmentState(segId, structuredClone(renderState));
    if (!vocal)
        return;
    const st = useProjectStore.getState();
    const track = st.tracks.find((t) => t.id === trackId);
    const seg = track?.segments.find((s) => s.id === segId);
    if (!track || !seg || seg.content.type !== "notes")
        return;
...
function stripBakeIfUnrenderable(seg, destTrack) {
    if (seg.content.type !== "notes" || !seg.processedOutputs)
        return;
    if (destTrack && resolveTrackVoice(destTrack))
        return;
...
export function pasteClipboard(targetTrackId) {
    if (!clipboard)
        return "empty";
    if (clipboard.kind === "track")
        return pasteTrackClipboard(targetTrackId);
        ...
        for (const plan of plans) {
            ...
            if (plan.fits && plan.candidate) {
                ...
                if (plan.type === "vocal" && plan.candidate?.trackType === "vocal") {
                    if (plan.candidate.vocalParams)
                        nt.vocalParams = structuredClone(plan.candidate.vocalParams);
                    if (plan.candidate.voiceModel)
                        nt.voiceModel = plan.candidate.voiceModel;
                    if (plan.candidate.voiceModelAvatar)
                        nt.voiceModelAvatar = plan.candidate.voiceModelAvatar;
                }
    ...
    for (const p of pasted)
        finalizePastedSegment(p.trackId, p.segmentId, p.renderState, p.vocal);
    useAppStore.getState().selectSegments(pasted.map((p) => ({ trackId: p.trackId, segmentId: p.segmentId })));
    if (useAudioStore.getState().isPlaying)
        useAudioStore.getState().bumpSchedule();
    return "ok";
}
function pasteTrackClipboard(belowTrackId) {
    if (!clipboard || clipboard.kind !== "track")
        return "empty";
    ...
    for (const f of finalize)
        finalizePastedSegment(clone.id, f.segId, f.renderState, f.vocal);
...
export function pasteWithFeedback(targetTrackId) {
