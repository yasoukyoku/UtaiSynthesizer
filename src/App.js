...
export function App() {
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (hasNonAscii) {
                ...
                setTimeout(() => {
                    try {
                        useAppStore.getState().showToast("⚠️ 安装路径含中文/非 ASCII 字符，内嵌 Python 训练环境可能无法正常工作。建议迁移到纯英文路径后再使用训练功能。", "error");
                    }
                    catch {
    ...
    useEffect(() => {
        ...
        try {
            ...
            void win
                ...
                if (useAppStore.getState().updateDialog)
    ...
    useEffect(() => {
        if (!isTauri())
            return;
        const update = () => void invoke("set_tray_labels", { show: i18n.t("tray.show"), quit: i18n.t("tray.quit") }).catch(() => { });
        update();
    ...
    useEffect(() => {
        void invoke("get_data_dir_issue")
            .then((issue) => {
            if (issue)
                useAppStore.getState().showToast(i18n.t("startup.dataDirIssue"), "error");
        })
            .catch(() => { });
    }, []);
    ...
    useEffect(() => {
        ...
        void (async () => {
            ...
            if (disposed || useAppStore.getState().confirm)
                return;
            try {
    ...
    useEffect(() => {
        ...
        void (async () => {
            ...
            for (let i = 0; i < 120 && !disposed && (useAppStore.getState().confirm || useAppStore.getState().updateDialog); i++) {
                await new Promise((r) => setTimeout(r, 1000));
            }
            if (disposed || useAppStore.getState().confirm || useAppStore.getState().updateDialog)
                return;
            ...
            if (disposed || !startupComponentCheckEnabled())
                return;
            try {
    ...
    useEffect(() => {
        const block = (e) => {
            ...
            // the editor's Ctrl+A/C/X/V/D + Ctrl+Z/Y are app-owned (handled elsewhere) and NOT blocked here.
            {
                ...
                if ((mod && !e.altKey && ["f", "g", "p", "u", "j", "r"].includes(bk)) || // find/find-next/print/view-source/downloads/reload
            ...
            if (e.ctrlKey || e.metaKey) {
                ...
                if (k === "z" && !e.shiftKey) {
                    if (editable)
                        return; // let the focused input do its own text-undo
        ...
        const noMiddleAutoscroll = (e) => { if (e.button === 1)
            e.preventDefault(); };
    ...
    return (_jsxs("div", { className: "app-shell", children: [_jsx(Titlebar, { splashLocked: splashOpen }), _jsx("div", { className: "app-content", children: !splashOpen && (_jsxs(_Fragment, { children: [_jsx(DawWorkflowSplit, {}), trainingPageOpen && (_jsx(Suspense, { fallback: null, children: _jsx(TrainingPage, {}) })), logViewerOpen && _jsx(LogViewer, { onClose: toggleLogViewer }), settingsOpen && (_jsx(Suspense, { fallback: null, children: _jsx(Settings, { onClose: toggleSettings }) })), modelManagerOpen && (_jsx(Suspense, { fallback: null, children: _jsx(MsstModelManager, { onClose: toggleModelManager }) })), soundfontManagerOpen && _jsx(SoundfontManager, {}), songStudioOpen && _jsx(SongStudioDialog, { onClose: toggleSongStudio })] })) }), _jsx(ToastContainer, {}), !splashOpen && _jsx(HistoryBanner, {}), _jsx(ConfirmDialog, {}), _jsx(UpdateDialog, {}), _jsx(MissingModelsDialog, {}), !splashOpen && _jsx(OnboardingTour, {}), splashOpen && _jsx(SplashWizard, { onClose: () => setSplashOpen(false) }), !splashOpen && _jsxs(_Fragment, { children: [lyricWizardOpen && _jsx(LyricToVocalWizard, { onClose: () => setLyricWizardOpen(false) }), _jsx(RenderLinkWatcher, {}), _jsx(AutoTuneWatcher, {}), _jsx(AmtWorkbenchHost, {}), _jsx(AmtConversionHost, {})] })] }));
}
...
function AmtWorkbenchHost() {
    ...
    return (_jsx(_Fragment, { children: openIds.map((id) => (_jsx(Suspense, { fallback: null, children: _jsx(MidiWorkbench, { nodeId: id, onClose: () => closeWorkbench(id) }) }, id))) }));
}
...
function AmtConversionHost() {
    ...
    if (!trackId)
        return null;
    return (_jsx(Suspense, { fallback: null, children: _jsx(AmtConversionDialog, { trackId: trackId, onClose: close }) }));
}
