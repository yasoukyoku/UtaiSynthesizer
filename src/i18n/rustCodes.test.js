...
function rsFiles(fs, dir) {
    const out = [];
    for (const name of fs.readdirSync(dir)) {
        const p = `${dir}/${name}`;
        if (fs.statSync(p).isDirectory())
            out.push(...rsFiles(fs, p));
        else if (name.endsWith(".rs"))
            out.push(p);
    }
    return out;
}
async function scanRustCodes() {
...
describe("Rust 的 CODE 必须能被本地化", () => {
    ...
    it("未映射的 CODE 集合不许增长", async () => {
        ...
        expect({
            新增了未映射的CODE_请补三语或加进快照并写明理由: unmapped.filter((c) => !UNMAPPED_SNAPSHOT_S126.includes(c)),
            快照里这些已经被映射了_请从快照删掉: UNMAPPED_SNAPSHOT_S126.filter((c) => !unmapped.includes(c)),
        }).toEqual({
