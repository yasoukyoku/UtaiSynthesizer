...
export const useChordTrackStore = create((set, get) => ({
    ...EMPTY,
    hasAnalysis: () => get().segments.length > 0,
    setAnalysis: (v) => set({ ...v }),
    renameLabel: (index, label) => set((s) => {
        const segs = s.segments;
        if (index < 0 || index >= segs.length)
            return s;
        const trimmed = label.trim();
        if (!trimmed || trimmed === segs[index].label)
            return s;
        const next = segs.slice();
        next[index] = { ...next[index], label: trimmed };
        return { segments: next };
    }),
    moveBoundary: (index, tick) => set((s) => {
        const segs = s.segments;
        ...
        if (snapped < lo || snapped > hi || snapped === a.endTick)
            return s;
        const next = segs.slice();
