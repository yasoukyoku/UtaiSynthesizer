...
function gmFont(id: string, maxProgram = 100): Soundfont {
  return {
    id,
    name: id,
    format: "sf2",
...
describe("GM_PROGRAM 映射", () => {
  it("11 类乐器全部有 program,且鼓为 0(鼓实际走 channel 9)", () => {
    const kinds = ["drums", "bass", "piano", "guitarArp", "guitarStrum", "epiano", "strings", "chords", "synthPad", "pluck", "lead"] as const;
    for (const k of kinds) expect(GM_PROGRAM[k]).toBeGreaterThanOrEqual(0);
...
describe("firstPresetOf", () => {
  it("SF2 优先 0:0", () => {
    const f: Soundfont = { id: "x", name: "x", format: "sf2", sizeBytes: 1, presets: [{ id: "0:5", name: "a" }, { id: "0:0", name: "b" }] };
