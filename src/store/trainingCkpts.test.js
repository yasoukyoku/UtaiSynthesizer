...
import { mergeCkptSources } from "./training";
/**
...
describe("mergeCkptSources", () => {
    ...
    it("matches paths case- and separator-insensitively (Windows)", () => {
        const out = mergeCkptSources([ev({ path: "d:/DATA/training/p/rvc/weights/M_E1_S100.PTH" })], [rec()], "rvc");
        expect(out).toHaveLength(1);
    });
    it("still shows a checkpoint the sidecar just announced but the scan has not seen", () => {
        const out = mergeCkptSources([ev({ path: "D:\\data\\training\\p\\rvc\\weights\\m_e2_s200.pth", step: 200 })], [rec()], "rvc");
    ...
    it("never guesses a kind for an event-only row", () => {
        const out = mergeCkptSources([ev({ path: "D:\\x\\best.pth", kind: "best" }), ev({ path: "D:\\x\\p.pth" })], [], "sovits");
