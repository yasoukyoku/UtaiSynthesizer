...
import { trainingDataOk } from "./training";
/** 一份**平铺**(单歌手)的项目数据集 —— 用户导入完音频之后的常态。 */
function flatDs(over = {}) {
    return {
        files: 12,
        bytes: 1234,
...
describe("trainingDataOk —— 浅扩散(§E2E,S144 实机买回来的)", () => {
    ...
    it("★★ 多歌手数据集 ⇒ 浅扩散不许进(后端会拒,而在这里挡住才不会走完向导才被拒)", () => {
        ...
        // 而宿主自己是多歌手时,捷径必须关掉
        expect(trainingDataOk("sovits_diff", multi, freshSlotInfo({ family: "sovits", n_speakers: 2 }))).toBe(false);
    });
});
