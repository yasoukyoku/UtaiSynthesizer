...
import {
  ...
  type SongTaskProgress,
  type SongTaskResult,
} from "../lib/song/runSongTask";
import type { SongTaskId } from "../lib/models/song-tasks";

export interface QueuedSongTask {
  id: string;
  task: SongTaskId;
  label: string;
  /** 历史占位条目 uuid，用于列表定位"生成中 / 排队中"那一行 */
  entryId?: string;
}
...
interface SongTaskQueueState {
  current: QueuedSongTask | null;
  waiting: QueuedSongTask[];
  progress: SongTaskProgress | null;
  taskError: string | null;
  generatingEntryId: string | null;
  enqueue: (req: EnqueueRequest) => Promise<SongTaskResult>;
  cancelCurrent: () => Promise<void>;
  setTaskError: (error: string | null) => void;
  setGeneratingEntryId: (id: string | null) => void;
}
...
export const useSongTaskQueue = create<SongTaskQueueState>((set, get) => ({
  current: null,
  waiting: [],
  progress: null,
  taskError: null,
  ...
  enqueue: (req) => {
    ...
    const run = async (): Promise<SongTaskResult> => {
      set((s) => ({
        current: item,
      ...
      try {
        ...
        set((s) => ({
          ...
          progress: null,
          generatingEntryId:
            req.entryId && s.generatingEntryId === req.entryId ? null : s.generatingEntryId,
        }));
      }
    };
...
export function enqueueSongTask(req: EnqueueRequest): Promise<SongTaskResult> {
