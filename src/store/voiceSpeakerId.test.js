...
describe("speaker id semantics (S81 drift guard)", () => {
    it("a vocal track resolves the SVC speaker, NOT the ScoreToCV one", () => {
        ...
        const vp = { backend: "sovits", speakerId: 49, sovits: {}, rvc: {} };
    ...
    it("honours an explicit SVC speaker and the dominant blend entry", () => {
        expect(vocalTrackSpeakerId({ backend: "sovits", sovits: { speaker_id: 3 } })).toBe(3);
        expect(vocalTrackSpeakerId({ backend: "rvc", rvc: { speaker_id: 2 } })).toBe(2);
    ...
    it("reads the backend's OWN options, not the other backend's", () => {
        ...
        const vp = { backend: "rvc", sovits: { speaker_id: 7 }, rvc: { speaker_id: 2 } };
        expect(vocalTrackSpeakerId(vp)).toBe(2);
        expect(vocalTrackSpeakerId({ ...vp, backend: "sovits" })).toBe(7);
    });
