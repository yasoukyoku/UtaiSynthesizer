...
const FRESH = structuredClone(useTrainingStore.getState().config);
function ws(over = {}) {
    return {
        exists: true,
        family: "sovits",
...
async function drive(backend, info) {
...
describe("§E2E-M11 冷启动之后,manifest 的值真的到了 start_training 的载荷上", () => {
    ...
    it("sovits_v2:5 份 + loudnorm(v2 同样送、同样折进它的 fp_text)", async () => {
        const req = await drive("sovits_v2", ws({ family: "sovits_v2", version: "4.0-v2", aug_copies: 5, loudnorm: true }));
