...
function notes() {
    return useProjectStore.getState().tracks[0].segments[0].content.notes;
}
function content() {
    return useProjectStore.getState().tracks[0].segments[0].content;
}
let uninstall = null;
beforeEach(() => {
    useAppStore.setState({ selectedSegment: null, selectedSegments: [], activeTrackId: null });
...
describe("Phase 3 — .usp save/load round-trips every vocal field (GATE C)", () => {
    const rich = vocalTrack(notesSeg([
        {
    ...
    ], { pitchDev: { xs: [0, 240], ys: [0, 50] }, paramCurves: { loudness: { xs: [0, 480], ys: [0, -3] } } }), { backend: "sovits", speakerId: 49, langId: 2, transpose: 2, formant: -3, breathToken: "AP",
    ...
    it("S73/S159:rangeExtend 的显式【非默认】值存读不丢(存量 bug 修复,S159 随极性翻面)", () => {
        const t = { ...rich, vocalParams: { ...rich.vocalParams, rangeExtend: false } };
        ...
        const on = { ...rich, vocalParams: { ...rich.vocalParams, rangeExtend: true } };
...
describe("Phase 4a — store additions", () => {
    ...
    it("createVocalPart adds an empty notes segment (returns id) and is undoable", () => {
        ...
        expect(seg.content.notes).toHaveLength(0);
    ...
    it("splitSegment on a notes part: partitions notes + rebases + slices curves + one undo step (§user)", () => {
        ...
        expect(left.content.notes.map((n) => n.id)).toEqual(["n1"]);
        const rn = right.content.notes;
        ...
        expect(left.content.pitchDev).toBeDefined();
        expect(right.content.pitchDev).toBeDefined();
        expect(left.content.paramCurves?.loudness).toBeDefined();
        expect(right.content.paramCurves?.loudness).toBeDefined();
    ...
    it("splitSegment SNAPS a mid-note split to the note's END, keeping the note whole (§user)", () => {
        ...
        expect(segs[0].content.notes).toHaveLength(1); // n1 whole, on the left
        expect(segs[1].content.notes).toHaveLength(0); // right half is the trailing rest
    });
...
describe("Phase 4a — untrusted .usp load boundary (§9.8.1)", () => {
    it("clamps/sanitizes hostile notes + vocalParams; drops id-less notes", () => {
        ...
        const seg = loaded.tracks[0].segments[0].content;
...
describe("Phase 4a — pure vocalNotes helpers", () => {
    const mk = (id, tick, dur) => ({ id, tick, duration: dur, pitch: 60, lyric: "あ", velocity: 100 });
...
describe("Phase 5 — property sidebar data-layer (transition override / vibrato)", () => {
    ...
    it("REPRO track-default undo: note edit THEN transaction-wrapped track-default edit → undo reverts the track-default (not the note)", () => {
        ...
        hist().beginTransaction();
        for (const v of [260, 275, 300, 330, 360])
            setVocalParams(T, { transition: { ...DEFAULT_TRANSITION, durLeftMs: v } });
        hist().commitTransaction();
    ...
    const sigBase = { backend: "sovits", speakerId: 49, langId: 2, transpose: 0, formant: 0, transition: DEFAULT_TRANSITION };
    ...
    it("★ phonemeSet folds OUT at its default and each convention is its OWN signature", () => {
        ...
        for (const phonemeSet of ["arpasing", "xsampa", "vccv"]) {
    ...
    it("★ an UNKNOWN convention from a newer build loads as the default, not as itself", () => {
        ...
        const withBad = { ...sigBase, phonemeSet: "delta" };
...
describe("S73 — autoTuned 调教所有权标记(假脏铁律全套)", () => {
    ...
    it("autoTuned 随 .usp 往返字节一致", () => {
        ...
        expect(parsed.tracks[0].segments[0].content.notes[0].autoTuned).toBe(true);
