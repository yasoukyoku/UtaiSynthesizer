...
interface NoteData {
  /** Stable edit id `${stemIdx}-${noteIdx}` — selection & undo identity. */
  id?: string;
  tick: number;
  duration: number;
  pitch: number;
  velocity?: number;
  /** 歌词（人声轨）：import_score_file 从 MIDI Lyric meta 读出，导入轨道时带到音符上。 */
  lyric?: string;
}
...
interface StemData {
  ...
  soloed: boolean;
  color: string;
  /** GM program (0-127) from the source file's metadata — preserved on edit write-back. */
  program?: number | null;
  /** First MIDI channel (0-15; 9 = drums) — preserved on edit write-back. */
  channel?: number | null;
  /** 该 stem 来自 midiPath 文件内的第几条 MIDI 轨 —— 自动歌词写回后按 (midiPath,
   *  trackIdx) 精确重导同一条轨。 */
  midiTrackIdx?: number;
}
 ...
 */
function buildNotesFromTrack(rawNotes: AmtNoteResult[], startTick: number, stemIdx: number): NoteData[] {
  const hasLyrics = rawNotes.some(
    (n) => String(n.lyric ?? "").trim() !== "" && !PLACEHOLDER_LYRICS.has(String(n.lyric ?? "")),
  );
  return rawNotes.map((n, ni) => ({
    id: `${stemIdx}-${ni}`,
    tick: (n.tick as number) + startTick,
    duration: n.duration as number,
    pitch: n.pitch as number,
    velocity: (n as unknown as { velocity?: number }).velocity ?? 100,
...
function SepWaveCanvas({ peaks, color, active, playFrac }: {
  ...
  return <canvas ref={canvasRef} className="amt-sep-wave-canvas" />;
}

export function AmtResultPanel({ result, onClose, style }: AmtResultPanelProps) {
  ...
  const [playSource, setPlaySource] = useState<"midi" | "original">("midi");
  ...
  const [quantScope, setQuantScope] = useState<"all_tracks" | "selected_notes">("all_tracks");
  ...
  const [activeTab, setActiveTab] = useState<"edit" | "export" | "lyrics">("edit");
  ...
  const sfMenuRef = useRef<HTMLDivElement>(null);
  const originalStemsRef = useRef<StemData[] | null>(null);
  const undoRef = useRef<StemData[][]>([]);
  const redoRef = useRef<StemData[][]>([]);
  const clipboardRef = useRef<NoteData[]>([]);
  ...
  const dragRef = useRef<{
    ...
    startX: number;
    startY: number;
    orig: Map<string, { tick: number; duration: number; pitch: number }>;
    draft: Map<string, { tick: number; duration: number; pitch: number }>;
  } | null>(null);
  ...
  const [prepAudio, setPrepAudio] = useState<{
    preview?: string;
    original?: string;
    /** Per-instrument WAVs keyed "gm:NNN" / "drums" (sidecar contract). */
  ...
  const [repairHighlight, setRepairHighlight] = useState<Set<string>>(new Set());
  const repairHighlightTimer = useRef<number | null>(null);
  ...
  const [lyricEdit, setLyricEdit] = useState<{ stemIdx: number; noteId: string; x: number; y: number; value: string } | null>(null);
  ...
  const isSeparation = !!(result as AmtResult).separationOnly ||
    (result.midiPaths.length === 0 && Object.values(result.stems ?? {}).some((s: any) => s?.audio));
    ...
    async function loadData() {
      ...
      try {
        ...
        for (const [k, v] of Object.entries(result.stems ?? {})) {
          if (v?.audio) wavMap[k] = v.audio as string;
        }
        ...
        if (isSeparation) {
          const sepEntries: Array<[string, { midi: string; audio?: string }]> =
            Object.entries(result.stems ?? {}).filter(([, v]) => !!v?.audio);
          let sepIdx = 0;
          ...
          for (const st of loadedStems) {
            if (!st.audioPath) continue;
            try {
              const data = await useAudioStore.getState().loadAudioFile(st.audioPath);
              if (data.peaks && data.peaks.length > 0) peaksMap[st.name] = data.peaks;
              if (data.durationMs > 0 && data.durationMs / 1000 > sepDur) sepDur = data.durationMs / 1000;
            } catch { /* best-effort waveform */ }
          }
          setStemPeaks(peaksMap);
          if (sepDur > 0) setDuration(sepDur);
          return;
        }
        ...
        if (result.midiPaths.length > 0) {
          try {
            const meta = await invoke<any>("amt_midi_metadata", { midiPath: result.midiPaths[0] });
            if (meta.bpm) {
              setBpm(meta.bpm);
              setDetectedBpm(meta.bpm);
              setProjBpm(meta.bpm);
            }
            if (meta.ppq) setPpq(meta.ppq);
          } catch (e) {
        ...
        for (let i = 0; i < result.midiPaths.length; i++) {
          const path = result.midiPaths[i] as string;
          ...
          for (const track of score.tracks) {
            const mt = metaTracks.find((m: any) => m.name === track.name) || metaTracks[0];
            let audioPath = matchInstrumentWav(
              wavMap,
              track.name || "",
              mt?.program ?? null,
              mt?.channel ?? null,
            ...
            const startTick = (track.start_tick as number) || 0;
            const rawNotes = track.notes as AmtNoteResult[];
            ...
            const displayName = isVocalTrackName(rawName)
              ...
              : rawName;
            if (isVocalTrackName(rawName)) vocalCount++;
            // §用户「干声转MIDI试听要是人声」：干音轨没匹配到独立 bus 时挂干音本身
            //（转换源就是人声）——纯人声结果的播放器配置据此用干音做 MIDI 侧音源。
            if (!audioPath) {
              const dryVocal = ((result as unknown) as any).vocalAudioPath || result.sourceAudioPath || result.originalAudio;
              if (dryVocal && isVocalTrackName(rawName)) audioPath = dryVocal;
            }
            loadedStems.push({
              name: displayName,
              notes,
        ...
        if (firstStem) setActiveInstr(firstStem.name);
  ...
  useEffect(() => {
    if (isSeparation || isLoading || autoLyricsRef.current) return;
    const vocalStems = stems.filter((s) => isVocalTrackName(s.name) && s.midiPath);
    if (vocalStems.length === 0) return;
    const anyLyrics = stems.some((s) =>
      s.notes.some((n) => String(n.lyric ?? "").trim() !== ""));
    if (anyLyrics) return;
    ...
    (async () => {
      try {
        ...
        const installed = await invoke<Array<{ filename: string; is_installed: boolean }>>(
          "list_amt_models",
        );
        const sizes = (installed ?? [])
          ...
          .filter(Boolean);
        if (!alive) return;
        const rank: Record<string, number> = { tiny: 0, base: 1, small: 2, medium: 3 };
        ...
        const dryVocal = ((result as unknown) as any).vocalAudioPath as string | undefined;
        ...
        for (const midiPath of targets) {
          ...
          setStems((prev) => prev.map((s) => {
            if (s.midiPath !== midiPath || s.midiTrackIdx == null) return s;
            const track = score.tracks?.[s.midiTrackIdx as number];
            if (!track) return s;
            const startTick = (track.start_tick as number) || 0;
        ...
        showToast(
          noteCount > 0
            ...
            : (t("amt.autoLyricsDone", { count: lines.length }) || `人声歌词已自动写回（${lines.length} 句）`),
          "success",
        );
      } catch (e) {
        const msg = String(e);
  ...
  const syncLyricsFromMidi = React.useCallback(async (midiPath: string) => {
    try {
      const score = await invoke<any>("import_score_file", { path: midiPath });
      setStems((prev) => prev.map((s) => {
        if (s.midiPath !== midiPath || s.midiTrackIdx == null) return s;
  ...
  useEffect(() => {
    invoke<any>("amt_export_tools_status")
  ...
  useEffect(() => {
    ...
    import("@tauri-apps/api/event").then(({ listen }) =>
      listen("amt-soundfont-installed", () => refreshSoundfonts()),
    ).then((u) => {
      if (cancelled) u();
      else un = u;
    }).catch(() => {});
  ...
  useEffect(() => {
    ...
    const onDown = (e: PointerEvent) => {
      if (sfMenuRef.current && !sfMenuRef.current.contains(e.target as Node)) {
        setSfOpen(false);
      }
    };
  ...
  useEffect(() => {
    ...
    if (Object.keys(wavMap).length === 0) return;
    setStems((prev) => {
  ...
  useEffect(() => {
    ...
    const safePpq = ppq > 0 ? ppq : 480;
    const safeBpm = bpm > 0 ? bpm : 120;
    const maxTick = stems.reduce(
      (max, s) => s.notes.reduce((m, n) => Math.max(m, n.tick + n.duration), max),
      0,
    );
    setDuration(maxTick / (safePpq * (safeBpm / 60)));
  ...
  useEffect(() => {
    ...
    if (!needPreview && !needOriginal && !needWavs) return;
    const srcAudio = result.sourceAudioPath || result.originalAudio;
    const midi = result.midiPaths[0];
    if (!srcAudio || !midi || !result.outputDir) return;

    let cancelled = false;
    setPreparingAudio(true);
    invoke<any>("amt_prepare_playback", {
      midiPath: midi,
      audioPath: srcAudio,
      outputDir: result.outputDir,
    })
      .then((res) => {
        ...
        setPrepAudio({
          ...
          wavs: (res?.instrument_wavs ?? {}) as Record<string, string>,
        });
      })
      .catch((e) => {
  ...
  useEffect(() => {
    ...
    useAudioStore
      ...
      .then((data) => {
        if (cancelled) return;
        if (data.peaks && data.peaks.length > 0) setMasterPeaks(data.peaks);
        if (data.durationMs > 0) setMasterDur(data.durationMs / 1000);
      })
      .catch(() => {
        if (!cancelled) {
          setMasterPeaks([]);
  ...
  const seekMasterTrack = (frac: number) => {
    const totalDur = masterDur || duration || 1;
  ...
  const syncPlayerRef = useRef<SynchronizedPlayer | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      const instrumentPaths: Record<string, string> = {};
      for (const s of stems) {
        if (s.audioPath) instrumentPaths[s.name] = s.audioPath;
      }
      ...
      const pureVocal =
        !isSeparation && noteStems.length > 0 &&
      ...
      if (frozenPosRef.current > 0) player.seek(frozenPosRef.current);
    })();
    return () => {
      player.dispose();
      if (syncPlayerRef.current === player) syncPlayerRef.current = null;
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    ...
    if (!player) return;
    const muted = new Set(
      stems.filter(s => s.audioPath && s.muted).map(s => s.name),
    );
    player.setMixState({
      mix: mixPercent / 100,
      stereo: stereoMode,
  ...
  const cloneStems = (src: StemData[]): StemData[] =>
    src.map((s) => ({ ...s, notes: s.notes.map((n) => ({ ...n })) }));

  const findNoteAt = (x: number, y: number) => {
    const rollHeight = 128 * ROW_HEIGHT;
    const hasSolo = stems.some((s) => s.soloed);
    for (let si = stems.length - 1; si >= 0; si--) {
      ...
      if (hasSolo && !stem.soloed) continue;
  ...
  const pitchForY = (y: number) => {
    const rollHeight = 128 * ROW_HEIGHT;
    return Math.max(0, Math.min(127, Math.floor((rollHeight - y) / ROW_HEIGHT) - 1));
  };
  const tickForX = (x: number) => Math.max(0, x / pixelsPerTick);
  ...
  const selectionEntries = useMemo(() => {
    const out: Array<{ si: number; ni: number; note: NoteData }> = [];
    stems.forEach((s, si) =>
      s.notes.forEach((n, ni) => {
        if (n.id && selNotes.has(n.id)) out.push({ si, ni, note: n });
      }),
    );
    return out;
  }, [stems, selNotes]);
  ...
  const resynthTimerRef = useRef<number | null>(null);
  const renderGenRef = useRef(0);

  const scheduleResynthesis = (next: StemData[]) => {
    if (isSeparation) return;
    const srcAudio = result.sourceAudioPath || result.originalAudio;
    if (!srcAudio || !result.outputDir) return; // no re-render source — edits stay visual-only
    if (resynthTimerRef.current) window.clearTimeout(resynthTimerRef.current);
    resynthTimerRef.current = window.setTimeout(async () => {
      ...
      try {
        ...
        const res = await invoke<any>("amt_prepare_playback", {
          midiPath,
          audioPath: srcAudio,
          outputDir: outDir,
        });
        if (gen !== renderGenRef.current) return; // superseded by a newer edit
        setPrepAudio({
          preview: res?.transcription_wav || "",
          original: res?.original_wav || "",
          wavs: (res?.instrument_wavs ?? {}) as Record<string, string>,
        ...
        showToast(
          t("amt.editorAudioFailed", { error: String(e) }) || `试听音频更新失败：${e}`,
          "error",
        );
      } finally {
        if (gen === renderGenRef.current) setEditRendering(false);
      }
    }, 400);
  };

  const applyEdit = (next: StemData[], record = true) => {
    if (record) {
  ...
  const resetEdits = () => {
    if (!originalStemsRef.current) return;
  ...
  const sfPreviewSrcRef = useRef<AudioBufferSourceNode | null>(null);
  ...
  const previewSoundfontSnippet = React.useCallback(async () => {
    try {
      ...
      if (picked.length === 0) picked = stem.notes.slice(0, 4);
      ...
      const wavPath = await invoke<string>("preview_notes_render", {
        bpm: safeBpm,
        notes,
  ...
  const handleImportSoundfont = async () => {
    try {
      ...
      const info = await invoke<any>("amt_import_soundfont", { sourcePath: src });
      refreshSoundfonts();
  ...
  const handleDeleteSoundfont = async (filename: string, name: string) => {
    try {
  ...
  const deleteSelection = () => {
    if (!editMode || selNotes.size === 0) return;
  ...
  const addNoteAt = (tick: number, pitch: number) => {
    if (!editMode || !activeInstr) return;
  ...
  const copySelection = () => {
    if (!editMode || selectionEntries.length === 0) return;
  ...
  const pasteClipboard = () => {
    if (!editMode || clipboardRef.current.length === 0) return;
    ...
    const byInstr = new Map<string, NoteData[]>();
    for (const n of clip) {
  ...
  const duplicateSelection = () => {
    ...
    for (const { si, note } of selectionEntries) {
      const copy: NoteData = { ...note, id: newNoteId(), tick: note.tick + clamped };
  ...
  const quantizeSelection = () => {
    ...
    next.forEach((s) =>
      s.notes.forEach((n) => {
        if (!all && (!n.id || !selNotes.has(n.id))) return;
  ...
  const smartCleanup = () => {
    ...
    if (stats.touched.length > 0) {
      ...
      if (repairHighlightTimer.current) window.clearTimeout(repairHighlightTimer.current);
      repairHighlightTimer.current = window.setTimeout(() => setRepairHighlight(new Set()), 6000);
    }
    const removedExtra =
      stats.removedShort + stats.removedRange + stats.removedGhost + stats.removedKey;
    showToast(
  ...
  const transformSelection = (
    command: "move_time" | "resize_time" | "pitch" | "velocity",
    amount: number,
  ) => {
    if (!editMode || selectionEntries.length === 0) return;
    const next = cloneStems(stems);
    const entries = selectionEntries
      ...
      .filter((e): e is { si: number; ni: number; note: NoteData } => e.note != null);
    if (command === "move_time") {
      let delta = gridTicks * amount;
      delta = Math.max(
        -Math.min(...entries.map((e) => e.note.tick)),
        Math.min(durationTicks - Math.max(...entries.map((e) => e.note.tick + e.note.duration)), delta),
      );
      for (const e of entries) e.note.tick += delta;
    } else if (command === "resize_time") {
      let delta = gridTicks * amount;
      delta = Math.max(
        -(Math.min(...entries.map((e) => e.note.duration)) - 1),
        Math.min(durationTicks - Math.max(...entries.map((e) => e.note.tick + e.note.duration)), delta),
      );
      for (const e of entries) e.note.duration = Math.max(1, e.note.duration + delta);
    } else if (command === "pitch") {
      const delta = Math.max(
        21 - Math.min(...entries.map((e) => e.note.pitch)),
        Math.min(108 - Math.max(...entries.map((e) => e.note.pitch)), amount),
      );
      for (const e of entries) e.note.pitch += delta;
    } else if (command === "velocity") {
      const delta = Math.max(
        1 - Math.min(...entries.map((e) => e.note.velocity ?? 100)),
        Math.min(127 - Math.max(...entries.map((e) => e.note.velocity ?? 100)), amount),
      );
      for (const e of entries) e.note.velocity = Math.max(1, Math.min(127, (e.note.velocity ?? 100) + delta));
    }
    applyEdit(next);
  };

  /** Velocity spin: sets the ABSOLUTE velocity on the selection (reference behavior). */
  const onVelocitySpin = (v: number) => {
    setVelSpin(v);
    if (!editMode || selectionEntries.length === 0) return;
    const next = cloneStems(stems);
    for (const { si, ni } of selectionEntries) {
      const target = next[si]?.notes[ni];
      if (target) target.velocity = v;
    }
    applyEdit(next);
  };

  // ── Roll mouse interactions (edit mode) ──
  const rollMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (hit) {
      ...
      if (nextSel.size === 1) {
        ...
        if (hitStem) setActiveInstr(hitStem.name);
        setVelSpin(hit.note.velocity ?? 100);
      }
      const nearRight = x > hit.nx + hit.nw - Math.max(6, hit.nw * 0.25);
      const orig = new Map<string, { tick: number; duration: number; pitch: number }>();
      const entries =
        nextSel.size > 0
          ? stems.flatMap((s) => s.notes).filter((n) => n.id && nextSel.has(n.id))
          : [hit.note];
      for (const n of entries) {
        if (n.id) orig.set(n.id, { tick: n.tick, duration: n.duration, pitch: n.pitch });
      }
      dragRef.current = {
      ...
      if (!additive) setSelNotes(new Set());
    }
  };

  const rollMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (drag) {
      ...
      if (drag.snap) dTick = Math.round(dTick / gridTicks) * gridTicks;
      const dPitch = pitchForY(y) - pitchForY(drag.startY);
      const vals = [...drag.orig.values()];
      const draft = new Map<string, { tick: number; duration: number; pitch: number }>();
      if (drag.mode === "move") {
        let delta = Math.max(
          -Math.min(...vals.map((v) => v.tick)),
          Math.min(durationTicks - Math.max(...vals.map((v) => v.tick + v.duration)), dTick),
        );
        if (!Number.isFinite(delta)) delta = dTick;
        ...
        if (Number.isFinite(room)) delta = Math.min(room, delta);
        for (const [id, v] of drag.orig) {
          draft.set(id, {
            tick: v.tick,
  ...
  const rollDoubleClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const rect = canvas.getBoundingClientRect();
    const x = e.clientX - rect.left;
  ...
  const commitRollLyric = (value: string) => {
    const le = lyricEditRef.current;
    setLyricEdit(null);
    if (!le) return;
    const v = value.trim();
    if (!v || v === (le.value ?? "").trim()) return;
    const next = cloneStems(stems);
  ...
  const rollContextMenu = (e: React.MouseEvent<HTMLCanvasElement>) => {
    if (!editMode) return;
    e.preventDefault();
    const canvas = canvasRef.current;
    if (!canvas) return;
    ...
    if (hit && hit.note.id) {
      ...
      if (!selNotes.has(hit.note.id)) setSelNotes(new Set([hit.note.id]));
      const hitStem = stems[hit.si];
      if (hitStem) setActiveInstr(hitStem.name);
    }
  ...
  const rollKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    // 「?」切换 / Esc 关闭快捷键速查卡（优先于其他手势）。
    if (e.key === "?" || (e.shiftKey && e.key === "/")) { e.preventDefault(); setRollHelpOpen((o) => !o); return; }
    if (e.key === "Escape" && rollHelpOpen) { e.preventDefault(); setRollHelpOpen(false); return; }
    if (!editMode) return;
    ...
    if (mod && !e.shiftKey && key === "z") return handle(undoEdit);
    if ((mod && key === "y") || (mod && e.shiftKey && key === "z")) return handle(redoEdit);
    if (mod && key === "a") return handle(selectAllNotes);
    if (mod && key === "x") return handle(cutSelection);
    if (mod && key === "c") return handle(copySelection);
    if (mod && key === "v") return handle(pasteClipboard);
    if (mod && (key === "d" || key === "b")) return handle(duplicateSelection);
    if ((e.altKey && key === "q") || (mod && key === "u")) return handle(quantizeSelection);
    if (key === "delete" || key === "backspace") return handle(deleteSelection);
    if (key === "escape") return handle(() => setSelNotes(new Set()));
    if (key === "arrowleft" || key === "arrowright") {
      const dir = key === "arrowleft" ? -1 : 1;
      return handle(() =>
        transformSelection(e.shiftKey ? "resize_time" : "move_time", dir),
      );
    }
    if (key === "arrowup" || key === "arrowdown") {
      const dir = key === "arrowup" ? 1 : -1;
      if (mod) return handle(() => transformSelection("velocity", dir));
      return handle(() => transformSelection("pitch", dir * (e.shiftKey ? 12 : 1)));
    }
  };

  // ── Tempo controls (reference bpm_spin / speed_spin linkage) ──
  const clampRate = (r: number) => Math.min(20, Math.max(0.05, r));
  const onProjBpmChange = (v: number) => {
    const clamped = Math.min(400, Math.max(4, v));
    setProjBpm(clamped);
    if (detectedBpm && detectedBpm > 0) setSpeed(clampRate(clamped / detectedBpm));
  };
  const onSpeedChange = (v: number) => {
    if (!detectedBpm || detectedBpm <= 0) return;
  ...
  const handleImportAll = async () => {
    try {
      ...
      for (const stem of stems) {
        ...
        const stemHasLyrics = stem.notes.some((n) => String((n as unknown as { lyric?: string }).lyric ?? "").trim() !== "");
          ...
          segments: [{
            ...
            content: {
              ...
              notes: stem.notes.map(n => ({
                ...
                duration: n.duration,
                pitch: n.pitch,
                lyric: ((n as unknown as { lyric?: string }).lyric ?? ""),
                velocity: n.velocity ?? 100
              }))
            },
  ...
  const mapExportError = (msg: string): string => {
  ...
  const writeCurrentMidiSnapshot = async (): Promise<string> => {
    const base =
      result.outputDir?.replace(/[\/\\]+$/, "") ||
      (stems[0]?.midiPath || "").replace(/[/\\][^/\\]+$/, "");
    if (!base) throw new Error("EXPORT_SRC_NOT_FOUND: no output dir");
  ...
  const handleExportZip = async () => {
    ...
    try {
      ...
      if (!isSeparation) {
        ...
        showToast(
          exported > 0
            ...
            : (t("amt.exportNoFiles") || "没有可导出的 MIDI 文件"),
      ...
      const pathsToZip = stems.map((s) => s.audioPath).filter(Boolean) as string[];
  ...
  const handleExportSheetMusic = async () => {
    ...
    try {
      ...
      const res = await invoke<{ zipPath: string }>("amt_export_sheet_music", {
        midiPath: midi,
  ...
  const handleExportTranscriptionAudio = async (preset: "hq" | "compat") => {
    setDownloadOpen(false);
    if (exportBusy) return;
    ...
    try {
      ...
      if (!out) return;
      const midi = await writeCurrentMidiSnapshot();
      const res = await invoke<string>("amt_export_midi_audio", {
        midiPath: midi,
  ...
  const handleExportStemsAudio = async (preset: "hq" | "compat") => {
    setDownloadOpen(false);
    if (exportBusy) return;
    ...
    try {
      ...
      if (!out) return;
      const midi = await writeCurrentMidiSnapshot();
      const res = await invoke<{ zipPath: string; members: string[] }>("amt_export_stems_audio", {
        midiPath: midi,
        outPath: out.toLowerCase().endsWith(".zip") ? out : `${out}.zip`,
        preset,
      });
      showToast(
        t("amt.exportStemsSaved", { count: res.members.length, path: res.zipPath }) ||
          `已导出 ${res.members.length} 个乐器 WAV`,
        "success",
      );
    } catch (e) {
      showToast(mapExportError(String(e)), "error");
  ...
  const handleExportStereoWav = async () => {
    ...
    try {
      ...
      if (!out) return;
      const midi = await writeCurrentMidiSnapshot();
      const res = await invoke<string>("amt_export_stereo_wav", {
  ...
  const pianoKeys = useMemo(() => {
    ...
    for (let i = 127; i >= 0; i--) {
      ...
      keys.push(
        <div key={i} className={`amt-key ${isBlack ? "black" : ""}`} style={{ height: ROW_HEIGHT }}>
          {i % 12 === 0 ? <span className="amt-key-label">{`${noteName}${octave}`}</span> : ""}
        </div>
      );
    }
    return keys;
  ...
  return (
    <div className="amt-result-panel" style={style}>
      {isLoading && (
        <div className="amt-loading-overlay">
          <div className="amt-spinner" />
        </div>
      )}
      
      <div className="amt-result-header">
        {/* Row 1 — 标题 + 功能按钮（紧凑排列，全部可见不遮挡） */}
        <div className="amt-header-row-top">
          <div className="amt-header-left">
            <button className="amt-close-btn" onClick={onClose} title={t("common.close")}>✕</button>
            <div className="amt-title-group">
              <h3>{t("amt.resultWorkbench")}</h3>
              <span className="amt-subtitle">
                {isSeparation
                  ? t("amt.separationDesc", { count: stems.length })
                  : t("amt.resultDesc", { count: result.midiPaths.length })}
              </span>
            </div>
          </div>

          <div className="amt-header-actions">
          <button className="amt-action-btn secondary" onClick={handleAnother} title={t("amt.transcribeAnotherTooltip") || "关闭当前结果并重新打开转换对话框"}>
            <span className="icon">🎵</span> {t("amt.transcribeAnother") || "再转一首"}
          </button>
          {!isSeparation && (
            <button
              className={`amt-action-btn secondary ${lyricsOpen ? "active" : ""}`}
              onClick={() => setLyricsOpen((v) => !v)}
              title={t("amt.lyricsTooltip") || "从歌曲中识别歌词（自动检测语言），编辑后写回 MIDI"}
            >
              <span className="icon">🎤</span> {t("amt.lyricsBtn") || "歌词"}
            </button>
          )}
          {!isSeparation && (
            <div className="amt-download-menu" ref={sfMenuRef}>
              <button
                className="amt-action-btn secondary"
                onClick={() => setSfOpen((v) => !v)}
                title={t("amt.sfTooltip") || "切换 MIDI 合成音源（SoundFont），试听与导出共用"}
              >
                <span className="icon">🎹</span> {t("amt.sfLabel") || "音源"}
                <span className="amt-download-caret">▾</span>
              </button>
              {sfOpen && (
                <div className="amt-download-panel amt-sf-panel" role="menu">
                  <div className="amt-download-group-title">{t("amt.sfListTitle") || "选择音源"}</div>
                  {soundfonts.length === 0 && (
                    <div className="amt-sf-empty">{t("amt.sfEmpty") || "未找到音源，请先下载内置音源"}</div>
                  )}
                  {soundfonts.map((sf) => (
                    <div key={sf.filename} className={`amt-sf-row ${sf.active ? "active" : ""}`}>
                      <button
                        className="amt-download-item"
                        role="menuitemradio"
                        aria-checked={sf.active}
                        onClick={() => handleSwitchSoundfont(sf.filename, sf.name)}
                        title={sf.filename}
                      >
                        <span className="amt-download-icon">{sf.active ? "●" : "○"}</span>
                        <span className="amt-download-label">
                          {sf.name}
                          {sf.isBuiltin ? `（${t("amt.sfBuiltin") || "内置"}）` : ""}
                        </span>
                      </button>
                      {!sf.isBuiltin && (
                        <button
                          className="amt-sf-delete"
                          title={t("amt.sfDeleteTooltip") || "删除该音源"}
                          onClick={() => handleDeleteSoundfont(sf.filename, sf.name)}
                        >
                          ✕
                        </button>
                      )}
                  ...
                  <button className="amt-download-item" onClick={handleImportSoundfont}>
                    <span className="amt-download-icon">＋</span>
                    <span className="amt-download-label">{t("amt.sfImport") || "导入音源（SF2 / SF3）…"}</span>
                  </button>
                </div>
              )}
            </div>
          ...
          {isSeparation ? (
            <button className="amt-action-btn secondary" onClick={handleExportZip}>
              <span className="icon">📦</span> {t("amt.exportStems") || "导出分轨"}
            </button>
          ) : (
            <div className="amt-download-menu" ref={downloadRef}>
              <button
                className={`amt-action-btn secondary ${exportBusy ? "busy" : ""}`}
                onClick={() => setDownloadOpen((v) => !v)}
                disabled={!!exportBusy}
                title={t("amt.downloadTooltip") || "导出 MIDI / 乐谱 / 音频"}
              >
                <span className="icon">📦</span>{" "}
                {exportBusy
                  ? (t("amt.exportRendering") || "渲染中…")
                  : t("amt.download") || "下载"}
                {!exportBusy && <span className="amt-download-caret">▾</span>}
              </button>
              {downloadOpen && (
                <div className="amt-download-panel" role="menu">
                  <button
                    className="amt-download-item"
                    role="menuitem"
                    disabled={!!exportBusy}
                    onClick={handleExportZip}
                    title={t("amt.downloadMidiTooltip") || "每个乐器一个 .mid 文件（含当前编辑）"}
                  >
                    <span className="amt-download-icon">🎼</span>
                    <span className="amt-download-label">{t("amt.downloadMidi") || "导出可编辑 MIDI"}</span>
                    {editDirty && <span className="amt-download-badge">{t("amt.downloadEdited") || "已编辑"}</span>}
                  </button>
                  <button
                    className="amt-download-item"
                    role="menuitem"
                    disabled={!!exportBusy || !exportTools?.musescoreAvailable}
                    title={
                      exportTools?.musescoreAvailable
                        ? (t("amt.downloadSheetTooltip") || "MusicXML + PDF 乐谱包")
                        : (t("amt.exportErrMusescore") || "MuseScore 未安装，请先在资源管理中下载")
                    }
                    onClick={handleExportSheetMusic}
                  >
                    <span className="amt-download-icon">📄</span>
                    <span className="amt-download-label">{t("amt.downloadSheetMusic") || "乐谱包（MusicXML / PDF）"}</span>
                  </button>
                  <div className="amt-download-group">
                    <div className="amt-download-group-title">{t("amt.downloadTranscription") || "转写合成 WAV"}</div>
                    <button
                      className="amt-download-item"
                      role="menuitem"
                      disabled={!!exportBusy || !exportTools?.fluidsynthAvailable || !exportTools?.soundfontAvailable}
                      onClick={() => handleExportTranscriptionAudio("hq")}
                    >
                      <span className="amt-download-icon">🎧</span>
                      <span className="amt-download-label">{t("amt.audioHq") || "24-bit / 48 kHz（推荐）"}</span>
                    </button>
                    <button
                      className="amt-download-item"
                      role="menuitem"
                      disabled={!!exportBusy || !exportTools?.fluidsynthAvailable || !exportTools?.soundfontAvailable}
                      onClick={() => handleExportTranscriptionAudio("compat")}
                    >
                      <span className="amt-download-icon">🎧</span>
                      <span className="amt-download-label">{t("amt.audioCompat") || "16-bit / 44.1 kHz（兼容）"}</span>
                    </button>
                  </div>
                  <div className="amt-download-group">
                    <div className="amt-download-group-title">{t("amt.downloadStemsAudio") || "下载分乐器 WAV（ZIP）"}</div>
                    <button
                      className="amt-download-item"
                      role="menuitem"
                      disabled={!!exportBusy || !exportTools?.fluidsynthAvailable || !exportTools?.soundfontAvailable}
                      onClick={() => handleExportStemsAudio("hq")}
                    >
                      <span className="amt-download-icon">🎚</span>
                      <span className="amt-download-label">{t("amt.audioHq") || "24-bit / 48 kHz（推荐）"}</span>
                    </button>
                    <button
                      ...
                      role="menuitem"
                      disabled={!!exportBusy || !exportTools?.fluidsynthAvailable || !exportTools?.soundfontAvailable}
                      onClick={() => handleExportStemsAudio("compat")}
                    >
                      <span className="amt-download-icon">🎚</span>
                      <span className="amt-download-label">{t("amt.audioCompat") || "16-bit / 44.1 kHz（兼容）"}</span>
                    </button>
                  ...
                  <button
                    ...
                    role="menuitem"
                    disabled={!!exportBusy || !exportTools?.fluidsynthAvailable || !exportTools?.soundfontAvailable}
                    title={t("amt.downloadStereoTooltip") || "左声道原声 / 右声道 MIDI 的对比音频"}
                    onClick={handleExportStereoWav}
                  >
                    <span className="amt-download-icon">↔</span>
                    <span className="amt-download-label">{t("amt.downloadStereo") || "原音左声道 + MIDI 右声道 WAV"}</span>
                  </button>
                </div>
              )}
            </div>
          ...
          {!isSeparation && (
            <button className="amt-action-btn primary" onClick={handleImportAll}>
        ...
        {/* Tab switcher — groups workbench actions for new users */}
        {!isSeparation && (
          <div className="amt-tabs-row">
            <button
              className={`amt-tab-btn ${activeTab === "edit" ? "active" : ""}`}
              onClick={() => setActiveTab("edit")}
              title={t("amt.tabEditTooltip") || "编辑 MIDI 分轨、试听合成效果"}
            >
              🎵 {t("amt.tabEdit", "编辑试听")}
            </button>
            <button
              className={`amt-tab-btn ${activeTab === "export" ? "active" : ""}`}
              onClick={() => setActiveTab("export")}
              title={t("amt.tabExportTooltip") || "一键导入工程或下载导出文件"}
            >
              📥 {t("amt.tabExport", "导出")}
            </button>
            <button
              className={`amt-tab-btn ${activeTab === "lyrics" ? "active" : ""}`}
              onClick={() => { setActiveTab("lyrics"); setLyricsOpen(true); }}
              title={t("amt.tabLyricsTooltip") || "识别并编辑歌词"}
            >
              🎤 {t("amt.tabLyrics", "歌词")}
            </button>
          </div>
        ...
        {/* Row 2 — 播放控制条：播放键居中、最突出，其余控件缩小分列两侧 */}
        <div className="amt-transport-bar">
          {/* 左侧：MIDI/原声切换 · 混音比例 · A/B 对比 */}
          <div className="amt-transport-group amt-transport-left">
            <div className="amt-source-switch">
              <button
                className={`amt-switch-btn ${playSource === "midi" ? "active" : ""}`}
                onClick={() => { setPlaySource("midi"); setMixPercent(100); }}
              >
                {isSeparation ? (t("amt.stemsPlayback") || "分轨") : "MIDI"}
              </button>
              <button
                className={`amt-switch-btn ${playSource === "original" ? "active" : ""}`}
                onClick={() => { setPlaySource("original"); setMixPercent(0); }}
              >
                {t("amt.original") || "原声"}
              </button>
            </div>

            {/* 原声↔MIDI 混音滑杆（复刻参考项目的 mix slider） */}
            <div className="amt-mix-control" title={t("amt.mixTooltip") || "原声 ↔ MIDI 混音比例"}>
              <span className="amt-mix-label">{t("amt.original") || "原声"}</span>
              <input
                type="range"
                className="amt-mix-slider"
                min={0}
                max={100}
                step={1}
                value={mixPercent}
              ...
              />
              <span className="amt-mix-label">{isSeparation ? (t("amt.stemsPlayback") || "分轨") : "MIDI"}</span>
            </div>

            <label className="amt-stereo-toggle" title={t("amt.stereoTooltip") || "左声道原声 / 右声道 MIDI 对比"}>
              <input
                type="checkbox"
                checked={stereoMode}
                onChange={(e) => setStereoMode(e.target.checked)}
              />
              {t("amt.stereoAB") || "A/B"}
            </label>
          </div>
          ...
          {/* 中间：停止 + 播放（居中、放大）+ 时间显示 */}
          <div className="amt-transport-center">
            <button className="amt-transport-btn" onClick={stopPlay} title={t("common.stop") || "Stop"}>⏹</button>
            <button className={`amt-transport-btn play ${isPlaying ? "playing" : ""}`} onClick={togglePlay}>
              {isPlaying ? "⏸" : "▶"}
            </button>
            <div className="amt-time-display">
          ...
          {/* 右侧：跟随播放头 · 工程速度 / 工程倍速 */}
          <div className="amt-transport-group amt-transport-right">
            {!isSeparation && (
              <label className="amt-follow-toggle" title={t("amt.followTooltip") || "播放时自动滚动钢琴卷帘跟随播放头"}>
                <input
                  type="checkbox"
                  checked={followMode}
            ...
            {/* 工程速度 / 工程倍速（reference bpm_spin / speed_spin 联动） */}
            {!isSeparation && detectedBpm != null && detectedBpm > 0 && (
              <div className="amt-tempo-controls">
                <label className="amt-tempo-field" title={t("amt.projBpmTooltip") || "设置试听和导出 MIDI 的 BPM。提高 BPM 会加快播放、缩短时长。"}>
                  <span className="amt-tempo-label">{t("amt.projectTempo") || "工程速度"}</span>
                  <input
                    type="number"
                    min={4}
                    max={400}
                    step={0.1}
                    value={Number(projBpm.toFixed(1))}
                    onChange={(e) => onProjBpmChange(parseFloat(e.target.value) || detectedBpm)}
                  />
                </label>
                <label className="amt-tempo-field" title={t("amt.speedTooltip") || "当前工程 BPM 对应的播放倍率（工程 BPM ÷ 检测 BPM）。修改该倍率会反向更新工程 BPM。"}>
                  <span className="amt-tempo-label">{t("amt.playbackSpeed") || "工程倍速"}</span>
                  <input
                    type="number"
                    min={0.05}
                    max={20}
                ...
                <span className="amt-tempo-status">{t("amt.tempoDetected", { bpm: Math.round(detectedBpm * 10) / 10 }) || `检测 BPM ${detectedBpm.toFixed(1)}`}</span>
              </div>
            )}
          </div>
        </div>

        {/* 底部：全宽播放进度条（任意位置可拖动定位） */}
        <div className="amt-header-progress">
          <input
            type="range"
            className="amt-progress-slider"
            min={0}
            max={duration || 1}
            step={0.01}
            value={currentTime}
      ...
      {/* Top playable master "音轨": click / drag on the waveform to seek */}
      <div className="amt-master-track">
        <div className="amt-master-track-label">
          <span className="amt-master-track-name">
            {isSeparation
              ? (playSource === "midi" ? (t("amt.stemsPlayback") || "分轨播放") : (t("amt.original") || "原声"))
              : playSource === "midi" ? (t("amt.midiMaster") || "MIDI 合成") : (t("amt.original") || "原声")}
          </span>
          <span className="amt-master-track-meta">{formatTime(currentTime)} / {formatTime(duration)}</span>
        </div>
        <div
          className="amt-master-track-canvas-wrap"
          onMouseMove={(e) => {
        ...
        >
          <canvas ref={masterCanvasRef} className="amt-master-track-canvas" />
        </div>
      </div>

      <div className="amt-result-content">
        <div className="amt-sidebar">
          <div className="amt-sidebar-header">{t("amt.tracks")}</div>
          <div className="amt-stem-list">
            {stems.map((stem, idx) => (
              <div 
                key={idx} 
                className={`amt-stem-item ${activeStemIdx === idx ? "active" : ""}`}
                onClick={() => setActiveStemIdx(idx)}
              >
                <div className="amt-stem-color-bar" style={{ backgroundColor: stem.color }} />
                <div className="amt-stem-main">
                  <div className="amt-stem-name" style={{ color: activeStemIdx === idx ? stem.color : "" }}>{stem.name}</div>
                  <div className="amt-stem-meta">
                    {stem.notes.length > 0
                      ? <>{stem.notes.length} {t("amt.notesCount")}</>
                      : (stem.audioPath ? (t("amt.audioStem") || "音频分轨") : "—")}
                  </div>
                </div>
                <div className="amt-stem-controls">
                  {isSeparation && stem.audioPath && (
                    <button
                      className="amt-mini-btn convert"
                      title={t("amt.stemToMidi") || "该分轨转 MIDI"}
                      onClick={(e) => {
                        e.stopPropagation();
                  ...
                  <button
                    className={`amt-mini-btn mute ${stem.muted ? "active" : ""}`}
                    title={t("amt.stemMuteTooltip") || "静音此乐器（点击任意静音会清除独奏）"}
                    onClick={(e) => {
                      e.stopPropagation();
                      toggleStemMute(idx);
                    }}
                  >M</button>
                  <button
                    className={`amt-mini-btn solo ${stem.soloed ? "active" : ""}`}
                    title={t("amt.stemSoloTooltip") || "独奏（静音其他乐器）"}
                    onClick={(e) => {
                      e.stopPropagation();
                      toggleStemSolo(idx);
                    }}
        ...
        {isSeparation ? (
          ...
          <div className="amt-sep-rows">
            {stems.map((stem, idx) => (
              <div
                key={idx}
                className={`amt-sep-row ${activeStemIdx === idx ? "active" : ""} ${stem.muted ? "muted" : ""}`}
                onClick={() => setActiveStemIdx(idx)}
              >
                <div className="amt-sep-row-name" style={{ color: stem.color }}>{stem.name}</div>
                <div className="amt-sep-row-wave"
                  onMouseDown={(e) => {
                    const rect = e.currentTarget.getBoundingClientRect();
                    if (rect.width <= 0) return;
                    seekMasterTrack((e.clientX - rect.left) / rect.width);
                ...
                >
                  <SepWaveCanvas
                    peaks={stemPeaks[stem.name] ?? []}
                    color={stem.color}
                    active={activeStemIdx === idx && !stem.muted}
                    playFrac={duration > 0 ? Math.min(1, currentTime / duration) : 0}
                  />
                </div>
                <div className="amt-sep-row-badge">
                  {stem.muted ? t("amt.muted") || "静音" : (stem.soloed ? "SOLO" : "WAV")}
                </div>
              </div>
            ))}
          </div>
        ) : (
        <div className="amt-piano-roll-container">
          {/* ── MIDI 编辑器工具栏（复刻参考项目 editor_panel）── */}
          <div className={`amt-editor-toolbar ${editMode ? "active" : ""}`}>
            <div className="amt-editor-row">
              <button
                className={`amt-ed-btn toggle ${editMode ? "on" : ""}`}
                onClick={() => setEditMode((v) => !v)}
                title={t("amt.editorHelp")}
              >{t("amt.editorToggle") || "编辑 MIDI"}</button>
              {editMode && (
                <>
                  <button className="amt-ed-btn" onClick={addNoteAtPlayhead}>{t("amt.editorAdd") || "新增音符"}</button>
                  <button className="amt-ed-btn" disabled={selNotes.size === 0} onClick={deleteSelection}>{t("amt.editorDelete") || "删除"}</button>
                  <button className="amt-ed-btn" disabled={undoRef.current.length === 0} onClick={undoEdit}>{t("amt.editorUndo") || "撤销"}</button>
                  <button className="amt-ed-btn" disabled={redoRef.current.length === 0} onClick={redoEdit}>{t("amt.editorRedo") || "重做"}</button>
                  <button className="amt-ed-btn" disabled={!editDirty} onClick={resetEdits}>{t("amt.editorReset") || "还原"}</button>
                  <button className="amt-ed-btn" onClick={selectAllNotes}>{t("amt.editorSelectAll") || "全选"}</button>
                  <button className="amt-ed-btn" disabled={selNotes.size === 0} onClick={cutSelection}>{t("amt.editorCut") || "剪切"}</button>
                  <button className="amt-ed-btn" disabled={selNotes.size === 0} onClick={copySelection}>{t("amt.editorCopy") || "复制"}</button>
                  <button className="amt-ed-btn" disabled={clipboardRef.current.length === 0} onClick={pasteClipboard}>{t("amt.editorPaste") || "粘贴"}</button>
                  <button className="amt-ed-btn" disabled={selNotes.size === 0} onClick={duplicateSelection}>{t("amt.editorDuplicate") || "向右复制"}</button>
                </>
              )}
              {/* 快捷键速查卡（§建议4）：「?」随时弹出，编辑模式内外都可查。 */}
              <div className="amt-editor-row-relative">
                <button
                  className={`amt-ed-btn${rollHelpOpen ? " on" : ""}`}
                  title={t("amt.rollHelpTitle")}
                  onClick={() => setRollHelpOpen((o) => !o)}
                >?</button>
                {rollHelpOpen && (
                  <div className="amt-help-card" role="dialog" aria-label={t("amt.rollHelpTitle")}>
                    <div className="amt-help-card-head">
                      <span>{t("amt.rollHelpTitle")}</span>
                      <button className="amt-help-close" onClick={() => setRollHelpOpen(false)}>×</button>
                    </div>
                    <table className="amt-help-table">
                      <tbody>
                        {([
                          ...
                          <tr key={k}>
                            <td className="amt-help-key"><kbd>{t(`amt.${kk}`)}</kbd></td>
                            <td>{t(`amt.${k}`)}</td>
                          </tr>
                        ))}
                      </tbody>
            ...
            {editMode && (
              <div className="amt-editor-row fields">
                <label className="amt-ed-field">
                  <span>{t("amt.editorQuantizeScope") || "量化范围"}</span>
                  <select
                    value={quantScope}
                    title={t("amt.editorQuantizeScopeTooltip")}
                    onChange={(e) => setQuantScope(e.target.value as "all_tracks" | "selected_notes")}
                  >
                    <option value="all_tracks">{t("amt.editorQuantizeScopeAll") || "全部轨道"}</option>
                    <option value="selected_notes">{t("amt.editorQuantizeScopeSelected") || "所选音符"}</option>
                  </select>
                </label>
                <label className="amt-ed-field">
                  <span>{t("amt.editorQuantizeGrid") || "量化网格"}</span>
                  <select
                    value={quantGrid}
                    title={t("amt.editorQuantizeGridTooltip")}
                    onChange={(e) => setQuantGrid(parseInt(e.target.value, 10))}
                  >
                ...
                <button
                  className="amt-ed-btn accent"
                  onClick={quantizeSelection}
                  disabled={quantScope === "selected_notes" && selNotes.size === 0}
                >{t("amt.editorQuantize") || "量化"}</button>
                <button
                  className="amt-ed-btn accent"
                  onClick={smartCleanup}
                  title={t("amt.editorCleanTooltip")}
                >{t("amt.editorClean") || "一键修复"}</button>
                <label className="amt-ed-field">
                  <span>{t("amt.editorInstrument") || "当前编辑乐器"}</span>
                  <select value={activeInstr} onChange={(e) => setActiveInstr(e.target.value)}>
                    {stems.filter((s) => s.notes.length > 0).map((s) => (
                      <option key={s.name} value={s.name}>{s.name}</option>
                    ))}
                  </select>
                </label>
                <label className="amt-ed-field">
                  <span>{t("amt.editorVelocity") || "力度"}</span>
                  <input
                    type="number"
                    min={1}
                    max={127}
                    value={velSpin}
                    disabled={selNotes.size === 0}
                    onChange={(e) => onVelocitySpin(Math.max(1, Math.min(127, parseInt(e.target.value, 10) || 100)))}
                  />
                </label>
                <span className="amt-ed-summary">
                  {t("amt.editorSummary", { count: stems.reduce((a, s) => a + s.notes.length, 0), changes: undoRef.current.length })}
                  {editRendering ? ` · ${t("amt.editorAudioRendering") || "正在更新试听音频…"}` : ""}
                </span>
              </div>
            )}
          </div>
          ...
          <div
            className="amt-piano-roll-viewport"
            ref={viewportRef}
            tabIndex={editMode ? 0 : -1}
            onKeyDown={rollKeyDown}
            onWheel={(e) => {
          ...
          >
            <div className="amt-piano-roll-canvas-wrap" style={{ width: PIANO_WIDTH + Math.max(1200, Math.ceil((duration || 1) * PIXELS_PER_SECOND)) }}>
              <div className="amt-piano-keys" style={{ width: PIANO_WIDTH }}>
                {pianoKeys}
              </div>
              <canvas
                ref={canvasRef}
                className={`amt-piano-roll-canvas ${editMode ? "editing" : ""}`}
                onMouseDown={rollMouseDown}
                onMouseMove={rollMouseMove}
                onMouseUp={rollMouseUp}
                onMouseLeave={rollMouseUp}
                onDoubleClick={rollDoubleClick}
                onContextMenu={rollContextMenu}
                onClick={(e) => {
                  if (editMode) return; // editing clicks are handled by rollMouseDown
                  const rect = e.currentTarget.getBoundingClientRect();
                  const x = e.clientX - rect.left;
              ...
              {lyricEdit && (
                <input
                  key={lyricEdit.noteId}
                  className="amt-lyric-input"
                  style={{ left: PIANO_WIDTH + Math.max(8, lyricEdit.x - 24), top: Math.max(0, lyricEdit.y - 6) }}
                  defaultValue={lyricEdit.value}
                  autoFocus
                  onKeyDown={(e) => {
                    if (e.key === "Enter") commitRollLyric((e.target as HTMLInputElement).value);
                    else if (e.key === "Escape") setLyricEdit(null);
                  }}
                  onBlur={(e) => commitRollLyric(e.target.value)}
                />
              )}
            </div>
          </div>

          {/* Zoom controls: scale the roll in/out; scrollbars cover up/down + left/right. */}
          <div className="amt-zoom-toolbar">
            <button
              className="amt-zoom-btn"
              title={t("amt.zoomOut") || "缩小"}
              onClick={() => setZoom((z) => clampZoom(z / 1.25))}
            >−</button>
            <span className="amt-zoom-value">{Math.round(zoom * 100)}%</span>
            <button
              className="amt-zoom-btn"
              title={t("amt.zoomIn") || "放大"}
              onClick={() => setZoom((z) => clampZoom(z * 1.25))}
            >+</button>
            <button
              className="amt-zoom-btn"
              title={t("amt.zoomReset") || "重置缩放"}
              onClick={() => setZoom(1)}
            >↺</button>
          </div>

          {preparingAudio && (
            <div className="amt-prep-badge">
              <span className="amt-spinner-sm" />
              {t("amt.preparingPlayback") || "正在合成播放音频..."}
            </div>
          )}
        </div>
        )}
      ...
      {/* Lyrics drawer (P0-B): slides in from the right over the workbench. */}
      {lyricsOpen && !isSeparation && (
        <AmtLyricsPanel result={result} onClose={() => setLyricsOpen(false)} onSavedToMidi={syncLyricsFromMidi} />
      )}

      {/* 音符右键菜单（右键菜单统一）：常用操作就近出现，不再藏在工具栏里。 */}
      {rollMenu && (
        <ContextMenu x={rollMenu.x} y={rollMenu.y} items={rollMenuItems()} onClose={() => setRollMenu(null)} />
      )}
    </div>
  );
}
