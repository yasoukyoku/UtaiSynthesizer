...
function headlessDeposit(trackId, segmentId, workflow) {
    if (depositing.has(segmentId))
        return;
...
export function RenderLinkWatcher() {
    ...
    useEffect(() => {
        ...
        for (const [segId, exec] of Object.entries(executions)) {
            if (exec.status === "running" || segId === workflowSegmentId || renderLinks[segId])
                continue;
            const loc = locate(segId);
            if (!loc)
                continue;
            const outs = loc.seg.processedOutputs;
            if (!outs?.some((o) => o.loading) && !hasUndepositedCache(segId, loc.seg.workflow, outs))
                continue;
            headlessDeposit(loc.trackId, segId, loc.seg.workflow);
        }
        if (changed)
            useAudioStore.getState().bumpSchedule(); // live playback follows the new lanes
