...
async function uniqueFilePath(dir, base, ext) {
...
function PngThumb({ path }) {
    ...
    if (!url)
        return null;
    return _jsx("img", { className: "wf-preview-thumb", src: url, alt: "" });
}
...
export function NodeOutputPreview({ statusSeg, nodeId, outputLabels, }) {
    ...
    if (!outputs || outputs.every((p) => !p))
        return null;
    const toggle = async (idx, path) => {
    ...
    const download = async (path, label) => {
        ...
        try {
            if (/\.(mid|png|json)$/i.test(path)) {
                ...
                await exportOneAudioFileToFolder({ label: label ?? nodeId, sourcePath: path }, out, label ? `${nodeId}_${label}` : nodeId);
            }
            useAppStore
                .getState()
                .showToast(t18({ zh: "已下载", en: "Downloaded", ja: "ダウンロードしました" }, i18n.language), "success");
        }
        catch (e) {
            useAppStore.getState().showToast(laneExportErrorMessage(e), "error");
        }
    };
    return (_jsx("div", { className: "wf-preview nodrag", onPointerDown: (e) => e.stopPropagation(), children: outputs.map((path, i) => {
            if (!path)
                return null; // sparse rehydrated slot
            ...
            if (isJson) {
                return (_jsxs("div", { className: "wf-preview-row", children: [_jsx("span", { className: "wf-preview-btn", style: { opacity: 0.7 }, children: "{ }" }), outputLabels?.[i] && (_jsx("span", { className: "wf-preview-label", title: outputLabels[i], children: outputLabels[i] })), _jsx("span", { className: "wf-preview-scrub", style: { opacity: 0.6 }, children: t18({ zh: "JSON 报告", en: "JSON report", ja: "JSON レポート" }, i18n.language) }), _jsx("button", { className: "wf-preview-dl", title: t18({ zh: "复制报告 JSON", en: "Copy report JSON", ja: "レポート JSON をコピー" }, i18n.language), onClick: () => {
                                void navigator.clipboard
                                    .writeText(path)
                                    .then(() => useAppStore
                                    .getState()
                                    .showToast(t18({ zh: "已复制", en: "Copied", ja: "コピーしました" }, i18n.language), "success"))
                                    .catch(() => useAppStore
                                    .getState()
                                    .showToast(t18({ zh: "复制失败", en: "Copy failed", ja: "コピーに失敗しました" }, i18n.language), "error"));
                            }, children: "\u29C9" })] }, i));
            }
            if (isPng) {
                return (_jsxs("div", { className: "wf-preview-row wf-preview-row-png", children: [_jsx(PngThumb, { path: path }), outputLabels?.[i] && (_jsx("span", { className: "wf-preview-label", title: outputLabels[i], children: outputLabels[i] })), _jsx("button", { className: "wf-preview-dl", title: t18({ zh: "下载此图片", en: "Download this image", ja: "この画像をダウンロード" }, i18n.language), onClick: () => void download(path, outputLabels?.[i]), children: "\u2B07" })] }, i));
            }
            if (isMidi) {
                return (_jsxs("div", { className: "wf-preview-row", children: [_jsx("span", { className: "wf-preview-btn", style: { opacity: 0.7 }, children: "\u266A" }), outputLabels?.[i] && (_jsx("span", { className: "wf-preview-label", title: outputLabels[i], children: outputLabels[i] })), _jsx("span", { className: "wf-preview-scrub", style: { opacity: 0.6 }, children: t18({ zh: "MIDI 文件", en: "MIDI file", ja: "MIDI ファイル" }, i18n.language) }), _jsx("button", { className: "wf-preview-dl", title: t18({ zh: "打开 MIDI 工作台", en: "Open MIDI workbench", ja: "MIDI ワークベンチを開く" }, i18n.language), onClick: () => useAmtStore.getState().openWorkbench(nodeId), children: "\u2699" }), _jsx("button", { className: "wf-preview-dl", title: t18({ zh: "下载此 MIDI", en: "Download this MIDI", ja: "この MIDI をダウンロード" }, i18n.language), onClick: () => void download(path, outputLabels?.[i]), children: "\u2B07" })] }, i));
            }
            return (_jsxs("div", { className: "wf-preview-row", children: [_jsx("button", { className: `wf-preview-btn ${isActive && phase !== "idle" ? "on" : ""}`, onClick: () => void toggle(i, path), children: glyph }), outputLabels?.[i] && (_jsx("span", { className: "wf-preview-label", title: outputLabels[i], children: outputLabels[i] })), _jsx(Scrubber, { className: "wf-preview-scrub", value: isActive && dur > 0 ? Math.min(1, pos / dur) : 0, onSeek: (frac) => {
                            if (isActive && dur > 0) {
                                preview.seek(frac);
                                setPos(preview.position);
                            }
                        ...
                        } }), _jsx("span", { className: "wf-preview-time", children: isActive && dur > 0 ? `${fmt(pos)}/${fmt(dur)}` : "" }), _jsx("button", { className: "wf-preview-dl", title: t18({ zh: "下载此音频", en: "Download this audio", ja: "この音声をダウンロード" }, i18n.language), onClick: () => void download(path, outputLabels?.[i]), children: "\u2B07" })] }, i));
        }) }));
}
