    ...
    return groups.map(group => {
      const filteredNodes = group.nodes.filter(node => {
        ...
        for (const [word, pinyin] of Object.entries(pinyinMap)) {
          if (label.includes(word) && pinyin.includes(query)) return true;
        }
    ...
    <aside className="node-palette">
      ...
      <div className="palette-search">
        <input
          ...
          placeholder={t("workflow.searchNodes") || "搜索节点..."}
          value={searchQuery}
        ...
        {searchQuery && (
          <button
            ...
            title={t("workflow.clearSearch") || "清除搜索"}
          >
            ✕
          </button>
        )}
      ...
      {filteredGroups.length === 0 && searchQuery && (
        <div className="palette-no-results">
          {t("workflow.noSearchResults") || "未找到匹配的节点"}
        </div>
      )}
