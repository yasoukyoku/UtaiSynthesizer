...
export const LossChart = forwardRef(function LossChart({ history, bestStep, height = 220 }, ref) {
    ...
    const isOn = (key) => enabled[key] ?? true;
    ...
    useEffect(() => {
        ...
        if (yMax <= 0)
            yMax = 1;
        yMax *= 1.05;
        const xOf = (step) => PAD_L + ((step - x0) / (x1 - x0)) * plotW;
    ...
    return (_jsxs("div", { className: "loss-chart", ref: wrapRef, children: [_jsx("canvas", { ref: canvasRef }), _jsx("div", { className: "loss-chart-legend", children: SERIES.map((s) => (_jsxs("button", { className: `loss-legend-chip ${isOn(s.key) ? "on" : ""}`, onClick: () => setEnabled((e) => ({ ...e, [s.key]: !(e[s.key] ?? true) })), children: [_jsx("span", { className: "loss-legend-swatch", style: { background: `var(${s.varName}, ${s.fallback})` } }), s.key] }, s.key))) })] }));
});
