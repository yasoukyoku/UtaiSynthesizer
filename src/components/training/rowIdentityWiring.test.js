...
describe("archive rows resolve their identity per row", () => {
    it("★ every audition render is handed THIS ROW's workspace, not a slot-level scalar", async () => {
        ...
        for (const cmd of [
            ...
            expect(window, `${cmd} must pass the row-resolved workspace — a slot-level one renders run B's row ` +
                `from run A's audition cache, and the cache hit predicate is only「model.json 在不在」`).toContain("workspace: auditionWs");
        }
    });
    it("★ the suggested import name is never taken from a slot-level scalar", async () => {
        ...
        expect(/\bconst exportName\b/.test(code) || /\bconst exportWorkspace\b/.test(code), "the slot-level export identity is back: two runs of one slot then propose the SAME model " +
            "name, and import_model replaces by name with no dialog").toBe(false);
        // every CALL to suggestedName must pass the row-resolved name as its second argument
        const calls = [...code.matchAll(/suggestedName\(([^)]*)\)/g)].filter((m) => !code.slice(Math.max(0, m.index - 40), m.index).includes("const suggestedName ="));
    ...
    it("★ a same-name install is never silent — on either import path", async () => {
        ...
        expect(code.slice(single, singleInvoke), "the single-row import reaches import_model without asking about a same-name replace").toContain("confirmReplaceInstalled(name)");
        ...
        expect(code.slice(batch, batchInvoke), "the batch import's confirmation does not say which rows would replace an installed model").toContain("importReplaceBody");
    });
    ...
    function callWindow(code, cmd, from) {
            ...
            for (const cmd of ["get_training_slot_info", "get_slot_export_context"]) {
                ...
                for (;;) {
                    ...
                    expect(window.length, `${file}: ${cmd} 的实参窗口长得不像一次调用(${window.length} 字符)`)
                        .toBeLessThan(1500);
                    expect(window, `${file}: ${cmd} 少传了 runId —— 两个 run 之后它问的是一个没有答案的问题(RUN_AMBIGUOUS)`).toContain("runId");
                    from = at + 1;
                }
            }
        }
    ...
    it("★ S141 §E2E-M3/M4/M5 —— 槽卡片的决策留在纯模块里,不许再长回组件体", async () => {
        ...
        expect(code, "ProjectDetail 不再从 lib/training/slotRows 取决策 —— 那几个决策要么被删了,要么被就地写回了").toContain("lib/training/slotRows");
        ...
        expect(/slotStarted\(runs,/.test(code), "`slotStarted` 不再吃全部行 —— 折叠一旦影响到它,收起来的槽会自称「尚未开始」").toBe(true);
    });
    it("★ S141 §E2E-M6/M7 —— 三条导入路的 toast 决策留在纯模块里,不许再长回闭包", async () => {
        ...
        const fs = await importFs();
        const code = codeOnly(fs.readFileSync(FILE, "utf8"));
        expect(code, "TrainingPage 不再从 lib/training/importToast 取决策").toContain("lib/training/importToast");
        ...
        expect([...code.matchAll(/collectWarningCodes\(/g)].length, "collectWarningCodes 的调用点少于三处 —— 三条导入路里有一条又不走同一个漏斗了").toBeGreaterThanOrEqual(3);
    });
    it("★ S142 §E2E-M11 —— 每一条走进参数页的路,都必须先把表单放回这个 run 已经在的地方", async () => {
        ...
        const fs = await importFs();
        const code = codeOnly(fs.readFileSync(PROJECT_DETAIL, "utf8"));
        expect(code, "ProjectDetail 不再从 lib/training/formForSlot 取决策").toContain("lib/training/formForSlot");
        // 具体形状排前面(S108):「同一条规则的第五份副本又长回来了」是诊断。
        expect(/diff_steps\s*>\s*0/.test(code), "ProjectDetail 又自己写了一份 `diff_steps > 0` —— 那正是 S128 收编掉的第五份副本").toBe(false);
        ...
        for (const h of hops) {
            ...
            expect(window, "这条进参数页的路没有摊开 formFor —— costly 那一档会被打回默认,下一次运行静默换池重跑").toContain("...formFor");
        }
    });
    it("★ S142 §E2E-M10 —— 代价提示还在屏幕上,而且每个控件都挂着它自己的那一条", async () => {
        ...
        const fs = await importFs();
        const code = codeOnly(fs.readFileSync(FILE, "utf8"));
        expect(code, "TrainingPage 不再从 lib/training/costlyNote 取决策").toContain("lib/training/costlyNote");
        ...
        for (const backend of backends) {
            for (const id of poolCostFieldIds(backend, true)) {
                if (NO_FORM_CONTROL.has(id))
                    continue; // `dataset` 归数据页,参数页没有控件代表它
                ...
                expect(window, `${backend}/${id}:${key} 这个控件旁边没有代价提示 —— 改它会静默换池、重跑整份预处理`).toContain(`costlyNote("${id}")`);
            }
        }
        // 反过来:多出来的挂点也要有人问一句(挂在一个**不**换池的字段上 = 屏幕在撒谎)。
        // 期望值由上面那张表推出,不是手写的常量。
        expect([...code.matchAll(/\{costlyNote\("/g)].length, "costlyNote 的挂点数与锁表推出来的控件数对不上").toBe(wantedKeys.size);
    });
    it("★ S141 —— 「再训一个」这条路不许再弹第三个对话框问「模型已存在」", async () => {
        ...
        // ⑴ 「再训一个」必须**短路**在弹框之前。
        expect(/if \(wantsRetrain\)\s*\{[\s\S]{0,200}fresh = true;/.test(window), "「再训一个」这条路又落进那个『模型已存在』对话框了 —— 用户已经答过两次,而这一问的标题" +
            "对他刚起的新名字是假话,且它已经没有任何东西要征求同意(wipe_confirmed 只对 sovits_diff 有效)").toBe(true);
        // ⑵ 而且**不许**在那条路上顺手把擦除同意也给了:后端今天读不到它,但将来分类若变,
        //    「没同意过」是 fail-closed 的方向。
        expect(/if \(wantsRetrain\)\s*\{[\s\S]{0,200}wipeConfirmed = true/.test(window), "「再训一个」这条路自作主张地把 wipeConfirmed 置成了 true —— 那是把一道 fail-closed 的" +
            "保险默认打开").toBe(false);
    ...
    it("★★ S143 §E2E-M25 ⑶ —— 四颗 per-run 按钮的 disabled 真的接到了那个决策上", async () => {
        ...
        const fs = await importFs();
        const code = codeOnly(fs.readFileSync(PROJECT_DETAIL, "utf8"));
        expect(code, "决策模块不再被引用 —— 四颗按钮又变回裸的布尔了").toContain('from "../../lib/training/liveRun"');
        expect(code, "行级门禁不再被调用").toContain("runRowActions({");
        // ★ 逐颗按钮:`disabled=` 里必须出现它自己那一档,而不是任何一个裸标志。
        //   期望是**由那四个字段名推出来的**,不是手抄一串 —— 手抄的那一份打错字会两边一起绿。
        for (const which of ["cont", "retrain", "del", "rename"]) {
            expect(code, `${which} 那颗按钮的 disabled 没有接到 runRowActions 上`).toContain(`disabled={gates.${which}.disabled}`);
        ...
        expect(starts.length, "槽级/浅扩散的「开始训练」没有全部接到同一条门禁上 —— 那条死路还留着入口").toBe(4);
        // ★ 订阅本身:载体接进来了、而这一页仍然收不到快照变化 ⇒ 徽章/禁用会对着一份挂载时的
        //   旧数据工作,**而所有纯模块判据照绿**。这是这一笔唯一能看见「它有没有真的活起来」的地方。
        expect(code, "这一页又不订阅实时快照了 —— 训练开始/结束时它连重渲染都不会发生").toContain("useTrainingStore((s) => s.snapshot.state)");
        // ⛔ 而且必须是**标量** selector:订整个 snapshot 会让整片卡片墙按训练步频率重渲染。
        expect(code, "订了整个 snapshot 对象 —— store 每一步都换一个新对象").not.toMatch(/useTrainingStore\(\(s\) => s\.snapshot\)/);
    ...
    it("★★ S143 §E2E-M25 ⑵ —— 屏幕真的说出了「正在跑的是哪一个」", async () => {
        ...
        expect(pd, "run 行不再问「这个槽里哪一行在跑」").toContain("liveRunIdFor(liveFacts, projectId, f)");
        // ⛔⛔ 比较必须是 `=== r.id`,**不能**是真值判断:`""` 是一个合法答案(未迁移槽的槽根
        //    就是那个 run),写成 `liveRow && …` 会让那种槽的唯一一行**永远不亮**,
        //    而所有 `r0/r1` 形状的夹具都看不见这一点。
        expect(pd, "徽章的条件不是逐字节比对 run id —— 空串那一档会被真值判断吃掉").toContain("liveRow === r.id");
        expect(pd, "徽章用了真值判断(`liveRow &&`)—— 未迁移槽那一行会永远不亮").not.toMatch(/liveRow\s*&&/);
        ...
        expect(badges.length, "「训练中」徽章的挂点不是两个(run 行 + 浅扩散卡)—— 有一处被删了或多长了一处").toBe(2);
        ...
        expect(tp, "页头那盏灯不再比 project_id —— 站在项目 B 时它会为项目 A 亮着而不说").toContain("snapshot.project_id !== route.projectId");
        ...
        for (const lang of ["zh", "en", "ja"]) {
            ...
            for (const k of ["activeNamed", "activeOther"]) {
                expect(json.training?.[k], `${lang}.json 的 training.${k} 没有 {{name}} 占位符`).toContain("{{name}}");
            }
        }
    });
    it("★★★ S143 §E2E-M25 ⑷ —— 排序落在对的那一跳,而浅扩散宿主**不跟它走**", async () => {
        ...
        expect(code, "展示序不再由 sortRunRows 决定").toContain("sortRunRows(visibleRuns(runs, liveRow), liveRow)");
        ...
        //    自造 slot,够不到组件里的这一跳)。
        expect(code, "浅扩散宿主的实参被换掉了 —— 它必须吃后端原序那一份").toContain("pickDiffHost(sovitsSlot)");
    ...
    it("★★ S143 笔 5 —— 改名那条路也有同名闸,而互锁拒绝不再显示成「错误」", async () => {
        ...
        expect(code, "改名对话框又变回只判空了").toContain("newRunNameProblem(v, slots.get(family)?.runs ?? [], run.id)");
