...
import { useTrainingStore, trainingDataOk, } from "../../store/training";
import { formForSlot } from "../../lib/training/formForSlot";
import { foldRunRows, newRunNameProblem, pickDiffHost, prepPoolLine, slotStarted, sortRunRows, startedRun, visibleRuns, } from "../../lib/training/slotRows";
import { diffusionIsLive, gateReasonKey, liveRunIdFor, runRowActions, trainingIsLive, } from "../../lib/training/liveRun";
...
const FAMILIES = ["rvc", "sovits", "sovits_v2", "vocoder"];
...
export function ProjectDetail() {
    ...
    const toastLevel = (e) => (isBusyError(e) ? "info" : "error");
    ...
    const addFilesTo = async (speaker) => {
    ...
    const renameRun = async (family, run) => {
        const typed = await showConfirm({
            ...
            input: {
                ...
                invalid: (v) => {
                    ...
                    if (problem === "empty")
                        return t("backend.TRAINING_NAME_EMPTY");
                    if (problem === "taken")
                        return t("backend.TRAINING_NAME_TAKEN");
                    return null;
                },
            },
        });
        if (!typed || typed === "__cancel")
            return;
        if (typed.trim() === (run.modelName ?? ""))
            return;
        setBusy(true);
        try {
    ...
    const deleteRun = async (family, run) => {
        ...
        setBusy(true);
        try {
            const report = await invoke("training_delete_run", {
                projectId,
                family,
                runId: run.id,
            });
    ...
    const load = useCallback(async () => {
        if (!projectId)
            return;
        try {
            const d = await invoke("get_training_project", { projectId });
            setDetail(d);
    ...
    const formFor = (family, run, pin) => formForSlot(family, run?.info, pin);
     ...
     *  之前就知道会发生什么。 */
    const retrainFamily = async (family, run) => {
        ...
        const versions = family === "sovits" ? ["4.1", "4.0"] : [];
        const choice = await showConfirm({
            ...
            buttons: [
                ...
                ...(versions.length > 0
                    ...
                    : [{ id: "go", label: t("training.slotRetrain"), kind: "danger" }]),
            ],
        });
        if (choice !== "go" && !versions.includes(choice))
            return;
        const pin = versions.includes(choice) ? choice : undefined;
            ...
            input: {
                ...
                invalid: (v) => {
                    ...
                    if (problem === "empty")
                        return t("backend.TRAINING_NAME_EMPTY");
                    if (problem === "taken")
                        return t("training.newRunNameTaken");
                    return null;
                },
            },
        });
        if (!newName || newName === "__cancel")
            return;
    ...
    const startDiff = async (version, run) => {
        const runName = await askRunName(run);
        if (runName === null)
            return;
    ...
    const shell = (body) => (_jsxs("div", { className: "tproj-detail", children: [_jsxs("div", { className: "tproj-detail-head", children: [_jsxs("button", { className: "tproj-back", onClick: () => useTrainingStore.getState().enterProject(""), children: ["\u2190 ", t("training.projectBack")] }), _jsx("span", { className: "tproj-detail-name", children: detail?.name ?? projectId })] }), body] }));
    if (error) {
        return shell(_jsxs(_Fragment, { children: [_jsx("div", { className: "training-name-exists", children: error }), _jsx("div", { className: "tproj-slot-actions", children: _jsx("button", { className: "training-btn small", onClick: () => void load(), children: t("training.projectRetry") }) })] }));
    }
    if (!detail)
        return shell(_jsx("div", { className: "training-empty", children: t("training.projectLoading") }));
    ...
    return shell(_jsxs(_Fragment, { children: [detail.note && _jsx("div", { className: "tproj-note", children: detail.note }), detail.needsAttention && (_jsx("div", { className: "training-name-exists", children: backendErrorMessage(detail.needsAttention) ?? t("training.projectNeedsAttention") })), _jsxs("div", { className: "tproj-section", children: [_jsxs("div", { className: "tproj-section-head", children: [_jsx("span", { className: "tproj-section-title", children: t("training.projectDataset") }), _jsxs("div", { className: "tproj-ds-head-actions", children: [detail.dataset.groups.length === 0 && (_jsxs("button", { className: "training-btn small", disabled: !!detail.needsAttention || busy, onClick: () => void addFilesTo(), children: ["\uFF0B ", t("training.addFiles")] })), _jsx("button", { className: "training-btn small", disabled: !!detail.needsAttention || busy, onClick: () => setRoute({ seg: "data", projectId }), children: detail.dataset.files > 0
                                            ? t("training.projectReimportData")
                                            : t("training.projectImportData") })] })] }), detail.dataset.files > 0 ? (_jsxs(_Fragment, { children: [_jsxs("div", { className: "tproj-dataset", children: [_jsx("span", { children: t("training.projectDatasetFiles", { count: detail.dataset.files }) }), _jsx("span", { className: "tproj-dot", children: "\u00B7" }), _jsx("span", { children: fmtSize(detail.dataset.bytes) }), detail.dataset.speakers.length > 0 && (_jsxs(_Fragment, { children: [_jsx("span", { className: "tproj-dot", children: "\u00B7" }), _jsx("span", { children: t("training.projectDatasetSpeakers", { count: detail.dataset.speakers.length }) })] }))] }), detail.dataset.groups.length > 0 && (_jsxs("div", { className: "tproj-ds-block", children: [_jsx("div", { className: "tproj-ds-sub", title: t("training.projectDatasetOrderNote"), children: t("training.projectDatasetStructure") }), detail.dataset.groups.map((g, i) => (_jsxs("div", { className: "tproj-ds-spk-row", children: [detail.dataset.orderKnown && (_jsx("span", { className: "training-spk-idx", children: i })), _jsx("span", { className: "tproj-ds-spk-name", title: g.slug, children: g.name || g.slug }), _jsxs("span", { className: "training-file-dur", children: [t("training.projectDatasetFiles", { count: g.files }), " \u00B7 ", fmtSize(g.bytes)] }), _jsx("button", { className: "training-btn small", disabled: !!detail.needsAttention || busy, onClick: () => void addFilesTo(g.name || g.slug), title: t("training.addFiles"), children: "\uFF0B" })] }, g.slug))), !detail.dataset.orderKnown && (_jsx("div", { className: "tproj-ds-note", children: t("training.projectDatasetOrderUnknown") }))] })), _jsxs("button", { className: "training-btn small tproj-ds-toggle", onClick: () => setShowFiles((v) => !v), "aria-expanded": showFiles, children: [showFiles ? "▼" : "▶", " ", t("training.projectDatasetFileList")] }), showFiles && (_jsxs(_Fragment, { children: [_jsx("div", { className: "training-file-list tproj-ds-files", children: detail.dataset.entries.map((e) => {
                                            const cut = e.rel.indexOf("/");
                                            const slug = cut > 0 ? e.rel.slice(0, cut) : "";
                                            const owner = slug
                                                ? detail.dataset.groups.find((g) => g.slug === slug)
                                                : undefined;
                                            return (_jsx(PreviewFileRow, { p: filePreview, 
                                                // `rel` is forward-slashed by contract; Windows takes it as-is
                                                path: `${detail.dataset.datasetDir}/${e.rel}`, title: e.rel, lead: slug ? (_jsx("span", { className: "tproj-ds-file-spk", children: owner?.name || slug })) : undefined, 
                                                ...
                                                name: e.name || e.rel, meta: fmtSize(e.bytes), onRemove: busy ? undefined : () => void removeFile(e.rel, e.name || e.rel) }, e.rel));
                                        }) }), detail.dataset.entries.some((e) => !e.name) && (_jsx("div", { className: "tproj-ds-note", children: t("training.projectDatasetNamesMissing", {
                                            count: detail.dataset.entries.filter((e) => !e.name).length,
                                        }) }))] }))] })) : (_jsx("div", { className: "tproj-dataset empty", children: t("training.projectDatasetEmpty") }))] }), _jsxs("div", { className: "tproj-section", children: [_jsx("div", { className: "tproj-section-head", children: _jsx("span", { className: "tproj-section-title", children: t("training.projectSlots") }) }), _jsxs("div", { className: "tproj-slots", children: [FAMILIES.map((f) => {
                                ...
                                return (_jsxs("div", { className: `tproj-slot ${started ? "started" : ""}`, children: [_jsxs("div", { className: "tproj-slot-head", children: [_jsx("span", { className: "tproj-slot-name", children: t(FAMILY_TEXT[f].label) }), runs.length > 1 && (_jsx("span", { className: "tproj-slot-ver", children: t("training.slotRunCount", { count: runs.length }) }))] }), _jsx("div", { className: "tproj-slot-desc", children: t(FAMILY_TEXT[f].desc) }), !started && (_jsx("div", { className: "tproj-slot-facts", children: _jsx("span", { children: t("training.slotNotStarted") }) })), rows.map((r) => {
                                            ...
                                            return (_jsxs("div", { className: "tproj-run", children: [_jsxs("div", { className: "tproj-run-head", children: [_jsxs("span", { className: "tproj-run-name", children: [r.modelName
                                                                        ? t("training.runNameFrozen", { name: r.modelName })
                                                                        : t("training.runUnnamed"), _jsx("button", { className: "tproj-run-rename", disabled: gates.rename.disabled, title: gateTitle(gates.rename, t("training.runRename")), onClick: () => void renameRun(f, r), children: "\u270E" })] }), r.info.version && _jsx("span", { className: "tproj-slot-ver", children: r.info.version }), liveRow === r.id && (_jsx("span", { className: "tproj-run-live", children: t("training.runTraining") }))] }), _jsxs("div", { className: "tproj-slot-facts", children: [_jsx("span", { children: r.hasResumePoint
                                                                    ? r.resumeStep != null
                                                                        ? t("training.slotResume", { step: r.resumeStep })
                                                                        : t("training.slotResumeLatest")
                                                                    : t("training.slotNoResume") }), _jsx("span", { className: "tproj-dot", children: "\u00B7" }), _jsx("button", { className: "tproj-archive-link", disabled: blocked, onClick: () => openArchive(f), title: t("training.archiveOpen"), children: t("training.slotArchive", { size: fmtSize(r.ckptBytes) }) })] }), r.info.speakers.length > 1 && (_jsxs("div", { className: "tproj-slot-spk", children: [_jsx("span", { className: "tproj-ds-sub", title: t("training.projectDatasetOrderNote"), children: t("training.slotSpeakers") }), r.info.speakers.map((n, i) => (_jsxs("span", { className: "tproj-slot-spk-item", children: [_jsx("span", { className: "training-spk-idx", children: i }), _jsx("span", { className: "tproj-slot-spk-name", children: n })] }, `${i}:${n}`)))] })), _jsxs("div", { className: "tproj-slot-actions", children: [_jsx("button", { className: "training-btn small primary", disabled: gates.cont.disabled, title: gateTitle(gates.cont), onClick: () => void startFamily(f, r), children: t("training.slotContinue") }), _jsx("button", { className: "training-btn small", disabled: gates.retrain.disabled, title: gateTitle(gates.retrain), onClick: () => void retrainFamily(f, r), children: t("training.slotRetrain") }), r.id !== "" && (_jsx("button", { className: "training-btn small danger", disabled: gates.del.disabled, title: gateTitle(gates.del), onClick: () => void deleteRun(f, r), children: t("training.runDelete") }))] })] }, r.id));
                                        }), (fold.hidden > 0 || runsOpen[f]) && (_jsx("button", { className: "tproj-runs-toggle", onClick: () => setRunsOpen((m) => ({ ...m, [f]: !m[f] })), children: runsOpen[f]
                                                ? `▾ ${t("training.runsFoldLess")}`
                                                : `▸ ${t("training.runsFoldMore", { count: fold.hidden })}` })), prepPools.show && (_jsx("div", { className: "tproj-slot-facts", children: _jsx("span", { title: t("training.slotPrepPoolsHint"), children: t("training.slotPrepPools", {
                                                    count: prepPools.count,
                                                    size: fmtSize(prepPools.bytes),
                                                }) }) })), !started && (_jsx("div", { className: "tproj-slot-actions", children: f === "sovits" ? (
                                            ...
                                            ["4.1", "4.0"].map((v) => (_jsx("button", { className: "training-btn small", disabled: slotStart.disabled, title: gateTitle(slotStart), onClick: () => void startFamily("sovits", runs[0], v), children: t("training.slotStartVersion", { version: v }) }, v)))) : (_jsx("button", { className: "training-btn small primary", disabled: slotStart.disabled, title: gateTitle(slotStart), onClick: () => void startFamily(f, runs[0]), children: t("training.slotStart") })) }))] }, f));
                            }), _jsxs("div", { className: `tproj-slot ${diffSteps > 0 ? "started" : ""}`, children: [_jsxs("div", { className: "tproj-slot-head", children: [_jsx("span", { className: "tproj-slot-name", children: t("training.backendDiff") }), sovitsVersionPinned && (_jsx("span", { className: "tproj-slot-ver", children: sovitsVersionPinned }))] }), _jsx("div", { className: "tproj-slot-desc", children: t("training.backendDiffDesc") }), diffusionIsLive(liveFacts, projectId) && (_jsx("div", { className: "tproj-slot-facts", children: _jsx("span", { className: "tproj-run-live", children: t("training.runTraining") }) })), _jsx("div", { className: "tproj-slot-facts", children: diffSteps > 0 ? (_jsx("span", { children: t("training.slotResume", { step: diffSteps }) })) : (_jsx("span", { children: t("training.slotNotStarted") })) }), _jsx("div", { className: "tproj-slot-actions", children: sovitsVersionPinned ? (_jsx("button", { className: "training-btn small primary", disabled: slotStart.disabled, title: gateTitle(slotStart), onClick: () => void startDiff(sovitsVersionPinned, diffHost), children: diffSteps > 0 ? t("training.slotContinue") : t("training.slotStart") })) : (
                                        ...
                                        ["4.1", "4.0"].map((v) => (_jsx("button", { className: "training-btn small", disabled: slotStart.disabled, title: gateTitle(slotStart), onClick: () => void startDiff(v, diffHost), children: t("training.slotStartVersion", { version: v }) }, v)))) }), _jsx("div", { className: "tproj-slot-note", children: t("training.diffAttachLater") })] })] })] }), _jsxs("div", { className: "tproj-section", children: [_jsx("div", { className: "tproj-section-head", children: _jsx("span", { className: "tproj-section-title", children: t("training.projectExported") }) }), detail.exported.length === 0 ? (_jsx("div", { className: "tproj-dataset empty", children: t("training.projectExportedNone") })) : (_jsx("div", { className: "tproj-exported", children: detail.exported.map((e) => (_jsxs("div", { className: `tproj-export-row ${e.installed && !e.sourceDeleted ? "" : "gone"}`, children: [_jsx("span", { className: "tproj-export-name", children: e.name }), _jsx("span", { className: "tproj-export-type", children: e.modelType }), _jsx("span", { className: "tproj-export-src", title: e.fromCkptRel, children: e.fromCkptRel }), !e.installed && (_jsx("span", { className: "tproj-export-gone", children: t("training.exportedDeleted") })), e.sourceDeleted && (_jsx("span", { className: "tproj-export-gone", children: t("training.exportedSourceGone") }))] }, `${e.name}:${e.fromCkptRel}`))) }))] })] }));
}
