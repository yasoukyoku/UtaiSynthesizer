...
type Block =
  | { kind: "p"; text: I18nText }
  | { kind: "steps"; items: I18nText[] }
  | { kind: "tip"; text: I18nText }
  | { kind: "warn"; text: I18nText };
...
export function UserGuideDialog({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="ug-overlay" onClick={onClose}>
      <div className="ug-panel" onClick={(e) => e.stopPropagation()}>
        <div className="ug-head">
          <span className="ug-title">📖 {t18({ zh: "使用说明（新手友好版）", en: "User Guide (Beginner-Friendly)", ja: "使い方ガイド（初心者向け）" }, lang)}</span>
            ...
            {SECTIONS.map((s) => (
              <section key={s.id} id={`ug-${s.id}`} className="ug-section">
                <h3 className="ug-section-title">
                  <span className="ug-section-icon">{s.icon}</span>
                  {t18(s.title, lang)}
                </h3>
                {s.blocks.map((b, i) => {
                  if (b.kind === "p") {
                    return (
                      <p key={i} className="ug-p">
                        {t18(b.text, lang).split("\n").map((line, j) => (
                          <span key={j} className="ug-line">{line}</span>
                        ))}
                      </p>
                    );
                  }
                  if (b.kind === "steps") {
                    return (
                      <ol key={i} className="ug-steps">
                        {b.items.map((it, j) => (
                          <li key={j} className="ug-step">{t18(it, lang)}</li>
                        ))}
                      </ol>
                    );
                  }
                  return (
                    <div key={i} className={`ug-callout ${b.kind === "warn" ? "warn" : "tip"}`}>
                      <span className="ug-callout-icon">{b.kind === "warn" ? "⚠️" : "💡"}</span>
                      <span>{t18(b.text, lang)}</span>
                    </div>
                  );
                })}
              </section>
        ...
        <div className="ug-foot">
          {t18({ zh: "按 Esc 或点击空白处关闭 · 更多帮助：QQ 202112", en: "Esc or click outside to close · More help: QQ 202112", ja: "Esc または外側クリックで閉じる · QQ 202112" }, lang)}
        </div>
      </div>
    </div>
  );
