...
/**
 * 📊 SpectrumAnalyzer — 实时频谱显示 (连接全局 AudioContext)
 * 使用全局共享的 AnalyserNode，显示真实音频数据。
 * compact=true 时为紧凑工具栏版本。
 */
interface Props {
  height?: number;
  compact?: boolean;
...
export function SpectrumAnalyzer({ height = 80, compact = false }: Props) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx2d.restore();
      if (!disposed) rafRef.current = requestAnimationFrame(draw);
    };
