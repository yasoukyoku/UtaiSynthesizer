...
import { midiName, setVocalRangeBounds } from "../../lib/vocal/rangeTest";
...
export function RangeBoundsEditor({ sp, modelName, backend, speakerId, speakerLabel, lang, onClose, }) {
    ...
    const commit = async (usable, comfort) => {
        setBusy(true);
        try {
    ...
    return (_jsxs("div", { className: "rbe", children: [_jsx(ParamSlider, { label: t18({ zh: "可用下限", en: "Usable low", ja: "使用可能域 下限" }, lang), title: t18({
                ...
                }, lang), min: auto.usable[0], max: auto.usable[1], step: 1, value: preview.usable[0], onChange: (v) => setULo(v), format: midiName }), _jsx(ParamSlider, { label: t18({ zh: "可用上限", en: "Usable high", ja: "使用可能域 上限" }, lang), min: auto.usable[0], max: auto.usable[1], step: 1, value: preview.usable[1], onChange: (v) => setUHi(v), format: midiName }), _jsx(ParamSlider, { label: t18({ zh: "目标下限", en: "Target low", ja: "目標範囲 下限" }, lang), title: t18({
                    ...
                    ja: "救済された音はこの範囲に着地します——ここで指定した値が常に優先されます。アルゴリズムが問題ないと判定した着地先が耳に合わないときは上限を下げれば救済もそれに従います。届かない場合は自動の着地先に戻り、ログに記録されます。「リセット」で測定値に戻ります。",
                }, lang), min: auto.usable[0], max: auto.usable[1], step: 1, value: preview.comfort[0], onChange: (v) => setCLo(v), format: midiName }), _jsx(ParamSlider, { label: t18({ zh: "目标上限", en: "Target high", ja: "目標範囲 上限" }, lang), min: auto.usable[0], max: auto.usable[1], step: 1, value: preview.comfort[1], onChange: (v) => setCHi(v), format: midiName }), _jsxs("div", { className: "rbe-actions", children: [_jsx("button", { className: "rm-range-btn", disabled: busy, onClick: () => void commit(preview.usable, preview.comfort), children: "OK" }), _jsx("button", { className: "rm-range-btn", disabled: busy || !boundsAreEdited(sp), title: t18({
                        ...
                        }, lang), onClick: () => void commit(auto.usable, auto.comfort), children: t18({ zh: "还原", en: "Reset", ja: "リセット" }, lang) }), onClose && (_jsx("button", { className: "rm-range-btn", disabled: busy, onClick: onClose, children: t18({ zh: "取消", en: "Cancel", ja: "キャンセル" }, lang) }))] }), _jsx("div", { className: "rbe-note", children: t18({
