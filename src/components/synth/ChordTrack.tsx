...
export function ChordTrack() {
  ...
  const [editing, setEditing] = useState<{ index: number; left: number; width: number; value: string } | null>(null);
  ...
  const hitBoundary = useCallback((xLocal: number): number => {
    const segs = segsRef.current;
    for (let i = 0; i + 1 < segs.length; i++) {
      const bx = segs[i]!.endTick * pptRef.current - scrollXRef.current;
      if (Math.abs(xLocal - bx) <= 4) return i;
    }
    return -1;
  }, []);
  ...
  const handleMouseLeave = useCallback(() => {
    if (dragIdxRef.current !== null || hoverIdxRef.current === null) return;
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      for (let i = 0; i < segs.length; i++) {
        const s = segs[i]!;
        if (tick >= s.startTick && tick < s.endTick) { hit = i; break; }
      }
      if (hit < 0) return;
      const s = segs[hit]!;
  ...
  return (
    <div className="chord-track">
      ...
      {editing && (
        <input
          className="chord-track-edit"
          style={{ left: editing.left, width: editing.width }}
          value={editing.value}
          autoFocus
