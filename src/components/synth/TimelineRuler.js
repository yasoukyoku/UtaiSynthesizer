...
export function TimelineRuler() {
    ...
    const clickToTick = useCallback((clientX) => {
    ...
    const seekTickAt = useCallback((clientX) => {
    ...
    const draw = useCallback(() => {
        ...
        for (let bar = startBar;; bar++) {
            ...
            if (x > width)
                break; // past the right edge — every later bar is further right
    ...
    return (_jsx("canvas", { ref: canvasRef, className: "timeline-ruler", style: { cursor: "pointer" }, onMouseDown: handleMouseDown }));
}
