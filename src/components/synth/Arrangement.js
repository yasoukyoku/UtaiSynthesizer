...
import { laneGroupId, laneVisiblePieces, clipIndexAtMs, trimClip, isWholeClips, ticksToMs, laneOpsSig, materializeClips, laneRowKey, laneLabelParts, laneReachesSeam, flooredDurationTicks, MIN_LANE_PIECE_MS, segStretch, segmentSourceWindowMs, } from "../../lib/audio/laneOps";
...
function boundaryIndexAt(contentY, trks, offsets, totalH, pad) {
    for (let i = 0; i <= trks.length; i++) {
        const by = i < trks.length ? offsets[i] : totalH;
        if (Math.abs(contentY - by) <= pad)
            return i;
    }
    return null;
}
...
function gainPointAt(curve, segStartTick, xContent, yContent, bandTop, bandH, ppt) {
    let best = -1;
    let bestD = 8;
...
export function Arrangement() {
    ...
    const [stretchPanel, setStretchPanel] = useState(null);
    ...
    const dragDurationsRef = useRef({});
    const lastGhostInsertRef = useRef(""); // guards redundant store writes for the shared placeholder gap
    const dragPosRef = useRef(null); // last OS-drag PHYSICAL position
    const dragScrollRef = useRef(0); // rAF id for the OS-drag edge auto-scroll
    const headerElRef = useRef(null); // cached .daw-tracklist-wrap (re-queried if detached)
    ...
    useEffect(() => {
        const unsubApp = useAppStore.subscribe((s) => {
            ...
            if (s.scrollY !== scrollYRef.current) {
                scrollYRef.current = s.scrollY;
                changed = true;
            }
            ...
            if (s.vZoom !== vZoomRef.current) {
                vZoomRef.current = s.vZoom;
                changed = true;
            }
            if (changed)
                requestRedraw();
    ...
    const canvasToTick = useCallback((clientX) => {
    ...
    const playheadTickAt = useCallback((clientX) => {
    ...
    const hitTest = useCallback((clientX, clientY) => {
        ...
        for (let i = 0; i < tracks.length; i++) {
            ...
            if (!track)
                continue;
            if (hidden.has(track.id) || track.isFolder)
                continue;
            ...
            if (gBandH > 0 && y >= trackY + trackH - gBandH) {
                ...
                for (let si = track.segments.length - 1; si >= 0; si--) {
                    ...
                    if (x >= sx && x <= sx + sw)
                        return { trackIdx: i, segId: seg.id, zone: "gain", gain };
                }
            ...
            if (y <= trackY + headerH) {
                for (let si = track.segments.length - 1; si >= 0; si--) {
                    ...
                    if (x >= sx && x <= sx + sw) {
                        if (x - sx < EDGE_ZONE)
                            return { trackIdx: i, segId: seg.id, zone: "left" };
                        if (sx + sw - x < EDGE_ZONE)
                            return { trackIdx: i, segId: seg.id, zone: "right" };
            ...
            for (let si = track.segments.length - 1; si >= 0; si--) {
                ...
                if (!out)
                    return null; // the row exists for the track, but THIS segment has no such lane → empty
                ...
                const clipAt = (pc) => clipIndexAtMs(stored, stemDurMs, (pc.startMs + pc.endMs) / 2);
                ...
                for (const pc of pieces) {
                    ...
                    if (dl <= EDGE_ZONE && dl <= dr)
                        return { trackIdx: i, segId: seg.id, zone: "left", lane: { group, clipIndex } };
                    if (dr <= EDGE_ZONE)
                        return { trackIdx: i, segId: seg.id, zone: "right", lane: { group, clipIndex } };
                ...
                for (const pc of pieces) {
                    const leftPx = pc.startTick * ppt, rightPx = pc.endTick * ppt;
                    if (Math.abs(x - leftPx) <= EDGE_ZONE)
                        return { trackIdx: i, segId: seg.id, zone: "left", lane: { group, clipIndex: clipAt(pc) } };
                    if (Math.abs(x - rightPx) <= EDGE_ZONE)
                        return { trackIdx: i, segId: seg.id, zone: "right", lane: { group, clipIndex: clipAt(pc) } };
                }
    ...
    const hitTrackIdx = useCallback((clientY) => {
        const canvas = canvasRef.current;
        if (!canvas)
            return -1;
        const rect = canvas.getBoundingClientRect();
    ...
    const placementFromPhysical = useCallback((position) => {
        const canvas = canvasRef.current;
        if (!canvas)
        ...
        if (boundary !== null)
            return { target: "new", index: boundary, tick };
        // Over a track body.
        ...
        if (idx >= 0 && idx < trks.length && contentY <= totalH) {
            ...
            if (track.trackType === "audio" && !track.isFolder) {
                ...
                if (!clipCollides(track, tick, durTicks))
                    return { target: "insert", index: idx, tick };
    ...
    const applyDrag = useCallback((clientX, clientY) => {
        ...
        if (!drag.dragged && Math.hypot(clientX - drag.startMouseX, clientY - drag.startMouseY) > 3)
            drag.dragged = true;
        if (!drag.dragged)
            return;
        ...
        if (drag.mode === "gainPoint") {
            ...
            if (!seg || seg.content.type !== "audioClip" || drag.gainPointIndex === undefined)
                return;
            const curve = drag.gainGroup ? seg.laneLoudness?.[drag.gainGroup] : seg.content.paramCurves?.["loudness"];
            if (!curve || drag.gainPointIndex >= curve.xs.length)
                return;
            ...
            if (drag.gainGroup)
                useProjectStore.getState().setSegmentLaneLoudness(srcTrack.id, seg.id, drag.gainGroup, { xs, ys });
            else
        ...
        if (drag.mode === "laneResizeL" || drag.mode === "laneResizeR" || drag.mode === "laneBoundary") {
            const seg = srcTrack.segments.find((s) => s.id === drag.segId);
            if (!seg || seg.content.type !== "audioClip" || drag.laneGroup === undefined || drag.origClips === undefined)
                return;
            const tp = tempoRef.current;
            ...
            if (drag.mode === "laneBoundary") {
                if (newMs >= (drag.laneBoundaryMs ?? newMs)) {
                    drag.mode = "laneResizeL";
                    drag.laneClipIndex = drag.laneRightClipIndex ?? 0;
                }
                else {
                    drag.mode = "laneResizeR";
                    drag.laneClipIndex = drag.laneLeftClipIndex ?? 0;
                }
            }
            const clips = trimClip(drag.origClips, stemDurMs, drag.laneClipIndex ?? 0, drag.mode === "laneResizeL" ? "start" : "end", newMs, offset, winEnd);
        ...
        if (drag.mode === "move") {
            ...
            if (snapTargets) {
                ...
                if (snapTargets.includes(snappedStart))
                    snapHintRef.current = snappedStart;
                else if (snapTargets.includes(snappedStart + drag.origDurationTicks))
                    snapHintRef.current = snappedStart + drag.origDurationTicks;
            }
            ...
            if (drag.moving.length > 1) {
                ...
                for (const m of drag.moving)
                    origBySeg[m.segId] = m.origStartTick;
            ...
            if (targetTrack && targetIdx !== drag.trackIdx && srcTrack.trackType === targetTrack.trackType && !vocalMoveBlocked) {
                ...
                if (seg) {
                    ...
                    if (movedSeg.content.type === "notes" &&
                        ...
                        if (rest.length > 0)
                            movedSeg.processedOutputs = rest;
                        else
                            delete movedSeg.processedOutputs;
                    }
                    ...
                    const copied = { trackId: targetTrack.id, controlKeys: [], muteKeys: [] };
            ...
            updateTrack(srcTrack.id, {
                segments: srcTrack.segments.map((seg) => seg.id === drag.segId ? { ...seg, startTick: Math.max(0, drag.origStartTick + moveDelta) } : seg),
            });
            return;
        }
        ...
        updateTrack(srcTrack.id, {
            segments: srcTrack.segments.map((seg) => {
                ...
                if (drag.mode === "resizeL") {
                    ...
                    const newOff = seg.content.type === "audioClip"
                    ...
                    const rebase = (bag) => {
                        if (!bag)
                            return undefined;
                        if (shrink > 0)
                            return sliceParamCurves(bag, shrink, "right");
                        if (shrink < 0) {
                            return mapCurveBag(bag, (cv) => normalizeCurve({ xs: cv.xs.map((xv) => xv - shrink), ys: [...cv.ys] }, "param"));
                        }
                        return bag;
                    };
                    ...
                    if (paramCurves)
                        content.paramCurves = paramCurves;
                    else
                        delete content.paramCurves;
                    const nextSeg = { ...seg, startTick: newStart, durationTicks: newDur, content };
                    if (laneLoudness)
                        nextSeg.laneLoudness = laneLoudness;
                    else
                        delete nextSeg.laneLoudness;
                    return nextSeg;
                }
                ...
                if ((drag.mode === "laneResizeL" || drag.mode === "laneResizeR") && drag.dragged && drag.laneGroup !== undefined) {
                    ...
                    if (track && seg && seg.content.type === "audioClip") {
                        ...
                        if (stored && kept && kept.length !== stored.length) {
                            updateSegmentLaneOps(track.id, seg.id, drag.laneGroup, isWholeClips(kept, seg.content.totalDurationMs) ? undefined : kept);
                        }
                    }
                }
    ...
    const handleMouseDown = useCallback((e) => {
        ...
        if (hit) {
            ...
            if (hit.zone === "gain") {
                ...
                if (!gTrack || !gSeg || gSeg.content.type !== "audioClip" || !hit.gain || ctrl)
                    return;
                const ppt = pptRef.current;
                ...
                const writeCurve = (cv) => {
                    if (gainGroup)
                        useProjectStore.getState().setSegmentLaneLoudness(gTrack.id, gSeg.id, gainGroup, cv);
                    else
                        useProjectStore.getState().setSegmentParamCurve(gTrack.id, gSeg.id, "loudness", cv);
                };
                const R = LOUDNESS_DB_RANGE;
            ...
            let collapseTo = null;
            if (already)
                collapseTo = { trackId: track.id, segId: seg.id };
            else
                selectSegment(track.id, seg.id);
            const trks = tracksRef.current;
    ...
    const handleCanvasMouseMove = useCallback((e) => {
        const canvas = canvasRef.current;
        if (canvas)
            mouseXRef.current = e.clientX - canvas.getBoundingClientRect().left;
        if (!dragRef.current) {
            const hit = hitTest(e.clientX, e.clientY);
    ...
    const handleDoubleClick = useCallback((e) => {
        const hit = hitTest(e.clientX, e.clientY);
        if (hit?.zone === "gain")
            return; // S59: the loudness band never opens an editor
        if (!hit) {
            ...
            if (tr && tr.trackType === "vocal") {
                ...
                const overlapped = tr.segments.find((s) => s.content.type === "notes" && s.startTick < startTick + barLen && startTick < s.startTick + s.durationTicks);
                if (overlapped) {
        ...
        for (const tr of useProjectStore.getState().tracks) {
            ...
            if (sg.content.type === "notes" && sg.content.notes.length === 0) {
                ...
                const better = tr.segments.find((s) => s.id !== sg.id &&
        ...
        if (isNotes)
            openVocalEditor(openId);
        else
            openWorkflow(hit.segId);
    ...
    const maxScrollY = () => Math.max(0, computeTotalTracksHeight(tracksRef.current, vZoomRef.current) + TRACK_ADD_FOOTER - useAppStore.getState().canvasHeight);
    const handleWheel = useCallback((e) => {
        ...
        if (e.shiftKey)
            wheelDyRef.current += e.deltaY;
        else
            wheelDxRef.current += e.deltaY;
    ...
    const handleContextMenu = useCallback((e) => {
            ...
            if (hit.segId && hit.gain) {
                ...
                if (gTrack && gSeg && curve) {
                    ...
                    if (idx >= 0) {
                        ...
                        if (gainGroup)
                            useProjectStore.getState().setSegmentLaneLoudness(gTrack.id, gSeg.id, gainGroup, next);
                        else
                            useProjectStore.getState().setSegmentParamCurve(gTrack.id, gSeg.id, "loudness", next);
                        if (useAudioStore.getState().isPlaying)
                            useAudioStore.getState().bumpSchedule();
                    }
                }
        ...
        if (hit) {
            ...
            if (hit.lane) {
                if (track)
                    selectLane(track.id, hit.segId, hit.lane.group, hit.lane.clipIndex);
    ...
    const ctxItems = (() => {
        ...
        if (ctxMenu.lane && ctxMenu.segId) {
            ...
            if (seg?.content.type === "audioClip" && !seg.loading) {
                ...
                const lanePath = seg.processedOutputs?.filter((o) => !o.loading && o.outputNodeId === group).slice(-1)[0]?.audioPath
                    ?? clip.sourcePath;
                if (lanePath) {
                    items.push({
                        type: "submenu",
            ...
            if (seg?.content.type === "audioClip" && !seg.loading) {
                ...
                if (td) {
                    ...
                    items.push({
                        ...
                        onClick: () => { if (x2)
                            useProjectStore.getState().setSegmentTempoDetect(track.id, segId, x2); },
                    });
                    items.push({
                        label: t("tempo.half"), disabled: !half,
                        onClick: () => { if (half)
                            useProjectStore.getState().setSegmentTempoDetect(track.id, segId, half); },
                    });
                    items.push({
                        label: t("tempo.nudge"),
                ...
                const finalPath = seg.processedOutputs?.filter((o) => !o.loading).slice(-1)[0]?.audioPath ?? clip.sourcePath;
                items.push({
                    type: "submenu",
                    label: i18n.t("songMenu.remixGroup"),
                    icon: "🎤",
    ...
    const importDroppedFiles = useCallback(async (paths, place, durations) => {
    ...
    const syncGhostInsert = useCallback((place) => {
        const trks = tracksRef.current;
        const gi = place && place.target === "new" && place.index < trks.length
    ...
    const startDragAutoScroll = useCallback(() => {
        ...
        const tick = () => {
            ...
            const canvas = canvasRef.current;
            if (!pos || !canvas || dragPathsRef.current.length === 0)
                return; // drag ended / no audio drag
            const rect = canvas.getBoundingClientRect();
        ...
        getCurrentWebview()
            ...
            if (p.type === "enter") {
                ...
                if (audioPaths.length === 0)
                    return; // non-audio drag: no affordance
                setDragOver(true);
    ...
    const draw = useCallback(() => {
        ...
        // anything is loading. (Static layer paints the base box; this overlays the loading look.)
        {
            ...
            for (let i = 0; i < tracks.length; i++) {
                ...
                if (trackY + computeTrackHeight(track, scale) < 0 || trackY > height)
                    continue;
                ...
                for (const seg of track.segments) {
                    ...
                    if (isSegLoading(seg, loadingPaths)) {
                        drawLoadingIndicator(ctx, sx, trackY + 2, sw, headerH - 4, c, now, label);
                    }
                    ...
                    if (track.expanded && seg.processedOutputs) {
                        ...
                        for (const out of seg.processedOutputs) {
                            ...
                            if (out.loading) {
                                drawLoadingIndicator(ctx, sx, laneY, sw, laneH, c, now, label);
                                continue;
                            }
                            ...
                            if (midiExtracting[exKey]) {
                                ...
                                drawLoadingIndicator(ctx, sx, laneY, sw, laneH, c, now, `${t("midiExtract.extracting")} ${pct}%`);
                            }
                        }
                    }
                }
        ...
        if (ghost && ghost.target !== "none") {
            ...
            const durAt = (i) => (paths[i] ? dragDurationsRef.current[paths[i]] : undefined) ?? DEFAULT_DURATION_MS;
            const drawBox = (gy, durMs, rgb = [96, 165, 250]) => {
            ...
            if (ghost.target === "insert") {
                ...
                if (fileCount > 1) {
                    ...
                    for (let i = 1; i < fileCount; i++)
                        drawBox(bottom + 2 + (i - 1) * rowH, durAt(i), ghostRgb(i));
                }
            }
            else {
                const rowTop = (ghost.index < trks.length ? offsets[ghost.index] : computeTotalTracksHeight(trks, scale)) - scrollY;
                for (let i = 0; i < fileCount; i++)
                    drawBox(rowTop + 2 + i * rowH, durAt(i), ghostRgb(i));
            }
            ctx.restore();
        }
    ...
    return (_jsxs("div", { className: "arrangement", children: [_jsx("canvas", { ref: canvasRef, className: "arrangement-canvas", style: { cursor }, onMouseDown: handleMouseDown, onMouseMove: handleCanvasMouseMove, onMouseLeave: handleMouseLeave, onDoubleClick: handleDoubleClick, onContextMenu: handleContextMenu, onWheel: handleWheel }), ctxMenu && _jsx(ContextMenu, { x: ctxMenu.x, y: ctxMenu.y, items: ctxItems, onClose: () => setCtxMenu(null) }), stretchPanel && _jsx(TempoStretchPanel, { ...stretchPanel, onClose: () => setStretchPanel(null) })] }));
}
function drawStaticContent(ctx, width, height, tracks, audioFiles, ppt, scrollX, scrollY, axis, tempo, selectedKeys, selectedLaneKey, dragOver, scale, ghostGap) {
    ...
    for (let i = 0; i < tracks.length; i++) {
        ...
        if (y + trackH < -CULL_PAD || y > height + CULL_PAD)
            continue;
        ctx.strokeStyle = rgba(SEPARATOR_RGB, 0.8);
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(0, y + trackH);
        ctx.lineTo(width, y + trackH);
        ctx.stroke();
        ...
        for (const seg of track.segments) {
            ...
            if (sx > width + CULL_PAD || sx + sw < -CULL_PAD)
                continue;
            const sy = y + 2;
            const sh = headerH - 4;
            ...
            if (seg.content.type === "audioClip" && seg.content.tempoDetect && sw > 24) {
                ...
                const msAt = (vx) => offMs + ((vx - sx) / sw) * srcWinMs;
                    ...
                    if (out.waveformPeaks && out.waveformPeaks.length > 0 && sw > 2) {
                        ...
                        if (!stored) {
                            ...
                            for (const p of laneVisiblePieces(seg, stored, lTotalMs, tempo, out.offsetMs ?? 0)) {
                                ...
                                if (pw < 0.5 || px > width + CULL_PAD || px + pw < -CULL_PAD)
                                    continue;
                    ...
                    if (track.laneLoudnessOpen?.[group]) {
                        ...
                        if (cv && cv.xs.length > 0) {
                            ...
                            for (let pi = 0; pi < cv.xs.length; pi++) {
                                ...
                                if (px < -CULL_PAD || px > width + CULL_PAD)
                                    continue;
        ...
        if (track.expanded && lanes.length > 0) {
            ...
            for (let r = 0; r < layout.runs.length; r++) {
                ...
                ctx.stroke();
                ctx.strokeStyle = rgba(SEPARATOR_RGB, 0.4);
                ctx.lineWidth = 0.5;
                ctx.beginPath();
                ...
                for (let k = 1; k < run.count; k++) {
                    ...
                    ctx.strokeStyle = rgba(SEPARATOR_RGB, 0.5);
                    ctx.lineWidth = 0.5;
                    ctx.beginPath();
                    ctx.moveTo(0, ry);
                    ctx.lineTo(width, ry);
        ...
        // track's bottom strip (computeTrackHeight includes it; it is NOT a lane row).
        {
            ...
            if (gBandH > 0) {
                ...
                for (const seg of track.segments) {
                    ...
                    if (gsx > width + CULL_PAD || gsx + gsw < -CULL_PAD)
                        continue;
        ...
        for (let si = 0; si + 1 < timeSorted.length; si++) {
            ...
            if (next.startTick < segEnd) {
                ...
                if (ox > width + CULL_PAD || ox + ow < -CULL_PAD)
                    continue; // cull crossfades scrolled out of view
                drawCrossfade(ctx, ox, y + 2, ow, headerH - 4, c);
                ...
                if (xfLanes) {
                    for (const out of seg.processedOutputs ?? []) {
                        ...
                        if (!laneReachesSeam(seg, rowKey, tempo, "end") || !laneReachesSeam(next, rowKey, tempo, "start"))
                            continue;
                        const li = layout.rowByKey.get(rowKey) ?? -1;
                        if (li < 0)
                            continue;
                        drawCrossfade(ctx, ox, y + layout.rowY[li] * scale, ow, laneH, c);
                    }
                }
            }
        }
...
function isSegLoading(seg, loadingPaths) {
    if (seg.loading)
        return true;
...
function drawLoadingIndicator(ctx, x, y, w, h, c, now, label) {
    if (w < 2)
        return;
    ctx.save();
    ctx.beginPath();
...
function drawCrossfade(ctx, x, y, w, h, c) {
    if (w < 2)
        return;
    ctx.save();
