...
interface BuildMenuOptions {
  ...
  hasNotes: boolean;
  audioPath?: string;
  onAmt?: (quality: 'fast' | 'standard' | 'best' | 'guitar' | 'piano') => void;
  onStemSeparation?: (quality: 'fast' | 'standard' | 'pro' | 'best') => void;
  onQuantize?: (gridSize: number) => void;
  onHumanize?: () => void;
  onVelocityCurve?: (type: 'crescendo' | 'diminuendo' | 'exponential') => void;
  onSoundfontRender?: () => void;
  onArrange?: () => void;
  onChordMidi?: () => void;
  onDetectChords?: () => void;
  onDetectDrums?: () => void;
  onSongStudio?: () => void;
  onMultiTrack?: () => void;
  onCreativeAssistant?: () => void;
  onCoverAssistant?: () => void;
  onMidiEditor?: () => void;
  onRename?: () => void;
  onCopy?: () => void;
  onPaste?: () => void;
...
export function buildMenuItems(options: BuildMenuOptions): MenuItem[] {
  const {
    isAudioTrack,
    isMidiTrack,
    hasNotes,
  ...
  items.push(
    {
      ...
      items: [
        {
          label: '✍️ 打开歌曲工作室',
          title: '完整的歌曲生成、翻唱、重绘等功能',
          onClick: onSongStudio
        },
        {
          label: '🎛️ 高级多轨工作室',
          title: '专业多轨编辑和制作',
          onClick: onMultiTrack,
          disabled: !isAudioTrack || !audioPath
        },
        {
          label: '🪄 创作助手',
          title: '单乐器叠加、乐谱续写等快捷功能',
          onClick: onCreativeAssistant
        },
        {
          label: '🎤 翻唱助手',
          title: '快速翻唱和风格转换',
          onClick: onCoverAssistant,
          disabled: !isAudioTrack || !audioPath
        },
        {
          label: '🎹 MIDI 编辑器',
          title: '编辑和导出 MIDI 文件',
          onClick: onMidiEditor,
          disabled: !isMidiTrack && !(isAudioTrack && audioPath)
        }
      ]
    }
  );
