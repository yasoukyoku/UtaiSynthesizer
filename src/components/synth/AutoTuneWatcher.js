 ...
 */
function followSig(content, tempo, p) {
    ...
    useEffect(() => {
        ...
        async function sweep() {
            ...
            if (busyRef.current || inGestureTransaction()) {
                ...
                if (!busyRef.current)
                    scheduleRetry(RETRY_MS); // 手势让路:busy 的 finally 无人补排,自排
                return;
            }
            ...
            try {
                ...
                for (const t of st.tracks) {
                    ...
                    for (const seg of t.segments) {
                        ...
                        if (!follow || doneRef.current.get(seg.id) === sig)
                            continue;
                        let res;
                        try {
                ...
                // 修剪(删段/换工程的滞留条目)
                for (const k of [...doneRef.current.keys()])
                    if (!liveIds.has(k))
                        doneRef.current.delete(k);
            }
            finally {
