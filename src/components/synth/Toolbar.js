...
import { formatBarBeat } from "../../lib/timeAxis";
...
export function Toolbar() {
    ...
    const [selectedKey, setSelectedKey] = useState(""); // 空字符串表示未检测到调性
    ...
    const [skinPos, setSkinPos] = useState(null);
    const THEMES = [
    ...
    const onPlaybackEnded = useCallback(() => {
        ...
        if (end > 0 && useProjectStore.getState().playheadTick < end - 1)
            return;
        animatingRef.current = false;
        setPlaying(false);
        if (end > 0)
    ...
    const rescheduleNow = useCallback((overrideTick) => {
    ...
    const handleTogglePlay = async () => {
        ...
        try {
            ...
            if (dirty.length > 0 && !workflowBusy) {
                ...
                if (autoRenderAbortRef.current)
                    return; // user cancelled the batch → don't start playback
            }
            ...
            const result = await playback.playAllTracks(freshTracks, useAudioStore.getState().audioFiles, startTick, freshTempo, onPlaybackEnded);
            if (result === "started") {
                setPlaying(true);
            }
        }
    ...
    useEffect(() => {
        const onKey = (e) => {
            ...
            if (e.key === " ") {
                ...
                if (useAppStore.getState().activePane !== "timeline")
                    return;
                e.preventDefault();
    ...
    return (_jsxs("div", { className: "toolbar", children: [_jsxs("div", { className: "toolbar-left", children: [_jsxs("div", { className: "toolbar-section snap-section", children: [_jsx("button", { className: `snap-toggle ${snapSegments ? "active" : ""}`, onClick: toggleSnapSegments, title: t("toolbar.snapClipTip"), children: _jsx("svg", { viewBox: "0 0 24 24", width: "15", height: "15", "aria-hidden": "true", children: _jsx("path", { fill: "currentColor", d: "M3 7v6a9 9 0 0 0 18 0V7h-4v6a5 5 0 0 1-10 0V7z M3 3h4v4H3z M17 3h4v4h-4z" }) }) }), _jsx("button", { className: `snap-toggle ${snapPlayhead ? "active" : ""}`, onClick: toggleSnapPlayhead, title: t("toolbar.snapPlayheadTip"), children: _jsx("svg", { viewBox: "0 0 24 24", width: "15", height: "15", "aria-hidden": "true", children: _jsx("path", { fill: "currentColor", d: "M6 3h12l-6 7z M11 9h2v12h-2z" }) }) }), _jsx(Dropdown, { className: "snap-grid-dropdown", value: snapGridStep, options: SNAP_GRID_OPTIONS.map(({ value, labelKey }) => ({ value, label: t(labelKey) })), onChange: setSnapGridStep })] }), _jsx("div", { className: "toolbar-divider" })] }), _jsxs("div", { className: "toolbar-center", children: [_jsxs("div", { className: "toolbar-section transport", children: [_jsx("button", { className: "transport-btn transport-btn-sm", onClick: handleReturnToStart, title: "\u56DE\u5230\u5F00\u5934", children: _jsx("span", { className: "transport-icon icon-return" }) }), _jsx("button", { className: `transport-btn transport-btn-lg play ${isPlaying ? "playing" : ""} ${autoRendering ? "rendering" : ""} ${preparing && !autoRendering ? "preparing" : ""}`, onClick: handleTogglePlay, title: autoRendering ? t("vocalEditor.render.autoRenderingCancel") : preparing ? t("transport.preparing") : isPlaying ? "暂停 (空格)" : "播放 (空格)", children: _jsx("span", { className: `transport-icon ${isPlaying ? "icon-pause" : "icon-play"}` }) })] }), _jsx("div", { className: "toolbar-overlay", children: _jsx(OverviewMap, {}) })] }), _jsxs("div", { className: "toolbar-right", children: [_jsxs("div", { className: "toolbar-section tempo-section", children: [_jsx("label", { className: "toolbar-label", children: t("toolbar.bpm") }), _jsx("input", { type: "number", className: "tempo-input mono", value: tempo, min: 20, max: 400, step: 1, onFocus: () => { useHistoryStore.getState().beginTransaction(); useProjectStore.getState().beginTempoScale(); }, onBlur: () => { useProjectStore.getState().endTempoScale(); useHistoryStore.getState().commitTransaction(); }, onKeyDown: (e) => { if (e.key === "Enter")
                                    e.target.blur(); }, onChange: (e) => setTempo(Number(e.target.value)) })] }), _jsx("div", { className: "toolbar-section time-sig-clickable", onClick: () => setTimeSigOpen(true), title: t("toolbar.timeSignature"), children: _jsxs("span", { className: "mono time-sig-display", children: [timeSignature[0], "/", timeSignature[1]] }) }), _jsx("div", { className: "toolbar-section key-selector", onClick: () => setKeyOpen(true), title: t("toolbar.key"), children: selectedKey ? (_jsx("span", { className: "key-display", children: selectedKey })) : (_jsxs("svg", { viewBox: "0 0 16 16", width: "16", height: "16", "aria-hidden": "true", children: [_jsx("path", { d: "M3 8a5 5 0 0 1 10 0M8 8v5", stroke: "currentColor", strokeWidth: "1.4", strokeLinecap: "round", fill: "none" }), _jsx("circle", { cx: "8", cy: "8", r: "1.5", fill: "currentColor" })] })) }), _jsx("div", { className: "toolbar-divider" }), _jsxs("div", { className: "toolbar-section position-section", children: [_jsx("label", { className: "toolbar-label", children: t("toolbar.position") }), _jsx("span", { ref: positionDisplayRef, className: "mono position-display", children: formatPosition(playheadRef.current, timeAxis) })] }), _jsx("div", { className: "toolbar-divider" }), _jsxs("div", { className: "toolbar-section utility-section", children: [_jsx("button", { className: "toolbar-btn icon-btn", title: t("history.panelTitle"), onClick: () => setHistoryOpen(true), children: _jsxs("svg", { viewBox: "0 0 16 16", width: "14", height: "14", "aria-hidden": "true", children: [_jsx("path", { d: "M8 1.5a6.5 6.5 0 1 1-6.5 6.5", fill: "none", stroke: "currentColor", strokeWidth: "1.4", strokeLinecap: "round" }), _jsx("path", { d: "M1.5 8V3.5M1.5 3.5H6", stroke: "currentColor", strokeWidth: "1.4", strokeLinecap: "round", strokeLinejoin: "round" })] }) }), _jsx("button", { className: "toolbar-btn icon-btn", title: t("titlebar.shortcuts"), onClick: () => setShortcutsOpen(true), children: _jsxs("svg", { viewBox: "0 0 16 16", width: "14", height: "14", "aria-hidden": "true", children: [_jsx("rect", { x: "1", y: "2", width: "14", height: "11", rx: "2", fill: "none", stroke: "currentColor", strokeWidth: "1.2" }), _jsx("path", { d: "M4 12.5v-2M4 8.5v-1M12 12.5v-6M10 9.5v-1M10 13v-.4M2.5 4.5h3M11.5 4.5h2", stroke: "currentColor", strokeWidth: "1.2", strokeLinecap: "round" })] }) }), _jsx("button", { className: "toolbar-btn icon-btn", title: t("titlebar.skin"), onClick: (e) => {
                                    const r = e.currentTarget.getBoundingClientRect();
                                    setSkinPos({ top: r.bottom + 6, right: window.innerWidth - r.right });
                                    setSkinOpen((v) => !v);
                                }, children: _jsxs("svg", { viewBox: "0 0 16 16", width: "14", height: "14", "aria-hidden": "true", children: [_jsx("circle", { cx: "5", cy: "4", r: "1.6", fill: "#fdba74" }), _jsx("circle", { cx: "9.6", cy: "3.2", r: "1.6", fill: "#4ade80" }), _jsx("circle", { cx: "12.6", cy: "6.4", r: "1.6", fill: "#60a5fa" }), _jsx("circle", { cx: "11.6", cy: "10.4", r: "1.6", fill: "#a78bfa" }), _jsx("circle", { cx: "7.4", cy: "11.4", r: "1.6", fill: "#f87171" })] }) })] })] }), skinOpen && (_jsxs("div", { className: "skin-popover", style: skinPos ?? undefined, children: [_jsx("div", { className: "skin-popover-title", children: t("titlebar.themeTitle") }), _jsx("div", { className: "theme-grid", children: THEMES.map((item, idx) => (_jsxs("button", { className: `theme-item ${item.theme === theme && item.skin === skin ? "active" : ""}`, onClick: () => {
                                setTheme(item.theme);
                                setSkin(item.skin);
                                setSkinOpen(false);
                            }, children: [_jsx("span", { className: "theme-color-dot", style: { backgroundColor: item.color } }), _jsx("span", { className: "theme-label", children: item.label })] }, idx))) })] })), timeSigOpen && (_jsxs(_Fragment, { children: [_jsx("div", { className: "popover-backdrop", onClick: () => setTimeSigOpen(false) }), _jsxs("div", { className: "time-sig-popover", children: [_jsx("div", { className: "popover-title", children: t("toolbar.timeSignature") }), _jsxs("div", { className: "time-sig-editor", children: [_jsxs("div", { className: "time-sig-row", children: [_jsx("label", { children: "\u5206\u5B50" }), _jsx("div", { className: "time-sig-options", children: TS_NUM_OPTIONS.slice(0, 8).map(opt => (_jsx("button", { className: `time-sig-option ${timeSignature[0] === opt.value ? "active" : ""}`, onClick: () => {
                                                        setTimeSignature(opt.value, timeSignature[1]);
                                                        setTimeSigOpen(false);
                                                    }, children: opt.label }, opt.value))) })] }), _jsxs("div", { className: "time-sig-row", children: [_jsx("label", { children: "\u5206\u6BCD" }), _jsx("div", { className: "time-sig-options", children: TS_DEN_OPTIONS.map(opt => (_jsx("button", { className: `time-sig-option ${timeSignature[1] === opt.value ? "active" : ""}`, onClick: () => {
                                                        setTimeSignature(timeSignature[0], opt.value);
                                                        setTimeSigOpen(false);
                                                    }, children: opt.label }, opt.value))) })] })] })] })] })), keyOpen && (_jsxs(_Fragment, { children: [_jsx("div", { className: "popover-backdrop", onClick: () => setKeyOpen(false) }), _jsxs("div", { className: "key-popover", children: [_jsx("div", { className: "popover-title", children: t("toolbar.key") }), _jsxs("div", { className: "key-section", children: [_jsx("div", { className: "key-section-title", children: t("toolbar.keyMajor") }), _jsx("div", { className: "key-grid", children: ["C", "G", "D", "A", "E", "B", "F♯", "C♯", "F", "B♭", "E♭", "A♭"].map(key => (_jsx("button", { className: `key-option ${selectedKey === key ? "active" : ""}`, onClick: () => {
                                                setSelectedKey(key);
                                                setKeyOpen(false);
                                            }, children: key }, key))) })] }), _jsxs("div", { className: "key-section", children: [_jsx("div", { className: "key-section-title", children: t("toolbar.keyMinor") }), _jsx("div", { className: "key-grid", children: ["Am", "Em", "Bm", "F♯m", "C♯m", "G♯m", "D♯m", "A♯m", "Dm", "Gm", "Cm", "Fm"].map(key => (_jsx("button", { className: `key-option ${selectedKey === key ? "active" : ""}`, onClick: () => {
                                                setSelectedKey(key);
                                                setKeyOpen(false);
                                            }, children: key }, key))) })] })] })] })), shortcutsOpen && _jsx(ShortcutsDialog, { onClose: () => setShortcutsOpen(false) }), historyOpen && _jsx(HistoryPanel, { onClose: () => setHistoryOpen(false) })] }));
}
...
const formatPosition = (tick, axis) => formatBarBeat(axis, tick);
