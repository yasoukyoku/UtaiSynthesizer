...
export function TempoStretchPanel({ x, y, trackId, segId, onClose }) {
    ...
    const [target, setTarget] = useState(() => (td ? td.bpm / r : 100 / r));
    ...
    return (_jsxs(_Fragment, { children: [_jsx("div", { className: "stretch-backdrop", onMouseDown: onClose, onContextMenu: (e) => { e.preventDefault(); onClose(); } }), _jsxs("div", { className: "stretch-panel", style: { left: px, top: py, width: pw }, onMouseDown: (e) => e.stopPropagation(), children: [_jsx("div", { className: "stretch-title", children: t("tempo.stretchTitle") }), _jsx(ParamSlider, { label: td ? t("tempo.targetBpm") : t("tempo.speedPct"), min: Math.round(min * 10) / 10, max: Math.round(max * 10) / 10, step: td ? 0.1 : 0.5, value: Math.round(target * 10) / 10, onChange: setTarget, format: (v) => (td ? v.toFixed(1) : `${v.toFixed(0)}%`), disabled: busy }), _jsxs("div", { className: "stretch-readout", children: [td && _jsxs("span", { children: [t("tempo.detectedBpm"), ": ", td.bpm.toFixed(1)] }), _jsxs("span", { children: [t("tempo.durationPct"), ": ", (factor * 100).toFixed(1), "%"] }), r !== 1 && _jsxs("span", { children: [t("tempo.current"), ": \u00D7", r.toFixed(3)] })] }), _jsxs("div", { className: "stretch-actions", children: [_jsx("button", { className: "stretch-btn", disabled: busy || !changed, onClick: () => run(factor), children: busy ? t("tempo.processing") : t("tempo.apply") }), _jsx("button", { className: "stretch-btn", disabled: busy || r === 1, onClick: () => run(1), children: t("tempo.reset") })] })] })] }));
}
