...
interface ClipInfo {
  ...
  offsetMs: number;
  durMs: number;
  /** 片段时间轴位置(tick) — 播放时据此判断播放头是否落在本 clip 内 */
  startTick: number;
  durTicks: number;
  /** S59 tempo stretch(播放时长/源时长,>1=更慢) — 播放头→源 ms 换算要除它 */
  stretch: number;
}
 ...
 *  颜色跟随轨道色。 */
export function TrackWaveform({ track, height }: { track: Track; height: number }) {
  ...
  for (const seg of track.segments) {
    if (seg.content.type !== "audioClip" || seg.loading) continue;
    const c = seg.content;
    if (c.totalDurationMs <= 0) continue;
    clips.push({
      src: c.sourcePath,
      offsetMs: c.offsetMs,
  ...
  const peakMaxRef = useRef<Map<string, number>>(new Map());
  ...
  useEffect(() => {
    ...
    if (!hasData || !isPlaying) return; // 暂停: 只保留上面 clearRect — 完全留白

    const rgb = resolveRgb(colorVar);

    /** 采样某通道 peaks 在 atMs 处 ±winMs 窗口的平均峰值(归一化后)。 */
    const sampleAt = (peaks: number[], maxKey: string, srcMs: number, atMs: number, winMs: number, gain: number): number => {
  ...
  return <canvas ref={canvasRef} className="track-waveform-canvas" style={{ width: 48, height }} />;
}

/** 把 "var(--track-vocal)" 或 "#hex" 解析成可用的 fill 颜色。canvas 不认 CSS 变量。 */
function resolveRgb(color: string): string {
