...
export function DawView() {
    ...
    const onHeaderDividerDown = useCallback((e) => {
        if (e.button !== 0)
            return;
        e.preventDefault();
        e.stopPropagation();
        ...
        const onUp = () => {
            ...
            if (headerWDragRafRef.current) {
                cancelAnimationFrame(headerWDragRafRef.current);
                headerWDragRafRef.current = 0;
            }
        ...
        const handler = (e) => {
            ...
            if (e.key.toLowerCase() === "d") {
                ...
                runAutoArrange(tid, "pop", "neutral").then((ok) => {
                    if (ok)
                        useAppStore.getState().showToast("✅ 已生成默认伴奏 (Ctrl+Z 撤销, Ctrl+Shift+D 再次生成, Ctrl+Shift+A 打开编曲面板)", "success");
                });
            }
                ...
                runChordMidi(tid, { chordsPerBar: 1, style: "POP_STANDARD", key: "auto" }).then((ok) => {
                    if (ok)
                        useAppStore.getState().showToast("✅ 和弦轨已生成 (Ctrl+Z 撤销)", "success");
                });
            }
        };
    ...
    const handleTrackListWheel = useCallback((e) => {
        e.stopPropagation();
        const st = useAppStore.getState();
    ...
    return (_jsxs("div", { className: "daw-view", onPointerDownCapture: () => useAppStore.getState().setActivePane("timeline"), children: [_jsx(Toolbar, {}), _jsxs("div", { className: "daw-grid", children: [_jsxs("div", { className: "daw-corner", style: { width: headerWidth }, children: [_jsx("button", { className: "corner-btn", onClick: () => addTrack(blankTrack(crypto.randomUUID(), `Audio ${useProjectStore.getState().tracks.length + 1}`, "audio")), title: t("toolbar.addTrack"), children: "+" }), _jsx("button", { className: "corner-btn", onClick: async () => {
                                    ...
                                    if (file) {
                                        ...
                                        await importAudioToNewTrack(file, useProjectStore.getState().playheadTick);
                                    }
                                }, title: "\u4E0A\u4F20\u97F3\u9891\u6587\u4EF6", children: "\uD83C\uDFB5" }), _jsx("button", { className: "corner-btn", onClick: () => {
                                    const track = blankTrack(crypto.randomUUID(), `Instrument ${useProjectStore.getState().tracks.length + 1}`, "instrument");
                                    addTrack(track);
                                }, title: "\u65B0\u5EFAMIDI\u8F68\u9053", children: "D" })] }), _jsx(TimelineRuler, {}), _jsx("div", { className: "daw-tracklist-wrap", onWheel: handleTrackListWheel, children: _jsx(TrackList, { width: headerWidth }) }), _jsx("div", { className: "daw-header-divider", onPointerDown: onHeaderDividerDown, title: "\u62D6\u62FD\u8C03\u6574\u8F68\u9053\u9762\u677F\u5BBD\u5EA6" }), _jsx("div", { className: "daw-canvas-container", ref: canvasContainerRef, children: _jsx(Arrangement, {}) }), _jsx("div", { className: "daw-scrollbar-corner", style: { width: headerWidth } }), _jsx(HScrollbar, { totalWidth: totalWidth, viewWidth: canvasWidth, onChange: (x) => { const st = useAppStore.getState(); st.setScroll(x, st.scrollY); } })] })] }));
}
